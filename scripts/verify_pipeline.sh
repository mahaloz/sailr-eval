...
sudo ./docker-eval.sh --decompile example_project --use-dec ghidra angr_sailr angr_phoenix angr_dream angr_comb --cores "$CORES" && \
echo "[+] Running measurement pipeline (gotos, bools, calls, cfged)..." && \
./eval.py --measure example_project --use-metric gotos cfged bools func_calls --use-dec source ghidra angr_sailr angr_phoenix angr_dream angr_comb --cores "$CORES" && \
echo "[+] Running aggregation pipeline..." && \
./eval.py --summarize-targets example_project --use-dec source ghidra angr_sailr angr_phoenix angr_dream angr_comb --use-metric gotos cfged bools func_calls --show-stats && \
# cleanup
