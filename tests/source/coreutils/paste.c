  ...
  if (opened_stdin && have_read_stdin)
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"standard "
                        "input is closed\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0, dcgettext(((void *)0), "standard input is closed", 5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0, dcgettext(((void *)0), "standard input is closed", 5)),
             ((0) ? (void)0 : __builtin_unreachable()))));
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "d:sz", longopts, ((void *)0))) !=
         -1) {
    switch (optc) {
    case 'd':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "paste", "GNU coreutils", Version, ("David M. Ihnat"),
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"delimiter list ends with "
             "an unescaped backslash: %s\", 5), quotearg_n_style_colon (0, "
             "c_maybe_quoting_style, delim_arg)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
  ...
  _Bool ok =
      ((serial_merge ? paste_serial : paste_parallel)(nfiles, &argv[optind]));
