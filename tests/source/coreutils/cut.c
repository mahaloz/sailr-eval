...
extern ssize_t getndelim2(char **lineptr, size_t *linesize, size_t offset,
                          size_t nmax, int delim1, int delim2, FILE *stream);
...
enum { OUTPUT_DELIMITER_OPTION = 0x7f + 1, COMPLEMENT_OPTION };
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             "  -b, --bytes=LIST        select only these "
                             "bytes\n  -c, --characters=LIST   select only "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "      --complement        complement the set of selected "
                  "bytes, characters\n                            or fields\n",
                  5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "delimiters\n      --output-delimiter=STRING  use STRING as "
...
static inline void next_item(uintmax_t *item_idx) {
  (*item_idx)++;
  ...
  while ((optc = getopt_long(argc, argv, "b:c:d:f:nsz", longopts,
    ...
    case 'f':
      ...
      if (spec_list_string)
        do {
          error(0, 0,
                (dcgettext(((void *)0), "only one list may be specified", 5)));
          usage(1);
        } while (0);
    ...
    case 'd':
      ...
      if (optarg[0] != '\0' && optarg[1] != '\0')
        do {
          error(0, 0,
                (dcgettext(((void *)0),
                           "the delimiter must be a single character", 5)));
          usage(1);
        } while (0);
      delim = optarg[0];
      delim_specified = 1;
  ...
  if (!spec_list_string)
    do {
      error(0, 0,
            (dcgettext(
                ((void *)0),
                "you must specify a list of bytes, characters, or fields", 5)));
      usage(1);
    } while (0);
  ...
  if (byte_mode) {
    if (delim_specified)
      do {
        error(0, 0,
              (dcgettext(((void *)0),
                         "an input delimiter may be specified only when "
                         "operating on fields",
                         5)));
        usage(1);
      } while (0);
