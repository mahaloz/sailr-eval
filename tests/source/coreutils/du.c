...
struct argv_iterator *argv_iter_init_argv(char **argv)
    ...
    __attribute__((__nonnull__(1)))
    __attribute__((__malloc__(argv_iter_free, 1)));
...
char *argv_iter(struct argv_iterator *, enum argv_iter_err *)
    ...
    __attribute__((__nonnull__(1, 2)));
size_t argv_iter_n_args(struct argv_iterator const *) __attribute__((__pure__))
__attribute__((__nonnull__(1)));
...
struct di_set *di_set_alloc(void) __attribute__((__malloc__))
__attribute__((__malloc__(di_set_free, 1)));

int di_set_insert(struct di_set *, dev_t, ino_t)
    __attribute__((__nonnull__(1)));
...
void free_exclude(struct exclude *) __attribute__((__nonnull__(1)));
struct exclude *new_exclude(void) __attribute__((__malloc__))
__attribute__((__returns_nonnull__))
__attribute__((__malloc__(free_exclude, 1)));
...
FTS *xfts_open(char *const *, int options,
...
static inline void duinfo_init(struct duinfo *a) {
  a->size = 0;
  a->inodes = 0;
  a->tmax.tv_sec = ((time_t) ~(
...
static inline void duinfo_set(struct duinfo *a, uintmax_t size,
                              struct timespec tmax) {
  a->size = size;
  a->inodes = 1;
  a->tmax = tmax;
...
static inline void duinfo_add(struct duinfo *a, struct duinfo const *b) {
...
enum time_type { time_mtime, time_ctime, time_atime };
...
static char const *const time_args[] = {"atime",  "access",   "use", "ctime",
                                        "status", ((void *)0)

};
static enum time_type const time_types[] = {time_atime, time_atime, time_atime,
...
enum time_style { full_iso_time_style, long_iso_time_style, iso_time_style };
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "  -0, --null            end each output line with NUL, not "
            "newline\n  -a, --all             write counts for all files, not "
            "just directories\n      --apparent-size   print apparent sizes "
            "rather than device usage; although\n                          the "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "a grand total\n  -D, --dereference-args  dereference only "
            "symlinks that are listed on the\n                          "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "the default)\n  -S, --separate-dirs   for directories do not "
            "include size of subdirectories\n      --si              like -h, "
            "but use powers of 1000 not 1024\n  -s, --summarize       display "
            "only a total for each argument\n",
            5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -X, --exclude-from=FILE  exclude files that match any "
                  "pattern in FILE\n      --exclude=PATTERN    exclude files "
                  "that match PATTERN\n  -x, --one-file-system    skip "
...
static void show_date(char const *format, struct timespec when, timezone_t tz) {
  ...
  if (localtime_rz(tz, &when.tv_sec, &tm))
    ...
    error(0, 0, dcgettext(((void *)0), "time %s is out of range", 5),
          quote(when_str));
...
static void print_size(const struct duinfo *pdui, char const *string) {
  print_only_size(opt_inodes ? pdui->inodes : pdui->size);
...
mount_point_in_fts_cycle(FTSENT const *ent) {
  ...
  while (ent && ent != cycle_ent) {
    if (di_set_lookup(di_mnt, ent->fts_statp->st_dev, ent->fts_statp->st_ino) >
        0) {
      return 1;
    }
    ent = ent->fts_parent;
...
process_file(FTS *fts, FTSENT *ent) {
  ...
  if (info == 4) {
    ...
    error(0, ent->fts_errno,
          dcgettext(((void *)0), "cannot read directory %s", 5),
    ...
    if (!excluded) {
      ...
      if (info == 11) {
        ...
        ((void)sizeof((e == ent) ? 1 : 0), __extension__({
           if (e == ent)
             ;
           else
             __assert_fail("e == ent", "src/du.c", 527,
      ...
      if (info == 10 || info == 13) {
        error(0, ent->fts_errno, dcgettext(((void *)0), "cannot access %s", 5),
              quotearg_style(shell_escape_always_quoting_style, file));
        return 0;
      }

      if (fts->fts_options & 0x0040 && 0 < ent->fts_level &&
    ...
    if (excluded ||
        (!opt_count_all &&
         (hash_all ||
          (!((((sb->st_mode)) & 0170000) == (0040000)) && 1 < sb->st_nlink)) &&
      ...
      if (info == 1) {
        ...
        ((void)sizeof((e == ent) ? 1 : 0), __extension__({
           if (e == ent)
             ...
             __assert_fail("e == ent", "src/du.c", 560,
                           __extension__ __PRETTY_FUNCTION__);
         }));
      }
  ...
  duinfo_set(&dui,
             (apparent_size ? (((0) > (sb->st_size)) ? (0) : (sb->st_size))
  ...
  if (n_alloc == 0) {
    ...
    if (level == prev_level) {
      ...
      ((void)sizeof((level == prev_level - 1) ? 1 : 0), __extension__({
         if (level == prev_level - 1)
           ;
         else
           __assert_fail("level == prev_level - 1", "src/du.c", 638,
                         __extension__ __PRETTY_FUNCTION__);
       }));
...
du_files(char **files, int bit_flags) {
  ...
  if (*files) {
      ...
      if (ent == ((void *)0)) {
        if ((*__errno_location()) != 0) {
          error(0, (*__errno_location()),
                dcgettext(((void *)0), "fts_read failed: %s", 5),
    ...
    if (rpl_fts_close(fts) != 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "fts_close failed", 5));
      ok = 0;
    }
  }
...
int main(int argc, char **argv) {
  ...
  human_options(getenv("DU_BLOCK_SIZE"), &human_output_opts,
                &output_block_size);

  while (1) {
    int oi = -1;
    int c =
        getopt_long(argc, argv, "0abd:chHklmst:xB:DLPSX:", long_options, &oi);
    if (c == -1)
      break;
    ...
    case 'd': {
      ...
      if (xstrtoumax(optarg, ((void *)0), 0, &tmp, "") == LONGINT_OK &&
        ...
        error(0, 0, dcgettext(((void *)0), "invalid maximum depth %s", 5),
              quote(optarg));
        ok = 0;
      }
    } break;
      ...
      if (opt_threshold == 0 && *optarg == '-') {
        ...
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "--threshold argument '-0'\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "invalid --threshold argument '-0'", 5)),
    ...
    case 'X':
      if (add_exclude_file(add_exclude, exclude, optarg, (1 << 28), '\n')) {
        error(0, (*__errno_location()), "%s",
              quotearg_n_style_colon(0, shell_escape_quoting_style, optarg));
        ok = 0;
      }
      break;
    ...
    case TIME_OPTION:
      opt_time = 1;
      time_type =
          (optarg ? ((time_types)[__xargmatch_internal(
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "du", "GNU coreutils", Version, ("Torbjorn Granlund"),
  ...
  if (opt_all && opt_summarize_only) {
    error(0, 0,
          dcgettext(((void *)0), "cannot both summarize and show all entries",
                    5));
    usage(1);
  }
  ...
  if (opt_summarize_only && max_depth_specified && max_depth != 0) {
    ...
    error(0, 0,
          dcgettext(((void *)0),
                    "warning: summarizing conflicts with --max-depth=%lu", 5),
          d);
    usage(1);
  }
  ...
  if (opt_time) {
    ...
    if (*time_style == '+')
      ...
      switch (((time_style_types)[__xargmatch_internal(
          "time style", time_style, time_style_args,
          (void const *)(time_style_types), sizeof *(time_style_types),
          argmatch_die, 1)])) {
  ...
  if (files_from) {
    ...
    if (!((strcmp(files_from, "-") == 0) ||
          freopen_safer(files_from, "r", stdin)))
      ((!!sizeof(struct {
        _Static_assert(
            1,
            "verify_expr ("
  ...
  while (1) {
    ...
    if (!file_name) {
      ...
      case AI_ERR_READ:
        error(
            0, (*__errno_location()),
            dcgettext(((void *)0), "%s: read error", 5),
    ...
    if (!file_name[0]) {
      ...
      if (files_from == ((void *)0))
        error(0, 0, "%s",
              dcgettext(((void *)0), "invalid zero-length file name", 5));
      else {
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"error reading %s\", 5), "
      ...
      int _gl_dummy;
         ...
         ? ((error(
                 1, 0, dcgettext(((void *)0), "error reading %s", 5),
                 quotearg_style(shell_escape_always_quoting_style, files_from)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(
                 1, 0, dcgettext(((void *)0), "error reading %s", 5),
