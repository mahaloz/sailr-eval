  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "unique to FILE2,\nand column three contains lines common to "
                  "both files.\n",
                  5),
        stdout)
    ...
    if (!issued_disorder_warning[whatfile - 1]) {
      ...
      if (0 < order) {
        error((check_input_order == CHECK_ORDER_ENABLED ? 1 : 0), 0,
              dcgettext(((void *)0), "file %d is not in sorted order", 5),
              whatfile);
...
static _Noreturn void compare_files(char **infiles) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    alt[i][2] = 0;
    streams[i] =
        ((strcmp(infiles[i], "-") == 0) ? stdin : fopen_safer(infiles[i], "r"));
    if (!streams[i])
      ((!!sizeof(struct {
        _Static_assert(
            1,
            ...
            "(0, shell_escape_quoting_style, infiles[i])), assume (false))"
            ")");
        int _gl_dummy;
      }))
  ...
  while (thisline[0] || thisline[1]) {
    ...
    if (!thisline[0])
      ...
      if (hard_LC_COLLATE)
        ...
        size_t len = ((thisline[0]->length) < (thisline[1]->length)
                          ? (thisline[0]->length)
                          : (thisline[1]->length)) -
                     1;
  ...
  if (total_option) {
    ...
    printf("%s%s%s%s%s%s%s%c", umaxtostr(total[0], buf1), col_sep,
           ...
           dcgettext(((void *)0), "total", 5), delim);
  }
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "123z", long_options, ((void *)0))) != -1)
      ...
      if (col_sep_len && !(strcmp(col_sep, optarg) == 0))
        ((!!sizeof(struct {
          _Static_assert(1,
                         ...
                         "output delimiters specified\", 5)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "multiple output delimiters specified", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0),
                                 "multiple output delimiters specified", 5)),
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "comm", "GNU coreutils", Version,
