...
_Static_assert((1024 * 3 * 10) % 12 == 0, "verify ("
                                          "ENC_BLOCKSIZE % 12 == 0"
                                          ")");
_Static_assert((1024 * 3) % 12 == 0, "verify ("
                                     "DEC_BLOCKSIZE % 12 == 0"
                                     ")");
...
static _Noreturn void do_encode(FILE *in, char const *infile, FILE *out,
  ...
  do {
    ...
    if (sum > 0) {
      ...
      wrap_write(outbuf, ((((sum) + 2) / 3) * 4), wrap_column, &current_column,
                 out);
    }
  } while (!feof_unlocked(in) && !ferror_unlocked(in) &&
           sum == (1024 * 3 * 10));
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "diw:", long_options, ((void *)0))) !=
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "base64", "GNU coreutils", Version,
