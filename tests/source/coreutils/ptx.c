...
extern void re_set_registers(struct re_pattern_buffer *__buffer,
                             ...
                             regoff_t *__ends);
extern int regcomp(regex_t *__restrict __preg, const char *__restrict __pattern,
                   int __cflags);
...
enum Format { UNKNOWN_FORMAT, DUMB_FORMAT, ROFF_FORMAT, TEX_FORMAT };
...
static char const *break_file = ((void *)0);
static char const *only_file = ((void *)0);
static char const *ignore_file = ((void *)0);
  ...
  ((!!sizeof(struct {
    _Static_assert(
        1, "verify_expr ("
           "1"
           ", "
           "(error (1, (*__errno_location ()), dcgettext (((void *)0), \"error "
           "in regular expression matcher\", 5)), assume (false))"
           ")");
    int _gl_dummy;
  }))
       ? ((error(1, (*__errno_location()),
                 dcgettext(((void *)0), "error in regular expression matcher",
                           5)),
  ...
  while (*string) {
    if (*string == '\\') {
      ...
      case 'x':
        value = 0;
        for (length = 0, string++;
             length < 3 && ((*__ctype_b_loc())[(int)((to_uchar(*string)))] &
                            (unsigned short int)_ISxdigit);
             length++, string++)
          value = value * 16 +
                  ((*string) >= 'a' && (*string) <= 'f'   ? (*string) - 'a' + 10
                   : (*string) >= 'A' && (*string) <= 'F' ? (*string) - 'A' + 10
                                                          : (*string) - '0');
        if (length == 0) {
          *cursor++ = '\\';
  ...
  if (message)
    ((!!sizeof(struct {
      _Static_assert(1,
                     ...
                     "1"
                     ", "
                     "(error (1, 0, dcgettext (((void *)0), \"%s (for regexp "
                     "%s)\", 5), message, quote (string)), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0, dcgettext(((void *)0), "%s (for regexp %s)", 5),
...
static void initialize_regex(void) {
  ...
  if (ignore_case)
    for (character = 0; character < 256; character++)
      folded_chars[character] = (__extension__({
        int __res;
        if (sizeof(character) > 1) {
          ...
          __res = (*__ctype_toupper_loc())[(int)(character)];
        __res;
      }));
  ...
  if (word_regex.string)
    ...
    if (gnu_extensions) {
      ...
      for (character = 0; character < 256; character++)
        word_fastmap[character] = !!((*__ctype_b_loc())[(int)((character))] &
...
static void swallow_file_in_memory(char const *file_name, BLOCK *block) {
  ...
  _Bool using_stdin =
      !file_name || !*file_name || (strcmp(file_name, "-") == 0);
  if (using_stdin)
  ...
  if (!block->start)
    ((!!sizeof(struct {
      _Static_assert(1,
                     ...
                     "using_stdin ? \"-\" : file_name)), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(1, (*__errno_location()), "%s",
                   quotearg_n_style_colon(0, shell_escape_quoting_style,
                                          using_stdin ? "-" : file_name)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, (*__errno_location()), "%s",
                   quotearg_n_style_colon(0, shell_escape_quoting_style,
                                          using_stdin ? "-" : file_name)),
...
static int compare_words(const void *void_first, const void *void_second) {

  ptrdiff_t length;
  ptrdiff_t counter;
  int value;

  length = ((const WORD *)void_first)->size < ((const WORD *)void_second)->size
               ? ((const WORD *)void_first)->size
               : ((const WORD *)void_second)->size;

  if (ignore_case) {
    for (counter = 0; counter < length; counter++) {
      value =
          (folded_chars[to_uchar(((const WORD *)void_first)->start[counter])] -
...
static int compare_occurs(const void *void_first, const void *void_second) {

  int value;

  value = compare_words(&((const OCCURS *)void_first)->key,
                        &((const OCCURS *)void_second)->key);
  return (value ? value
          : ((const OCCURS *)void_first)->key.start <
                  ((const OCCURS *)void_second)->key.start
              ? -1
              : ((const OCCURS *)void_first)->key.start >
                    ((const OCCURS *)void_second)->key.start);
}
  ...
  while (cursor < file_contents.end) {
    ...
    if (cursor > word_start) {
      if (table->length == table->alloc)
        table->start =
            x2nrealloc(table->start, &table->alloc, sizeof *table->start);
...
static void find_occurs_in_text(int file_index) {
  ...
  if (input_reference) {
    while (line_scan < text_buffer->end &&
           !((*__ctype_b_loc())[(int)((to_uchar(*line_scan)))] &
  ...
  for (cursor = text_buffer->start; cursor < text_buffer->end;
      ...
      case 0:
        ((!!sizeof(struct {
          _Static_assert(1,
                         ...
                         "1"
                         ", "
                         "(error (1, 0, dcgettext (((void *)0), \"error: "
                         "regular expression has a match of length zero: %s\", "
                         "5), quote (context_regex.string)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "error: regular expression has a match of "
                                 "length zero: %s",
                                 5),
    ...
    while (context_end > context_start &&
           ((*__ctype_b_loc())[(int)((to_uchar(context_end[-1])))] &
            (unsigned short int)_ISspace))
      context_end--;
    ...
    while (1) {
      ...
      {
        regoff_t r =
            re_search(&word_regex.pattern, cursor, context_end - cursor, 0,
                      context_end - cursor, &word_regs);
        if (r == -2)
          matcher_error();
        if (r == -1)
          break;
      ...
      if (number_of_occurs[0] == occurs_alloc[0])
        occurs_table[0] = x2nrealloc(occurs_table[0], &occurs_alloc[0],
      ...
      if (input_reference && line_start == context_start) {
        while (context_start < context_end &&
               !((*__ctype_b_loc())[(int)((to_uchar(*context_start)))] &
                 (unsigned short int)_ISspace))
          context_start++;
        while (context_start < context_end &&
               ((*__ctype_b_loc())[(int)((to_uchar(*context_start)))] &
  ...
  for (cursor = field.start; cursor < field.end; cursor++) {
    ...
    if (edited_flag[character]) {
      ...
      case '            case '_':
          ...
          putchar_unlocked('\\');
          putchar_unlocked(character); break;
...
static void fix_output_parameters(void) {
  ...
  for (character = 0; character < 256; character++)
    edited_flag[character] = !!((*__ctype_b_loc())[(int)((character))] &
                                (unsigned short int)_ISspace);
  edited_flag['\f'] = 1;
  ...
  case TEX_FORMAT:
      ...
      for (cursor = "$%&        edited_flag[to_uchar (*cursor)] = 1;
...
static void define_all_fields(OCCURS *occurs) {
  ...
  while (cursor < right_context_end &&
    ...
    if (word_regex.string) {
      regoff_t count;
      count = re_match(&word_regex.pattern, cursor, right_context_end - cursor,
                       0, ((void *)0));
      if (count == -2)
        matcher_error();
  ...
  while (keyafter.end > keyafter.start &&
         ((*__ctype_b_loc())[(int)((to_uchar(keyafter.end[-1])))] &
          (unsigned short int)_ISspace))
    keyafter.end--;
  if (-occurs->left > half_line_width + maximum_word_length) {
    ...
    if (word_regex.string) {
      regoff_t count;
      count = re_match(&word_regex.pattern, left_field_start,
                       keyafter.start - left_field_start, 0, ((void *)0));
      if (count == -2)
        matcher_error();
  ...
  while (before.end > before.start &&
         ((*__ctype_b_loc())[(int)((to_uchar(before.end[-1])))] &
          (unsigned short int)_ISspace))
    before.end--;
  ...
  while (before.start + before_max_width < before.end)
    if (word_regex.string) {
      regoff_t count;
      count = re_match(&word_regex.pattern, before.start,
                       before.end - before.start, 0, ((void *)0));
      if (count == -2)
        matcher_error();
  ...
  if (truncation_string) {
    ...
    while (cursor > buffer_start &&
           ((*__ctype_b_loc())[(int)((to_uchar(cursor[-1])))] &
            (unsigned short int)_ISspace))
      cursor--;
  ...
  while (before.start < buffer_end &&
         ((*__ctype_b_loc())[(int)((to_uchar(*before.start)))] &
          (unsigned short int)_ISspace))
    before.start++;
  ...
  if (tail_max_width > 0) {
    ...
    while (tail.start < buffer_end &&
           ((*__ctype_b_loc())[(int)((to_uchar(*tail.start)))] &
            (unsigned short int)_ISspace))
      tail.start++;
    ...
    while (cursor < right_context_end && cursor < tail.start + tail_max_width) {
      ...
      if (word_regex.string) {
        regoff_t count;
        count = re_match(&word_regex.pattern, cursor,
                         right_context_end - cursor, 0, ((void *)0));
        if (count == -2)
          matcher_error();
        cursor += count == -1 ? 1 : count;
      } else if (word_fastmap[to_uchar(*cursor)])
    ...
    while (tail.end > tail.start &&
           ((*__ctype_b_loc())[(int)((to_uchar(tail.end[-1])))] &
            (unsigned short int)_ISspace))
      tail.end--;
  } else {
  ...
  if (head_max_width > 0) {
    ...
    while (head.end > buffer_start &&
           ((*__ctype_b_loc())[(int)((to_uchar(head.end[-1])))] &
            (unsigned short int)_ISspace))
      head.end--;
    ...
    while (head.start + head_max_width < head.end)
      if (word_regex.string) {
        regoff_t count;
        count = re_match(&word_regex.pattern, head.start, head.end - head.start,
                         0, ((void *)0));
        if (count == -2)
          matcher_error();
    ...
    while (head.start < head.end &&
           ((*__ctype_b_loc())[(int)((to_uchar(*head.start)))] &
            (unsigned short int)_ISspace))
      head.start++;
  } else {
  ...
  if (auto_reference) {
    ...
    while (reference.end < right_context_end &&
           !((*__ctype_b_loc())[(int)((to_uchar(*reference.end)))] &
             (unsigned short int)_ISspace))
      reference.end++;
  }
}
...
static void output_one_dumb_line(void) {
  if (!right_reference) {
    if (auto_reference) {
      ...
      print_field(reference);
      putchar_unlocked(':');
      print_spaces(reference_max_width + gap_size -
  ...
  if (tail.start < tail.end) {
    ...
    print_spaces(half_line_width - gap_size - (before.end - before.start) -
  ...
  if (head.start < head.end) {
    ...
    print_spaces(half_line_width - (keyafter.end - keyafter.start) -
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             "Output a permuted index, including context, of "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "'xx'\n  -O, --format=roff              generate output as roff "
            "directives\n  -R, --right-side-refs          put references at "
            "right, not counted in -w\n  -S, --sentence-regexp=REGEXP   for "
            "end of lines or end of sentences\n  -T, --format=tex              "
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "  -W, --word-regexp=REGEXP       use REGEXP to match each "
            "keyword\n  -b, --break-file=FILE          word break characters "
            "in this FILE\n  -f, --ignore-case              fold lower case to "
            "upper case for sorting\n  -g, --gap-size=NUMBER          gap size "
            "in columns between output fields\n  -i, --ignore-file=FILE        "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -r, --references               first field of each line "
                  "is a reference\n  -t, --typeset-mode               - not "
                  "implemented -\n  -w, --width=NUMBER             output "
                  "width in columns, reference excluded\n",
                  5),
        stdout)
...
static char const *const format_args[] = {"roff", "tex", ((void *)0)

};

static enum Format const format_vals[] = {ROFF_FORMAT, TEX_FORMAT};
  ...
  while (optchar = getopt_long(argc, argv, "AF:GM:ORS:TW:b:i:fg:o:trw:",
      ...
      if (!(xstrtoimax(optarg, ((void *)0), 0, &tmp, "") == LONGINT_OK &&
        ...
        ((!!sizeof(struct {
          _Static_assert(1,
                         ...
                         "1"
                         ", "
                         "(error (1, 0, dcgettext (((void *)0), \"invalid gap "
                         "width: %s\", 5), quote (optarg)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0, dcgettext(((void *)0), "invalid gap width: %s", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0, dcgettext(((void *)0), "invalid gap width: %s", 5),
                       quote(optarg)),
      ...
      if (!(xstrtoimax(optarg, ((void *)0), 0, &tmp, "") == LONGINT_OK &&
        ...
        ((!!sizeof(struct {
          _Static_assert(1,
                         ...
                         "1"
                         ", "
                         "(error (1, 0, dcgettext (((void *)0), \"invalid line "
                         "width: %s\", 5), quote (optarg)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "invalid line width: %s", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "invalid line width: %s", 5),
    ...
    case 10:
      output_format = ((format_vals)[__xargmatch_internal(
          "--format", optarg, format_args, (void const *)(format_vals),
          sizeof *(format_vals), argmatch_die, 1)]);
      break;
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "ptx", "GNU coreutils", Version,
                  proper_name_utf8("F. Pinard", "Fran\xc3\xa7ois Pinard"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
