...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0), "Run COMMAND, ignoring hangup signals.\n\n", 5),
        stdout)
...
int main(int argc, char **argv) {
  ...
  parse_gnu_standard_options_only(argc, argv, "nohup", "GNU coreutils", Version,
                                  0, usage, ("Jim Meyering"),
                                  (char const *)((void *)0));
  ...
  if (ignoring_input) {
    if (fd_reopen(0, "/dev/null", 01, 0) < 0)
      error(exit_internal_failure, (*__errno_location()),
            ...
            dcgettext(((void *)0), "failed to render standard input unusable",
                      5));
    if (!redirecting_stdout && !redirecting_stderr)
      error(0, 0, dcgettext(((void *)0), "ignoring input", 5));
  }
  ...
  if (redirecting_stdout || (redirecting_stderr && stdout_is_closed)) {
    ...
    out_fd = (redirecting_stdout ? fd_reopen(1, file, flags, mode)
    ...
    if (out_fd < 0) {
      ...
      if (home) {
        ...
        out_fd = (redirecting_stdout ? fd_reopen(1, in_home, flags, mode)
                                     : open(in_home, flags, mode));
      }
      if (out_fd < 0) {
        int saved_errno2 = (*__errno_location());
        error(0, saved_errno, dcgettext(((void *)0), "failed to open %s", 5),
              quotearg_style(shell_escape_always_quoting_style, file));
        if (in_home)
          error(0, saved_errno2, dcgettext(((void *)0), "failed to open %s", 5),
    ...
    error(0, 0,
          ...
          dcgettext(((void *)0),
                    ignoring_input ? "ignoring input and appending output to %s"
  ...
  if (redirecting_stderr) {
    ...
    if (dup2(out_fd, 2) < 0)
      error(exit_internal_failure, (*__errno_location()),
            ...
            dcgettext(((void *)0), "failed to redirect standard error", 5));
  ...
  if (dup2(saved_stderr_fd, 2) == 2)
    error(0, saved_errno, dcgettext(((void *)0), "failed to run command %s", 5),
