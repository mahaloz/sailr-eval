...
pclmul_supported(void) {
  ...
  if (!(ecx & (1 << 1)) || !(ecx & (1 << 28))) {
    if (cksum_debug)
      error(0, 0, "%s",
            dcgettext(((void *)0), "pclmul support not detected", 5));
    return 0;
  }

  if (cksum_debug)
    error(0, 0, "%s",
          dcgettext(((void *)0), "using pclmul hardware support", 5));
...
void output_crc(char const *file, int binary_file, void const *digest,
