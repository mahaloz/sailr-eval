...
enum { STRIP_TRAILING_SLASHES_OPTION = 0x7f + 1 };
...
static void cp_option_init(struct cp_options *x) {
  ...
  _Bool selinux_enabled = (0 < is_selinux_enabled());
...
do_move(char const *source, char const *dest, int dest_dirfd,
  ...
  _Bool ok = copy(source, dest, dest_dirfd, dest_relname, 0, x, &copy_into_self,
                  &rename_succeeded);
      ...
      ((void)sizeof((((status) == RM_OK || (status) == RM_USER_DECLINED ||
       ...
       __extension__({
         if (((status) == RM_OK || (status) == RM_USER_DECLINED ||
           ...
           __assert_fail("VALID_STATUS (status)", "src/mv.c", 225,
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "into DIRECTORY\n  -T, --no-target-directory    treat DEST as a "
...
int main(int argc, char **argv) {
  ...
  _Bool remove_trailing_slashes = 0;
  ...
  while ((c = getopt_long(argc, argv, "bfint:uvS:TZ", long_options,
    ...
    case 'Z':
      ...
      if (selinux_enabled) {
        ...
        x.set_security_context = selabel_open(0, ((void *)0), 0);
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "mv", "GNU coreutils", Version, ("Mike Parker"),
  ...
  if (no_target_directory) {
    ...
    if (n_files == 2)
      x.rename_errno = (renameatu(-100, file[0], -100, lastfile,
  ...
  if (target_directory) {
    ...
    for (int i = 0; i < n_files; ++i) {
      ...
      char *dest_relname;
      char *dest =
          file_name_concat(target_directory, source_basename, &dest_relname);
    ...
    x.last_file = 1;
    ok = do_move(file[0], file[1], -100, file[1], &x);
  }
