...
_Bool parse_datetime(struct timespec *__restrict__, char const *,
                     struct timespec const *);

_Bool parse_datetime2(struct timespec *__restrict__, char const *,
                      struct timespec const *, unsigned int flags, timezone_t,
                      char const *);
...
static _Bool show_date(char const *, struct timespec, timezone_t);
...
static enum Time_spec const time_spec[] = {TIME_SPEC_HOURS, TIME_SPEC_MINUTES,
                                           TIME_SPEC_DATE, TIME_SPEC_SECONDS,
                                           TIME_SPEC_NS};
_Static_assert(
    (sizeof(time_spec_string) / sizeof *(time_spec_string)) ==
        (sizeof(time_spec) / sizeof *(time_spec)) + 1,
    "verify ("
    "ARRAY_CARDINALITY (time_spec_string) == ARRAY_CARDINALITY (time_spec) + 1"
    ")");

static char const rfc_email_format[] = "%a, %d %b %Y %H:%M:%S %z";

enum {
  DEBUG_DATE_PARSING_OPTION = 0x7f + 1,
  RESOLUTION_OPTION,
  RFC_3339_OPTION
};
...
static struct option const long_options[] = {
    {"date", 1, ((void *)0), 'd'},
    {"debug", 0, ((void *)0), DEBUG_DATE_PARSING_OPTION},
    {"file", 1, ((void *)0), 'f'},
    {"iso-8601", 2, ((void *)0), 'I'},
    {"reference", 1, ((void *)0), 'r'},
    {"resolution", 0, ((void *)0), RESOLUTION_OPTION},
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             ...
                             "-s, or with [MMDDhhmm, set the date and time.\n",
                             5),
                   stdout)
    ...
    fputs_unlocked(
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "  -f, --file=DATEFILE        like --date; once "
                             "for each line of DATEFILE\n",
                             5),
                   stdout)
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "  --resolution               output the available resolution of "
            "timestamps\n                               Example: 0.000000001\n",
            5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -R, --rfc-email            output date and time in RFC "
                  "5322 format.\n                               Example: Mon, "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "      --rfc-3339=FMT         output date/time in RFC 3339 "
                  "format.\n                               FMT='date', "
                  "'seconds', or 'ns'\n                               for date "
...
static int res_width(long int res) {
  int i = 9;
...
batch_convert(char const *input_filename, char const *format, timezone_t tz,
              char const *tzstring) {
    ...
    if (in_stream == ((void *)0)) {
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "(0, shell_escape_quoting_style, input_filename)), assume (false))"
            ")");
        int _gl_dummy;
      }))
  ...
  while (1) {
    ...
    if (!parse_datetime2(&when, line, ((void *)0), parse_datetime_flags, tz,
                         tzstring)) {
      if (line[line_length - 1] == '\n')
        line[line_length - 1] = '\0';
      error(0, 0, dcgettext(((void *)0), "invalid date %s", 5), quote(line));
      ok = 0;
    } else {
...
int main(int argc, char **argv) {
  int optc;
  char const *datestr = ((void *)0);
  char const *set_datestr = ((void *)0);
  struct timespec when;

  _Bool set_date = 0;
  char const *format = ((void *)0);

  _Bool get_resolution = 0;
  ...
  while ((optc = getopt_long(argc, argv, short_options, long_options,
    ...
    case RFC_3339_OPTION: {
      static char const rfc_3339_format[][32] = {
          "%Y-%m-%d", "%Y-%m-%d %H:%M:%S%:z", "%Y-%m-%d %H:%M:%S.%N%:z"};
      enum Time_spec i =
          ((time_spec +
            2)[__xargmatch_internal("--rfc-3339", optarg, time_spec_string + 2,
                                    (void const *)(time_spec + 2),
                                    sizeof *(time_spec + 2), argmatch_die, 1)]);
      new_format = rfc_3339_format[i];
      break;
    }
    case 'I': {
      static char const iso_8601_format[][32] = {
          "%Y-%m-%d", "%Y-%m-%dT%H:%M:%S%:z", "%Y-%m-%dT%H:%M:%S,%N%:z",
          "%Y-%m-%dT%H%:z", "%Y-%m-%dT%H:%M%:z"};
      enum Time_spec i = (optarg ? ((time_spec)[__xargmatch_internal(
                                       "--iso-8601", optarg, time_spec_string,
                                       (void const *)(time_spec),
                                       sizeof *(time_spec), argmatch_die, 1)])
                                 : TIME_SPEC_DATE);
      ...
      if (format)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "output formats specified\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "multiple output formats specified", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0),
                                 "multiple output formats specified", 5)),
  ...
  int option_specified_date =
      (!!datestr + !!batch_file + !!reference + get_resolution);
  ...
  if (batch_file != ((void *)0))
    ...
    if (!option_specified_date && !set_date) {
      if (optind < argc) {
        ...
        set_date = 1;
        datestr = argv[optind];
        valid_date = posixtime(&when.tv_sec, datestr, (1 | 2 | 4));
        when.tv_nsec = 0;
      } else {
    ...
    if (!valid_date)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          ...
                          "date %s\", 5), quote (datestr)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0, dcgettext(((void *)0), "invalid date %s", 5),
                     quote(datestr)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0, dcgettext(((void *)0), "invalid date %s", 5),
                     quote(datestr)),
    ...
    if (set_date) {
      ...
      if (settime(&when) != 0) {
        error(0, (*__errno_location()),
              dcgettext(((void *)0), "cannot set date", 5));
        ok = 0;
      }
    }
...
show_date(char const *format, struct timespec when, timezone_t tz) {
  ...
  if (parse_datetime_flags & 1)
    error(0, 0, dcgettext(((void *)0), "output format: %s", 5), quote(format));
