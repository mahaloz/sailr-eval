...
static gid_t parse_group(char const *name) {
  gid_t gid = -1;
      ...
      if (!(xstrtoumax(name, ((void *)0), 10, &tmp, "") == LONGINT_OK &&
        ...
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "group: %s\", 5), quote (name)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        ...
        stdout);
    printf(dcgettext(((void *)0),
                     "\nExamples:\n  %s staff /u      Change the group of /u "
                     "to \"staff\".\n  %s -hR staff /u  Change the group of /u "
                     "and subfiles to \"staff\".\n",
                     5)
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "HLPRcfhv", long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "chgrp", "GNU coreutils", Version,
  ...
  bit_flags |= 0x0400;
  ok = chown_files(argv + optind, bit_flags, (uid_t)-1, gid, (uid_t)-1,
                   (gid_t)-1, &chopt);
