...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... [FILE]...\n", 5)
           ...
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0), "Concatenate FILE(s) to standard output.\n", 5),
        stdout)
    ...
    fputs_unlocked(
        ...
        stdout);
    printf(dcgettext(((void *)0),
                     "\nExamples:\n  %s f - g  Output f's contents, then "
                     "standard input, then g's contents.\n  %s        Copy "
...
cat(char *inbuf, idx_t insize, char *outbuf, idx_t outsize,
    ...
    _Bool show_nonprinting, _Bool show_tabs, _Bool number,
  ...
  while (1) {
    do {
      ...
      if (bpin > eob) {
        ...
        if (use_fionread && ioctl(input_desc, 0x541B, &n_to_read) < 0) {
          ...
          if ((*__errno_location()) == 95 || (*__errno_location()) == 25
            ...
            error(0, (*__errno_location()),
                  dcgettext(((void *)0), "cannot do ioctl on %s", 5),
...
static int copy_cat(void) {
  ...
  for (_Bool some_copied = 0;; some_copied = 1)
    switch (
        copy_file_range(input_desc, ((void *)0), 1, ((void *)0), copy_max, 0)) {
    case 0:
      return some_copied;
    ...
    case -1:
      if ((*__errno_location()) == 38 || is_ENOTSUP((*__errno_location())) ||
          ...
          || (*__errno_location()) == 9 || (*__errno_location()) == 18 ||
          (*__errno_location()) == 26
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "benstuvAET", long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "cat", "GNU coreutils", Version,
  ...
  _Bool out_isreg = ((((stat_buf.st_mode)) & 0170000) == (0100000)) != 0;
  ...
  do {
    ...
    if (out_isreg && stat_buf.st_dev == out_dev && stat_buf.st_ino == out_ino &&
        lseek(input_desc, 0, 1) < stat_buf.st_size) {
      error(0, 0, dcgettext(((void *)0), "%s: input file is output file", 5),
            quotearg_n_style_colon(0, shell_escape_quoting_style, infile));
      ok = 0;
      goto contin;
    }
    ...
    if (!(number || show_ends || show_nonprinting || show_tabs ||
          squeeze_blank)) {
      int copy_cat_status =
      ...
      ok &= cat(inbuf, insize, outbuf, outsize, show_nonprinting, show_tabs,
                number, number_nonblank, show_ends, squeeze_blank);
