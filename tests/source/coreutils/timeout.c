...
enum { FOREGROUND_OPTION = 0x7f + 1, PRESERVE_STATUS_OPTION };
    ...
    if (timer_settime(timerid, 0, &its, ((void *)0)) == 0)
      ...
      if (warn)
        error(0, (*__errno_location()),
              dcgettext(((void *)0), "warning: timer_settime", 5));
      timer_delete(timerid);
    }
  } else if (warn && (*__errno_location()) != 38)
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "warning: timer_create", 5));
...
static void cleanup(int sig) {
  ...
  if (monitored_pid) {
    ...
    if (verbose) {
      ...
      error(0, 0, dcgettext(((void *)0), "sending signal %s to command %s", 5),
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "      --preserve-status\n                 exit with the same "
            "status as COMMAND, even when the\n                   command "
            "times out\n      --foreground\n                 when not running "
        ...
        stdout);
    fputs_unlocked(dcgettext(((void *)0),
                             "  -v, --verbose  diagnose to stderr any signal "
                             "sent upon timeout\n",
                             5),
                   stdout);
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "\nUpon timeout, send the TERM signal to COMMAND, if no other "
            "SIGNAL specified.\nThe TERM signal kills any process that does "
            "not block or catch that signal.\nIt may be necessary to use the "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "--preserve-status is not specified\n  125  if the timeout "
                  "command itself fails\n  126  if COMMAND is found but cannot "
                  "be invoked\n  127  if COMMAND cannot be found\n  137  if "
                  "COMMAND (or timeout itself) is sent the KILL (9) signal "
...
static double parse_duration(char const *str) {
  ...
  if (!(xstrtod(str, &ep, &duration, cl_strtod) || (*__errno_location()) == 34)
    ...
    error(0, 0, dcgettext(((void *)0), "invalid time interval %s", 5),
          quote(str));
    usage(EXIT_CANCELED);
  }
...
static void unblock_signal(int sig) {
  ...
  if (sigprocmask(1, &unblock_set, ((void *)0)) != 0)
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "warning: sigprocmask", 5));
}
...
disable_core_dumps(void) {
  ...
  if (prctl(4, 0) == 0)
    return 1;
  error(0, (*__errno_location()),
        dcgettext(((void *)0), "warning: disabling core dumps failed", 5));
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "+k:s:v", long_options, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'k':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "timeout", "GNU coreutils", Version,
                  ("Padraig Brady"), (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (monitored_pid == -1) {
    ...
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "failed to run command %s", 5),
          quote(command));
    return exit_status;
  } else {
    pid_t wait_result;
    ...
    if (wait_result < 0) {
      ...
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "error waiting for command", 5));
      ...
      if ((((status)&0x7f) == 0))
        ...
        if (((status)&0x80))
          error(0, 0,
                dcgettext(((void *)0), "the monitored command dumped core", 5));
        ...
        error(0, 0,
              dcgettext(((void *)0), "unknown status from command (%d)", 5),
              status);
        status = 1;
      }
    }
