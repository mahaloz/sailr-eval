...
enum { PRESUME_INPUT_PIPE_OPTION = 0x7f + 1 };
...
void usage(int status) {
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0),
                  "  -c, --bytes=[-]NUM       print the first NUM bytes of "
                  "each file;\n                             with the leading "
                  "'-', print all but the last\n                             "
                  "NUM bytes of each file\n  -n, --lines=[-]NUM       print "
                  "the first NUM lines instead of the first %d;\n              "
...
static void diagnose_copy_fd_failure(enum Copy_fd_status err,
                                     char const *filename) {
  switch (err) {
  ...
  case COPY_FD_UNEXPECTED_EOF:
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: file has shrunk too much", 5),
...
static void write_header(char const *filename) {
...
static enum Copy_fd_status copy_fd(int src_fd, uintmax_t n_bytes) {
  char buf[8192];
...
static off_t elseek(int fd, off_t offset, int whence, char const *filename) {
  ...
  if (new_offset < 0)
    error(0, (*__errno_location()),
          ...
          dcgettext(((void *)0),
                    whence == 0 ? "%s: cannot seek to offset %s"
  ...
  if ((18446744073709551615UL) < n_elide_0 + 8192) {
    ...
    ((!!sizeof(struct {
      _Static_assert(
          ...
          "1"
          ", "
          "(error (1, 0, dcgettext (((void *)0), \"%s: number of bytes is too "
          "large\", 5), umaxtostr (n_elide_0, umax_buf)), assume (false))"
          ")");
      int _gl_dummy;
    }))
         ? ((error(
                 1, 0,
                 dcgettext(((void *)0), "%s: number of bytes is too large", 5),
...
elide_tail_bytes_file(char const *filename, int fd, uintmax_t n_elide,
  ...
  if (presume_input_pipe || current_pos < 0 ||
      size <=
...
elide_tail_lines_seekable(char const *pretty_filename, int fd,
                          uintmax_t n_lines, off_t start_pos, off_t size) {
  char buffer[8192];
  size_t bytes_read;
  off_t pos = size;
...
static uintmax_t string_to_integer(_Bool count_lines, char const *n_string) {
  return xdectoumax(n_string, 0, (18446744073709551615UL), "bkKmMGTPEZY0",
                    count_lines
                        ? dcgettext(((void *)0), "invalid number of lines", 5)
...
int main(int argc, char **argv) {
  ...
  if (1 < argc && argv[1][0] == '-' &&
    ...
    for (; *a; a++) {
      ...
      default:
        error(0, 0, dcgettext(((void *)0), "invalid trailing option -- %c", 5),
              *a);
        usage(1);
      }
    }
  ...
  while ((c = getopt_long(argc, argv, "c:n:qvz0123456789", long_options,
                          ((void *)0))) != -1) {
    switch (c) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "head", "GNU coreutils", Version, ("David MacKenzie"),
  ...
  if (!count_lines && elide_from_end &&
      ((off_t)(!(!((off_t)0 < (off_t)-1))
                   ? (off_t)-1
                   : ((((off_t)1 << ((sizeof(off_t) * 8) - 2)) - 1) * 2 + 1))) <
          n_units) {
    char umax_buf[((((((sizeof(n_units) * 8) -
                       (!((__typeof__(n_units))0 < (__typeof__(n_units))-1))) *
                          146 +
                      484) /
                     485) +
                    (!((__typeof__(n_units))0 < (__typeof__(n_units))-1))) +
    ...
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        ...
                        "\"invalid number of bytes\", 5), quote (umaxtostr "
                        "(n_units, umax_buf))), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 75, "%s: %s",
