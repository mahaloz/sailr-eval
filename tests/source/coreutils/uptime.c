...
static void print_uptime(size_t n, const STRUCT_UTMP *this) {
    ...
    if (boot_time == 0)
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       "*)0), \"couldn't get boot time\", 5)), assume (false))"
                       ")");
        int _gl_dummy;
      }))
           ? ((error(1, (*__errno_location()),
                     dcgettext(((void *)0), "couldn't get boot time", 5)),
  ...
  if (tmn)
    ...
    fprintftime(stdout, dcgettext(((void *)0), " %H:%M:%S  ", 5), tmn, 0, 0);
  else
    printf(dcgettext(((void *)0), " ??:????  ", 5));
  if (uptime == (time_t)-1)
    printf(dcgettext(((void *)0), "up ???? days ??:??,  ", 5));
  else {
    if (0 < updays)
      printf(dcngettext(((void *)0), "up %ld day %2d:%02d,  ",
                        "up %ld days %2d:%02d,  ", select_plural(updays), 5)

                 ,
             updays, uphours, upmins);
    else
      printf(dcgettext(((void *)0), "up  %2d:%02d,  ", 5), uphours, upmins);
  }
  printf(dcngettext(((void *)0), "%lu user", "%lu users",
                    select_plural(entries), 5),
  ...
  if (loads == -1)
    ...
    if (loads > 0)
      printf(dcgettext(((void *)0), ",  load average: %.2f", 5), avg[0]);
    if (loads > 1)
      printf(", %.2f", avg[1]);
    if (loads > 2)
      printf(", %.2f", avg[2]);
...
static _Noreturn void uptime(char const *filename, int options) {
  size_t n_users;
...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(
        ...
        "up,\nthe number of users on the system, and the average number of "
    ...
    printf(dcgettext(((void *)0),
                     "  Processes in\nan uninterruptible sleep state also "
