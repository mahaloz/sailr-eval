...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION] [COMMAND [ARG]...]\n", 5),
           program_name);
    printf(
        dcgettext(((void *)0),
                  ...
                  "to the process) to %d (least favorable to the process).\n",
                  5)
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc;) {
    ...
    if (s[0] == '-' &&
      ...
      case GETOPT_VERSION_CHAR:
        version_etc(stdout, "nice", "GNU coreutils", Version,
  ...
  if (adjustment_given) {
    ...
    if (LONGINT_OVERFLOW < xstrtol(adjustment_given, ((void *)0), 10, &tmp, ""))
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "adjustment %s\", 5), quote (adjustment_given)), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(EXIT_CANCELED, 0,
                     dcgettext(((void *)0), "invalid adjustment %s", 5),
    ...
    adjustment =
        (((MIN_ADJUSTMENT) >
          ((((tmp) < (MAX_ADJUSTMENT)) ? (tmp) : (MAX_ADJUSTMENT))))
             ? (MIN_ADJUSTMENT)
             : ((((tmp) < (MAX_ADJUSTMENT)) ? (tmp) : (MAX_ADJUSTMENT))));
  }
  ...
  if (i == argc) {
    if (adjustment_given) {
      error(0, 0,
            dcgettext(((void *)0), "a command must be given with an adjustment",
                      5));
      usage(EXIT_CANCELED);
    }
    ...
    if (current_niceness == -1 && (*__errno_location()) != 0)
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "*)0), \"cannot get niceness\", 5)), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(EXIT_CANCELED, (*__errno_location()),
                     dcgettext(((void *)0), "cannot get niceness", 5)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(EXIT_CANCELED, (*__errno_location()),
                     dcgettext(((void *)0), "cannot get niceness", 5)),
               ((0) ? (void)0 : __builtin_unreachable()))));
  ...
  if (!ok) {
    error(perm_related_errno((*__errno_location())) ? 0 : EXIT_CANCELED,
          (*__errno_location()),
          dcgettext(((void *)0), "cannot set niceness", 5));
