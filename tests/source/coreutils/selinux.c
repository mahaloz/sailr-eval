...
extern int defaultcon(struct selabel_handle *selabel_handle, char const *path,
                      mode_t mode);
static int computecon(char const *path, mode_t mode, char **con) {
  char *scon = ((void *)0);
  char *tcon = ((void *)0);
  security_class_t tclass;
  int rc = -1;
...
int defaultcon(struct selabel_handle *selabel_handle, char const *path,
               mode_t mode) {
  int rc = -1;
  char *scon = ((void *)0);
  char *tcon = ((void *)0);
...
static int restorecon_private(struct selabel_handle *selabel_handle,
                              char const *path) {
  int rc = -1;
  struct stat sb;
  char *scon = ((void *)0);
                                  ...
                                  recurse)
{
  ...
  if (!recurse) {
    ...
    _Bool ok = restorecon_private(selabel_handle, path) != -1;
    int err = (*__errno_location());
    free(newpath);
  ...
  char const *ftspath[2] = {path, ((void *)0)};
  FTS *fts = xfts_open((char *const *)ftspath, 0x0010, ((void *)0));

  int err = 0;
  for (FTSENT *ent; (ent = rpl_fts_read(fts));)
