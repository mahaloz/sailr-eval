...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0),
                     "Base%d encode or decode FILE, or standard input, to "
                     "standard output.\n",
                     5)
    ...
    printf(
        dcgettext(((void *)0),
                  "\nThe data are encoded as described for the %s alphabet in "
                  "RFC 4648.\nWhen decoding, the input may contain newlines in "
                  "addition to the bytes of\nthe formal %s alphabet.  Use "
                  "--ignore-garbage to attempt to recover\nfrom any other "
...
_Static_assert((1024 * 3 * 10) % 40 == 0, "verify ("
                                          "ENC_BLOCKSIZE % 40 == 0"
                                          ")");
_Static_assert((1024 * 5) % 40 == 0, "verify ("
                                     "DEC_BLOCKSIZE % 40 == 0"
                                     ")");
...
static _Noreturn void do_encode(FILE *in, char const *infile, FILE *out,
  ...
  outbuf = xmalloc((((((1024 * 3 * 10)) + 4) / 5) * 8));
  ...
  do {
    ...
    if (sum > 0) {
      ...
      base32_encode(inbuf, sum, outbuf, ((((sum) + 4) / 5) * 8));

      wrap_write(outbuf, ((((sum) + 4) / 5) * 8), wrap_column, &current_column,
                 out);
    }
  } while (!feof_unlocked(in) && !ferror_unlocked(in) &&
           sum == (1024 * 3 * 10));
...
static _Noreturn void do_decode(FILE *in, char const *infile, FILE *out,
  ...
  inbuf = xmalloc((((((1024 * 5)) + 4) / 5) * 8));
  outbuf = xmalloc((1024 * 5));
  ...
  do {
    ...
    sum = 0;
    do {
      idx_t n = fread_unlocked(inbuf + sum, 1,
                               (((((1024 * 5)) + 4) / 5) * 8) - sum, in);

      if (ignore_garbage) {
        for (idx_t i = 0; n > 0 && i < n;) {
          if (isbase32(inbuf[sum + i]) || inbuf[sum + i] == '=')
            i++;
          else
        ...
        ((!!sizeof(struct {
          ...
          int _gl_dummy;
             ...
             : ((error(1, (*__errno_location()),
                       dcgettext(((void *)0), "read error", 5)),
                 ((0) ? (void)0 : __builtin_unreachable()))));
    } while (sum < (((((1024 * 5)) + 4) / 5) * 8) && !feof_unlocked(in));

    for (int k = 0; k < 1 + !!feof_unlocked(in); k++) {
      if (k == 1 && ctx.i == 0)
        break;
      idx_t n = (1024 * 5);
      ok = base32_decode_ctx(&ctx, inbuf, (k == 0 ? sum : 0), outbuf, &n);
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "diw:", long_options, ((void *)0))) !=
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "base32", "GNU coreutils", Version,
                  ("Simon Josefsson"), (char *)((void *)0));
      exit(0);
      break;
      ;
