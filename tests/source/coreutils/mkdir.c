...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... DIRECTORY...\n", 5),
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0),
...
static int process_dir(char *dir, struct savewd *wd, void *options) {
  ...
  if (o->set_security_context) {
    if (!o->make_ancestor_function &&
        defaultcon(o->set_security_context, dir, 0040000) < 0 &&
        !ignorable_ctx_err((*__errno_location())))
      error(0, (*__errno_location()),
            dcgettext(((void *)0),
                      "failed to set default creation context for %s", 5),
  ...
  int ret =
      (make_dir_parents(dir, wd, o->make_ancestor_function, options, o->mode,
                        announce_mkdir, o->mode_bits, (uid_t)-1, (gid_t)-1, 1)
           ? 0

           : 1);

  if (ret == 0 && o->set_security_context && o->make_ancestor_function) {
    if (!restorecon(o->set_security_context, last_component(dir), 0) &&
        !ignorable_ctx_err((*__errno_location())))
      error(0, (*__errno_location()),
...
int main(int argc, char **argv) {
  ...
  options.mode = ((0400 | 0200 | 0100) | ((0400 | 0200 | 0100) >> 3) |
                  (((0400 | 0200 | 0100) >> 3) >> 3));
  options.mode_bits = 0;
  ...
  while ((optc = getopt_long(argc, argv, "pm:vZ", longopts, ((void *)0))) !=
         -1) {
    switch (optc) {
    case 'p':
    ...
    case 'v':
      options.created_directory_format =
          dcgettext(((void *)0), "created directory %s", 5);
      break;
    case 'Z':
      if (is_smack_enabled()) {
        ...
        if (optarg)
          ...
          options.set_security_context = selabel_open(0,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "mkdir", "GNU coreutils", Version,
  ...
  if (options.make_ancestor_function || specified_mode) {
    ...
    options.umask_ancestor = umask_value & ~(0200 | 0100);
        ...
        ((!!sizeof(struct {
          ...
          int _gl_dummy;
             ...
             : ((error(1, 0, dcgettext(((void *)0), "invalid mode %s", 5),
                 ...
                 ((0) ? (void)0 : __builtin_unreachable()))));
      options.mode =
          mode_adjust(((0400 | 0200 | 0100) | ((0400 | 0200 | 0100) >> 3) |
                       (((0400 | 0200 | 0100) >> 3) >> 3)),
                      1, umask_value, change, &options.mode_bits);
  ...
  return savewd_process_files(argc - optind, argv + optind, process_dir,
                              &options);
}
