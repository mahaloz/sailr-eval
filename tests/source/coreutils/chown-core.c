...
static void describe_change(char const *file, enum Change_status changed,
  ...
  old_spec = user_group_str(user ? old_user : ((void *)0),
                            group ? old_group : ((void *)0));

  switch (changed) {
  case CH_SUCCEEDED:
    fmt =
        (user ? dcgettext(((void *)0),
                          "changed ownership of %s from %s to %s\n", 5)

         : group
             ? dcgettext(((void *)0), "changed group of %s from %s to %s\n", 5)

             : dcgettext(((void *)0), "no change to ownership of %s\n", 5));
    break;
  case CH_FAILED:
    if (old_spec) {
      fmt = (user ? dcgettext(
             ...
             : group
                 ...
                 : dcgettext(((void *)0), "failed to change ownership of %s\n",
                             5));
    } else {
      fmt = (user ? dcgettext(((void *)0),
                              "failed to change ownership of %s to %s\n", 5)

             : group ? dcgettext(((void *)0),
  ...
  case CH_NO_CHANGE_REQUESTED:
    fmt = (user ? dcgettext(((void *)0), "ownership of %s retained as %s\n", 5)

           : group ? dcgettext(((void *)0), "group of %s retained as %s\n", 5)

                   : dcgettext(((void *)0), "ownership of %s retained\n", 5));
    break;
  default:
    abort();
  }

  printf(fmt, quotearg_style(shell_escape_always_quoting_style, file), old_spec,
         spec);
...
change_file_owner(FTS *fts, FTSENT *ent, uid_t uid, gid_t gid,
      ...
      if ((chopt->root_dev_ino &&
        ...
        do {
          if ((strcmp(file_full_name, "/") == 0))
            error(0, 0,
                  dcgettext(((void *)0),
                            "it is dangerous to operate recursively on %s", 5),
  ...
  if (!ok) {
    ...
    if (chopt->affect_symlink_referent &&
        ((((file_stats->st_mode)) & 0170000) == (0120000))) {
      if (fstatat(fts->fts_cwd_fd, file, &stat_buf, 0) != 0) {
        if (!chopt->force_silent)
          error(0, (*__errno_location()),
                dcgettext(((void *)0), "cannot dereference %s", 5),
                quotearg_style(shell_escape_always_quoting_style,
                               file_full_name));
        ok = 0;
      }
    ...
    do_chown =
        (ok &&
         (required_uid == (uid_t)-1 || required_uid == file_stats->st_uid) &&
         (required_gid == (gid_t)-1 || required_gid == file_stats->st_gid));
  }

  if (ok &&
      ((ent)->fts_info == 1 || (ent)->fts_info == 2 || (ent)->fts_info == 6 ||
       (ent)->fts_info == 4) &&
      (chopt->root_dev_ino &&
    ...
    do {
      ...
      error(0, 0,
            dcgettext(((void *)0),
                      "use --no-preserve-root to override this failsafe", 5));
    } while (0);
    return 0;
  }
  ...
  if (do_chown) {
    ...
    if (do_chown && !ok && !chopt->force_silent)
      error(0, (*__errno_location()),
            (uid != (uid_t)-1
                 ? dcgettext(((void *)0), "changing ownership of %s", 5)

                 : dcgettext(((void *)0), "changing group of %s", 5)),
            quotearg_style(shell_escape_always_quoting_style, file_full_name));
  }

  if (chopt->verbosity != V_off) {

    _Bool changed = ((do_chown && ok && symlink_changed) &&
    ...
    if (changed || chopt->verbosity == V_high) {
      enum Change_status ch_status = (!ok                ? CH_FAILED
                                      ...
                                      : !changed ? CH_NO_CHANGE_REQUESTED
                                                 : CH_SUCCEEDED);
      char *old_usr =
          file_stats ? uid_to_name(file_stats->st_uid) : ((void *)0);
      char *old_grp =
          file_stats ? gid_to_name(file_stats->st_gid) : ((void *)0);
      char *new_usr = chopt->user_name ? chopt->user_name
                      : uid != -1      ? uid_to_str(uid)
                                       : ((void *)0);
      char *new_grp = chopt->group_name ? chopt->group_name
                      : gid != -1       ? gid_to_str(gid)
                                        : ((void *)0);
      describe_change(file_full_name, ch_status, old_usr, old_grp, new_usr,
                      new_grp);
      free(old_usr);
      free(old_grp);
...
chown_files(char **files, int bit_flags, uid_t uid, gid_t gid,
            uid_t required_uid, gid_t required_gid,
            struct Chown_option const *chopt) {
  ...
  int stat_flags =
      ((required_uid != (uid_t)-1 || required_gid != (gid_t)-1 ||
        chopt->affect_symlink_referent || chopt->verbosity != V_off)
           ? 0
           : 0x0008);
  ...
  while (1) {
    ...
    if (ent == ((void *)0)) {
      if ((*__errno_location()) != 0) {
        ...
        if (!chopt->force_silent)
          error(0, (*__errno_location()),
                dcgettext(((void *)0), "fts_read failed", 5));
        ok = 0;
      }
      break;
    }

    ok &= change_file_owner(fts, ent, uid, gid, required_uid, required_gid,
                            chopt);
  }
