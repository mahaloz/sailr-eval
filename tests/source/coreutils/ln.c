...
inline int priv_set_remove_linkdir(void) { return -1; }
...
static int atomic_link(char const *source, int destdir_fd,
...
do_link(char const *source, int destdir_fd, char const *dest_base,
  ...
  if (link_errno) {
    if (!symbolic_link && !hard_dir_link &&
        ((((source_stats.st_mode)) & 0170000) == (0040000))) {
      error(
          0, 0,
          dcgettext(((void *)0), "%s: hard link not allowed for directory", 5),
    ...
    _Bool force =
        (remove_existing_files || interactive || backup_type != no_backups);
    if (force) {
      struct stat dest_stats;
      if (fstatat(destdir_fd, dest_base, &dest_stats, 0x100) != 0) {
        ...
        error(0, 0, dcgettext(((void *)0), "%s: cannot overwrite directory", 5),
              quotearg_n_style_colon(0, shell_escape_quoting_style, dest));
        goto fail;
        ...
        if (backup_type != no_backups ? !symbolic_link
          ...
          if (source_status == 0 &&
              ...
              (source_stats.st_nlink == 1 ||
               same_nameat(-100, source, destdir_fd, dest_base))) {
            error(
                0, 0, dcgettext(((void *)0), "%s and %s are the same file", 5),
                quotearg_n_style(0, shell_escape_always_quoting_style, source),
                quotearg_n_style(1, shell_escape_always_quoting_style, dest));
            goto fail;
          }
        }
        ...
        if (link_errno < 0 || link_errno == 17) {
          if (interactive) {
            fprintf(stderr, dcgettext(((void *)0), "%s: replace %s? ", 5),
                    program_name,
                    quotearg_style(shell_escape_always_quoting_style, dest));
            if (!yesno()) {
              free(rel_source);
          ...
          if (backup_type != no_backups) {
            backup_base =
                find_backup_file_name(destdir_fd, dest_base, backup_type);
    ...
    link_errno =
        (symbolic_link
             ...
             : force_linkat(-100, source, destdir_fd, dest_base,
                            logical ? 0x400 : 0, force, link_errno));
  }
  ...
  if (link_errno <= 0) {
    ...
    if (verbose) {
      ...
      if (backup_base) {
        ...
        quoted_backup =
            quotearg_n_style(2, shell_escape_always_quoting_style, backup);
        backup_sep = " ~ ";
        free(alloc);
      }
    ...
    error(0, link_errno,
          (symbolic_link
               ? (link_errno != 36 && *source
                      ? dcgettext(((void *)0),
                                  "failed to create symbolic link %s", 5)

                      : dcgettext(((void *)0),
                                  "failed to create symbolic link %s -> %s", 5))
               : (link_errno == 31

                      ? dcgettext(((void *)0),
                                  "failed to create hard link to %.0s%s", 5)

                      : (link_errno == 122 || link_errno == 17

                         || link_errno == 28 || link_errno == 30)
                            ? dcgettext(((void *)0),
                                        "failed to create hard link %s", 5)
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "current directory.\nIn the 3rd and 4th forms, create links to "
            "each TARGET in DIRECTORY.\nCreate hard links by default, symbolic "
            "links with --symbolic.\nBy default, each destination (name of new "
            "link) should not already exist.\nWhen creating hard links, each "
            "TARGET must exist.  Symbolic links\ncan hold arbitrary text; if "
            "later resolved, a relative link is\ninterpreted in relation to "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "the superuser to attempt to hard link\n                           "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "TARGETs that are symbolic links\n  -n, --no-dereference     "
                  "   treat LINK_NAME as a normal file if\n                    "
                  "            it is a symbolic link to a directory\n  -P, "
                  "--physical              make hard links directly to "
                  "symbolic links\n  -r, --relative              with -s, "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "-t, --target-directory=DIRECTORY  specify the DIRECTORY in which "
            "to create\n                                the links\n  -T, "
    ...
    printf(dcgettext(((void *)0),
                     ...
                     "option specified controls\nbehavior when a TARGET is a "
                     "symbolic link, defaulting to %s.\n",
                     5)
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "bdfinrst:vFLPS:T", long_options,
        ...
        if (stat(optarg, &st) != 0)
          ((!!sizeof(struct {
            _Static_assert(
                ...
                "\"failed to access %s\", 5), quotearg_style "
                "(shell_escape_always_quoting_style, optarg)), assume (false))"
                ")");
            int _gl_dummy;
          }))
        ...
        if (!((((st.st_mode)) & 0170000) == (0040000)))
          ((!!sizeof(struct {
            _Static_assert(
                ...
                "1"
                ", "
                "(error (1, 0, dcgettext (((void *)0), \"target %s is not a "
                "directory\", 5), quotearg_style "
                "(shell_escape_always_quoting_style, optarg)), assume (false))"
                ")");
            int _gl_dummy;
          }))
               ? ((error(1, 0,
                         dcgettext(((void *)0), "target %s is not a directory",
                                   5),
                         quotearg_style(shell_escape_always_quoting_style,
                                        optarg)),
                   ((0) ? (void)0 : __builtin_unreachable())))
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "ln", "GNU coreutils", Version, ("Mike Parker"),
  ...
  if (relative && !symbolic_link)
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"cannot do "
                        "--relative without --symbolic\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0),
                             "cannot do --relative without --symbolic", 5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0),
                             "cannot do --relative without --symbolic", 5)),
  ...
  if (no_target_directory) {
    if (target_directory)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          ...
                          "combine --target-directory \" \"and "
                          "--no-target-directory\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
    ...
    if (n_files != 2) {
      if (n_files < 2)
        error(0, 0,
              ...
              dcgettext(((void *)0),
                        "missing destination file operand after %s", 5),
    ...
    if (link_errno < 0 || link_errno == 17 || link_errno == 20
        ...
        || link_errno == 22) {
      char const *d = target_directory ? target_directory : file[n_files - 1];
  ...
  if (target_directory) {
    ...
    if (2 <= n_files && remove_existing_files
      ...
      dest_set = hash_initialize(DEST_INFO_INITIAL_CAPACITY,
                                 ...
                                 ((void *)0), triple_hash, triple_compare,
                                 triple_free);
      if (dest_set == ((void *)0))
        xalloc_die();
    }
    ...
    for (int i = 0; i < n_files; ++i) {
      char *dest_base;
      char *dest = file_name_concat(target_directory, last_component(file[i]),
                                    &dest_base);
