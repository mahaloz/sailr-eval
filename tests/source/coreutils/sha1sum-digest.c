...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
  ...
  algo_name_len = strlen("SHA1");
  if ((strncmp(s + i, "SHA1", algo_name_len) == 0)) {
    i += algo_name_len;
    if (s[i] == ' ')
      ++i;
    if (s[i] == '(') {
...
digest_check(char const *checkfile_name) {
  ...
  unsigned char bin_buffer_unaligned[(160 / 8) + 4];
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "sha1sum", "GNU coreutils", Version,
  ...
  min_digest_line_length = ((160 / 4) + 1 + 1);
  digest_hex_bytes = (160 / 4);
