...
FILE *freopen_safer(char const *, char const *, FILE *)
    ...
    __attribute__((__nonnull__(2, 3)));
enum Shell_syntax { SHELL_SYNTAX_BOURNE, SHELL_SYNTAX_C, SHELL_SYNTAX_UNKNOWN };

static struct obstack lsc_obstack;

static char const *const slack_codes[] = {"NORMAL",
                                          "NORM",
                                          "FILE",
                                          "RESET",
                                          "DIR",
...
static char const *const ls_codes[] = {
    "no", "no", "fi", "rs", "di", "ln", "ln", "ln",       "or", "mi",
    "pi", "pi", "so", "bd", "bd", "cd", "cd", "do",       "ex", "lc",
    "lc", "rc", "rc", "ec", "ec", "su", "su", "sg",       "sg", "st",
    "ow", "ow", "tw", "tw", "ca", "mh", "cl", ((void *)0)

};
_Static_assert((sizeof(slack_codes) / sizeof *(slack_codes)) ==
                   (sizeof(ls_codes) / sizeof *(ls_codes)),
               "verify ("
               "ARRAY_CARDINALITY (slack_codes) == ARRAY_CARDINALITY (ls_codes)"
               ")");

static _Bool print_ls_colors;

enum {
  PRINT_LS_COLORS_OPTION = 0x7f + 1,
};
...
static struct option const long_options[] = {
    ...
    {"print-ls-colors", 0, ((void *)0), PRINT_LS_COLORS_OPTION},
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "-c, --csh, --c-shell        output C shell code to set "
...
static void parse_line(char const *line, char **keyword, char **arg) {
  ...
  *arg = ((void *)0);
  ...
  for (p = line; ((*__ctype_b_loc())[(int)((to_uchar(*p)))] &
...
static void append_quoted(char const *str) {
  ...
  while (*str != '\0') {
    if (!print_ls_colors)
      ...
      case '\'':
        __extension__({
          struct obstack *__o = (&lsc_obstack);
          if (__extension__({
...
static void append_entry(char prefix, char const *item, char const *arg) {
  ...
  __extension__({
    ...
    if (__extension__({
      ...
      _obstack_newchunk(__o, 1);
    ((void)(*((__o)->next_free)++ = (print_ls_colors ? '\t' : '=')));
  });
  append_quoted(arg);
  if (print_ls_colors)
...
dc_parse_stream(FILE *fp, char const *filename) {
  ...
  while (1) {
    ...
    if (arg == ((void *)0)) {
      error(0, 0,
            dcgettext(((void *)0),
                      "%s:%lu: invalid line;  missing second token", 5),
    ...
    if (unrecognized && (state == ST_TERMSURE || state == ST_TERMYES)) {
      error(0, 0, dcgettext(((void *)0), "%s:%lu: unrecognized keyword %s", 5),
            (filename ? quotearg_n_style_colon(0, shell_escape_quoting_style,
                                               filename)
                      : dcgettext(((void *)0), "<internal>", 5)),
...
dc_parse_file(char const *filename) {
  ...
  if (!(strcmp(filename, "-") == 0) &&
      freopen_safer(filename, "r", stdin) == ((void *)0)) {
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "bcp", long_options, ((void *)0))) !=
         -1)
    switch (optc) {
    case 'b':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "dircolors", "GNU coreutils", Version,
                  ("H. Peter Anvin"), (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (print_database && print_ls_colors) {
    error(0, 0,
          ...
          dcgettext(((void *)0),
                    "options --print-database and --print-ls-colors "
  ...
  if (print_database) {
      ...
      if (syntax == SHELL_SYNTAX_UNKNOWN) {
        ((!!sizeof(struct {
          _Static_assert(
              ...
              "1"
              ", "
              "(error (1, 0, dcgettext (((void *)0), \"no SHELL environment "
              "variable, and no shell type option given\", 5)), assume (false))"
              ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "no SHELL environment variable, and no shell "
                                 "type option given",
                                 5)),
    ...
    if (ok) {
      size_t len = __extension__({
        struct obstack const *__o = (&lsc_obstack);
        (size_t)(__o->next_free - __o->object_base);
      });
      char *s = __extension__({
        struct obstack *__o1 = (&lsc_obstack);
