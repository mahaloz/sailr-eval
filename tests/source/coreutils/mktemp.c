...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... [TEMPLATE]\n", 5),
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0),
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "Files are created u+rw, and directories u+rwx, "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "      --suffix=SUFF   append SUFF to TEMPLATE; SUFF must "
                  "not contain a slash.\n                        This option "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -t                  interpret TEMPLATE as a single file "
                  "name component,\n                        relative to a "
...
static size_t count_consecutive_X_s(char const *s, size_t len) {
  size_t n = 0;
...
static int mkstemp_len(char *tmpl, size_t suff_len, size_t x_len,
                       _Bool dry_run) {
  return gen_tempname_len(tmpl, suff_len, 0, dry_run ? 2 : 0, x_len);
}

static int mkdtemp_len(char *tmpl, size_t suff_len, size_t x_len,
                       _Bool dry_run) {
  return gen_tempname_len(tmpl, suff_len, 0, dry_run ? 2 : 1, x_len);
}
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "dp:qtuV", longopts, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'd':
      create_directory = 1;
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "mktemp", "GNU coreutils", Version, ("Jim Meyering"),
                  ("Eric Blake"), (char *)((void *)0));
      exit(0);
      break;
      ;
    default:
  ...
  n_args = argc - optind;
  if (2 <= n_args) {
    error(0, 0, dcgettext(((void *)0), "too many templates", 5));
    usage(1);
  }
    ...
    if (!len || template[len - 1] != 'X') {
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "1"
            ", "
            "(error (1, 0, dcgettext (((void *)0), \"with --suffix, template "
            "%s must end in X\", 5), quote (template)), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0),
                               "with --suffix, template %s must end in X", 5),
                     quote(template)),
  ...
  if (suffix_len && last_component(suffix) != suffix) {
    ((!!sizeof(struct {
      _Static_assert(
          ...
          "1"
          ", "
          "(error (1, 0, dcgettext (((void *)0), \"invalid suffix %s, contains "
          "directory separator\", 5), quote (suffix)), assume (false))"
          ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
  ...
  if (x_count < 3)
    ((!!sizeof(struct {
      _Static_assert(1,
                     ...
                     "1"
                     ", "
                     "(error (1, 0, dcgettext (((void *)0), \"too few X's in "
                     "template %s\", 5), quote (template)), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0), "too few X's in template %s", 5),
                   quote(template)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0), "too few X's in template %s", 5),
      ...
      if (last_component(template) != template)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "template, %s, contains directory separator\", 5), "
      ...
      if ((((template)[0]) == '/'))
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"invalid "
                            "template, %s; with --tmpdir,\" \" it may not be "
                            "absolute\", 5), quote (template)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
  ...
  if (create_directory) {
    ...
    if (err != 0) {
      if (!suppress_file_err)
        error(0, (*__errno_location()),
              dcgettext(((void *)0),
                        "failed to create directory via template %s", 5),
              quote(template));
      status = 1;
    }
  } else {
    ...
    if (fd < 0 || (!dry_run && close(fd) != 0)) {
      if (!suppress_file_err)
        error(
            0, (*__errno_location()),
            dcgettext(((void *)0), "failed to create file via template %s", 5),
            quote(template));
      status = 1;
    }
  }
  ...
  if (status == 0) {
    ...
    if (!dry_run && ((stdout_closed = 1), close_stream(stdout) != 0)) {
      ...
      remove(dest_name);
      if (!suppress_file_err)
        error(0, saved_errno, dcgettext(((void *)0), "write error", 5));
      status = 1;
    }
  }
