...
extern char *xreadlink(char const *filename) __attribute__((__malloc__))
__attribute__((__malloc__(free, 1)));
enum strtol_error {
  LONGINT_OK = 0,
...
static int parse_size(char const *str, size_t *size) {
  uintmax_t tmp_size;
  enum strtol_error e =
      xstrtoumax(str, ((void *)0), 10, &tmp_size, "EGkKMPTYZ0");
...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s OPTION... COMMAND\n", 5),
           program_name);
    fputs_unlocked(dcgettext(((void *)0),
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "fully buffered with the buffer\nsize set to MODE bytes.\n",
            5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "streams ('tee' does\nfor example) then that will override "
                  "corresponding changes by 'stdbuf'.\nAlso some filters (like "
                  "'dd' and 'cat' etc.) don't use streams for I/O,\nand are "
...
static void set_program_path(char const *arg) {
  if (strchr(arg, '/')) {
    ...
    if (path)
      ...
      char *dir;
      path = xstrdup(path);
      for (dir = strtok(path, ":"); dir != ((void *)0);
           dir = strtok(((void *)0), ":")) {
...
static void set_LD_PRELOAD(void) {
    ...
    if (!*path)
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "EXIT_CANCELED"
            ", "
            "(error (EXIT_CANCELED, 0, dcgettext (((void *)0), \"failed to "
            "find %s\", 5), quote (\"libstdbuf.so\")), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(EXIT_CANCELED, 0,
                     dcgettext(((void *)0), "failed to find %s", 5),
                     quote("libstdbuf.so")),
  ...
  if (old_libs)
    ret =
        rpl_asprintf(&LD_PRELOAD, "%s=%s:%s", preload_env, old_libs, libstdbuf);
  else
  ...
  if (ret != 0) {
    ((!!sizeof(struct {
      _Static_assert(EXIT_CANCELED,
                     ...
                     "EXIT_CANCELED"
                     ", "
                     "(error (EXIT_CANCELED, (*__errno_location ()), dcgettext "
                     "(((void *)0), \"failed to update the environment with "
                     "%s\", 5), quote (LD_PRELOAD)), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(EXIT_CANCELED, (*__errno_location()),
                   dcgettext(((void *)0),
                             "failed to update the environment with %s", 5),
                   quote(LD_PRELOAD)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(EXIT_CANCELED, (*__errno_location()),
                   dcgettext(((void *)0),
    ...
    if (stdbuf[i].optarg) {
      ...
      if (*stdbuf[i].optarg == 'L')
        ret = rpl_asprintf(
            ...
            (__extension__({
              int __res;
              if (sizeof(stdbuf[i].optc) > 1) {
                if (__builtin_constant_p(stdbuf[i].optc)) {
                  int __c = (stdbuf[i].optc);
                  __res = __c < -128 || __c > 255
                              ? __c
                              : (*__ctype_toupper_loc())[__c];
                } else
                  __res = toupper(stdbuf[i].optc);
              } else
                __res = (*__ctype_toupper_loc())[(int)(stdbuf[i].optc)];
              __res;
            })));
      else
        ret = rpl_asprintf(
      ...
      if (putenv(var) != 0) {
        ((!!sizeof(struct {
          _Static_assert(
              ...
              ", "
              "(error (EXIT_CANCELED, (*__errno_location ()), dcgettext "
              "(((void *)0), \"failed to update the environment with %s\", 5), "
              "quote (var)), assume (false))"
              ")");
          int _gl_dummy;
        }))
             ? ((error(EXIT_CANCELED, (*__errno_location()),
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "+i:o:e:", longopts, ((void *)0))) !=
         -1) {
    int opt_fileno;
    ...
    case 'o':
      ...
      ((void)sizeof(
       ...
       __extension__({
         if (0 <= opt_fileno &&
             opt_fileno < (sizeof(stdbuf) / sizeof *(stdbuf)))
           ;
         else
           __assert_fail(
      ...
      if (c == 'i' && *optarg == 'L') {
        ...
        error(0, 0,
              dcgettext(((void *)0), "line buffering stdin is meaningless", 5));
        usage(EXIT_CANCELED);
      }
      ...
      if (!(strcmp(optarg, "L") == 0) &&
        ...
        ((!!sizeof(struct {
          _Static_assert(EXIT_CANCELED,
                         ...
                         ", "
                         "(error (EXIT_CANCELED, (*__errno_location ()), "
                         "dcgettext (((void *)0), \"invalid mode %s\", 5), "
                         "quote (optarg)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(EXIT_CANCELED, (*__errno_location()),
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "stdbuf", "GNU coreutils", Version, ("Padraig Brady"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (!set_libstdbuf_options()) {
    error(
        0, 0,
        dcgettext(((void *)0), "you must specify a buffering mode option", 5));
    usage(EXIT_CANCELED);
  }
  ...
  error(0, (*__errno_location()),
        dcgettext(((void *)0), "failed to run command %s", 5), quote(argv[0]));
  return exit_status;
}
