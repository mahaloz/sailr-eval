...
void usage(int status) {
  ...
  ((void)sizeof((status == 0) ? 1 : 0), __extension__({
     if (status == 0)
       ;
     else
       __assert_fail("status == EXIT_SUCCESS", "src/echo.c", 40,
  ...
  fputs_unlocked(
      dcgettext(((void *)0),
                ...
                "have issues outputting option-like strings.\n",
                5),
      stdout)
...
int main(int argc, char **argv) {
  ...
  if (allow_options && argc == 2) {
    ...
    if ((strcmp(argv[1], "--version") == 0)) {
      version_etc(stdout, "echo", "GNU coreutils", Version, ("Brian Fox"),
                  ("Chet Ramey"), (char *)((void *)0));
      return 0;
    }
  }
