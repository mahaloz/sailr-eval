...
static _Bool fmt(FILE *f, char const *);
...
void usage(int status) {
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0), "Usage: %s [-WIDTH] [OPTION]... [FILE]...\n", 5),
        program_name);
    fputs_unlocked(dcgettext(((void *)0),
                             "Reformat each paragraph in the FILE(s), writing "
                             "to standard output.\nThe option -WIDTH is an "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -c, --crown-margin        preserve indentation of first "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "two after sentences\n  -w, --width=WIDTH         maximum line "
            "width (default of 75 columns)\n  -g, --goal=WIDTH          goal "
            "width (default of 93% of width)\n",
            5),
        stdout)
...
int main(int argc, char **argv) {
  ...
  while ((optchar = getopt_long(argc, argv, "0123456789cstuw:p:g:",
    ...
    default:
      if (((unsigned int)(optchar) - '0' <= 9))
        error(0, 0,
              dcgettext(((void *)0),
                        "invalid option -- %c; -WIDTH is recognized only when "
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "fmt", "GNU coreutils", Version, ("Ross Paterson"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (max_width_option) {
    ...
    max_width = xdectoumax(max_width_option, 0, 5000 / 2, "",
                           ...
                           dcgettext(((void *)0), "invalid width", 5), 0);
  }
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, (*__errno_location ()), \"%s\", dcgettext (((void "
...
fmt(FILE *f, char const *file) {
  ...
  if (f == stdin)
    clearerr_unlocked(f);
  else if (rpl_fclose(f) != 0 && err < 0)
    err = (*__errno_location());
  if (0 <= err)
    error(0, err, err ? "%s" : dcgettext(((void *)0), "read error", 5),
          quotearg_n_style_colon(0, shell_escape_quoting_style, file));
  return err < 0;
}

static void set_other_indent(_Bool same_paragraph) {
  if (split)
...
get_paragraph(FILE *f) {
  ...
  ((void)sizeof((unused_word_type < word_limit) ? 1 : 0), __extension__({
     if (unused_word_type < word_limit)
       ;
     else
       __assert_fail("word < word_limit", "src/fmt.c", 624,
...
static int get_prefix(FILE *f) {
  ...
  if (prefix_length == 0)
    next_prefix_indent =
        prefix_lead_space < in_column ? prefix_lead_space : in_column;
  else {
    char const *p;
...
static void check_punctuation(WORD *w) {
  ...
  w->punct =
      !!((*__ctype_b_loc())[(int)((fin))] & (unsigned short int)_ISpunct);
...
static void flush_paragraph(void) {
  ...
  memmove(unused_word_type, split_point,
          (word_limit - split_point + 1) * sizeof *unused_word_type);
  ...
  for (start = word_limit - 1; start >= unused_word_type; start--) {
    ...
    do {
      ...
      if (start == unused_word_type && last_line_length > 0)
        wcost += ((((COST)((len - last_line_length) * 10)) *
                   ((COST)((len - last_line_length) * 10))) /
                  2);
      if (wcost < best) {
        best = wcost;
        start->next_break = w;
