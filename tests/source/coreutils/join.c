...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... FILE1 FILE2\n", 5)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "\n  -a FILENUM             also print unpairable lines from "
                  "file FILENUM, where\n                           FILENUM is "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -e STRING              replace missing (empty) input "
                  "fields with STRING;\n                           I.e., "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "comparing fields\n  -j FIELD               equivalent to "
                  "'-1 FIELD -2 FIELD'\n  -o FORMAT              obey FORMAT "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "output lines\n  -1 FIELD               join on this FIELD of file "
            ...
            "correctly sorted\n      --header           treat the first line "
            "in each file as field headers,\n                           print "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "field number counted\nfrom 1.  FORMAT is one or more comma or "
            "blank separated specifications,\neach being 'FILENUM.FIELD' or "
            "'0'.  Default FORMAT outputs the join field,\nthe remaining "
            "fields from FILE1, the remaining fields from FILE2, "
            "all\nseparated by CHAR.  If FORMAT is the keyword 'auto', then "
            "the first\nline of each file determines the number of fields "
            "output for each line.\n\nImportant: FILE1 and FILE2 must be "
            "sorted on the join fields.\nE.g., use \"sort -k 1b,1\" if 'join' "
            "has no options,\nor use \"join -t ''\" if 'sort' has no "
            "options.\nNote, comparisons honor the rules specified by "
            "'LC_COLLATE'.\nIf the input is not sorted and some lines cannot "
...
static void extract_field(struct line *line, char *field, size_t len) {
  if (line->nfields >= line->nfields_allocated) {
    line->fields = ((!!sizeof(struct {
      _Static_assert(sizeof *(line->fields) != 1,
                     "verify_expr ("
                     "sizeof *(line->fields) != 1"
                     ", "
                     "x2nrealloc (line->fields, &line->nfields_allocated, "
                     "sizeof *(line->fields))"
                     ")");
      int _gl_dummy;
    }))
...
static int keycmp(struct line const *line1, struct line const *line2,
...
static void check_order(const struct line *prev, const struct line *current,
                        int whatfile) {
  if (check_input_order != CHECK_ORDER_DISABLED &&
    ...
    if (!issued_disorder_warning[whatfile - 1]) {
      ...
      if (keycmp(prev, current, join_field, join_field) > 0) {
        ...
        len = (((0x7fffffff) < (len)) ? (0x7fffffff) : (len));
        ...
        error((check_input_order == CHECK_ORDER_ENABLED ? 1 : 0), 0,
              dcgettext(((void *)0),
                        ...
                        ": is not sorted: %.*s",
                        5),
              g_names[whatfile - 1], line_no[whatfile - 1], (int)len,
              current->buf.buffer);
...
static inline void reset_line(struct line *line) { line->nfields = 0; }
...
static void prfield(size_t n, struct line const *line) {
  size_t len;
...
static void prfields(struct line const *line, size_t join_field,
                     size_t autocount) {
  size_t i;
...
static void system_join(FILE *fp1, FILE *fp2) {
  ...
  while (seq1.count && seq2.count) {
    ...
    while (!keycmp(seq1.lines[seq1.count - 1], seq2.lines[0], join_field_1,
                   join_field_2));
    ...
    while (!keycmp(seq1.lines[0], seq2.lines[seq2.count - 1], join_field_1,
                   join_field_2));
    ...
    if (!eof1) {
      do {
        struct line *tmp = seq1.lines[0];
        seq1.lines[0] = seq1.lines[seq1.count - 1];
        seq1.lines[seq1.count - 1] = tmp;
      } while (0);
      ;
      seq1.count = 1;
    } else
    ...
    if (!eof2) {
      do {
        struct line *tmp = seq2.lines[0];
        seq2.lines[0] = seq2.lines[seq2.count - 1];
        seq2.lines[seq2.count - 1] = tmp;
      } while (0);
      ;
      seq2.count = 1;
    } else
...
static void add_field(int file, size_t field) {
  struct outlist *o;

  ((void)sizeof((file == 0 || file == 1 || file == 2) ? 1 : 0), __extension__({
     if (file == 0 || file == 1 || file == 2)
       ;
     else
       __assert_fail("file == 0 || file == 1 || file == 2", "src/join.c", 823,
                     __extension__ __PRETTY_FUNCTION__);
   }));

  ((void)sizeof((file != 0 || field == 0) ? 1 : 0), __extension__({
     if (file != 0 || field == 0)
       ;
     else
       __assert_fail("file != 0 || field == 0", "src/join.c", 824,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  ...
  if (s_err == LONGINT_OVERFLOW ||
    ...
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"invalid field "
                        "number: %s\", 5), quote (str)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0, dcgettext(((void *)0), "invalid field number: %s", 5),
                   quote(str)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0, dcgettext(((void *)0), "invalid field number: %s", 5),
                   quote(str)),
...
static void decode_field_spec(char const *s, int *file_index,
                              size_t *field_index) {
    ...
    if (s[1]) {
      ...
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       ", "
                       "(error (1, 0, dcgettext (((void *)0), \"invalid field "
                       "specifier: %s\", 5), quote (s)), assume (false))"
                       ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0), "invalid field specifier: %s", 5),
                     quote(s)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0,
                     dcgettext(((void *)0), "invalid field specifier: %s", 5),
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"invalid file number in "
             "field spec: %s\", 5), quote (s)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0),
                             "invalid file number in field spec: %s", 5),
                   quote(s)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0),
...
static void set_join_field(size_t *var, size_t val) {
  if (*var != (18446744073709551615UL) && *var != val) {
    ...
    ((!!sizeof(struct {
      _Static_assert(1,
                     ...
                     "1"
                     ", "
                     "(error (1, 0, dcgettext (((void *)0), \"incompatible "
                     "join fields %lu, %lu\", 5), var1, val1), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(
                 1, 0,
                 dcgettext(((void *)0), "incompatible join fields %lu, %lu", 5),
                 var1, val1),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(
                 1, 0,
        ...
        ((!!sizeof(struct {
          _Static_assert(
              1, "verify_expr ("
                 ...
                 ", "
                 "(error (1, 0, dcgettext (((void *)0), \"invalid field "
                 "number: %s\", 5), quote (optarg)), assume (false))"
                 ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "invalid field number: %s", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "invalid field number: %s", 5),
        ...
        ((!!sizeof(struct {
          _Static_assert(
              1, "verify_expr ("
                 "1"
                 ", "
                 "(error (1, 0, dcgettext (((void *)0), \"conflicting "
                 "empty-field replacement strings\", 5)), assume (false))"
                 ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
          ...
          ((!!sizeof(struct {
            _Static_assert(
                1, "verify_expr ("
                   ...
                   "tab %s\", 5), quote (optarg)), assume (false))"
                   ")");
            int _gl_dummy;
          }))
               ? ((error(1, 0,
      ...
      if (0 <= tab && tab != newtab)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), "
                            "\"incompatible tabs\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0, dcgettext(((void *)0), "incompatible tabs", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0, dcgettext(((void *)0), "incompatible tabs", 5)),
                 ((0) ? (void)0 : __builtin_unreachable()))));
      tab = newtab;
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             ...
             "\"both files cannot be standard input\", 5)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, (*__errno_location()),
                   dcgettext(((void *)0), "both files cannot be standard input",
                             5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, (*__errno_location()),
                   dcgettext(((void *)0), "both files cannot be standard input",
  ...
  if (issued_disorder_warning[0] || issued_disorder_warning[1])
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"input is not "
                        "in sorted order\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0), "input is not in sorted order", 5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0), "input is not in sorted order", 5)),
             ((0) ? (void)0 : __builtin_unreachable()))));
