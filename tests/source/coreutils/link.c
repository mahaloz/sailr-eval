...
void usage(int status) {
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0), "Usage: %s FILE1 FILE2\n  or:  %s OPTION\n", 5)

            ,
        program_name, program_name);
    fputs_unlocked(dcgettext(((void *)0),
                             "Call the link function to create a link named "
...
int main(int argc, char **argv) {
  ...
  parse_gnu_standard_options_only(argc, argv, "link", "GNU coreutils", Version,
                                  1, usage, ("Michael Stone"),
                                  (char const *)((void *)0));
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             ...
             "\"cannot create link %s to %s\", 5), quotearg_n_style (0, "
             "shell_escape_always_quoting_style, argv[optind + 1]), "
      ...
      int _gl_dummy;
         ...
         ? ((error(1, (*__errno_location()),
                   dcgettext(((void *)0), "cannot create link %s to %s", 5),
