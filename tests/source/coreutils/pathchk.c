...
enum { PORTABILITY_OPTION = 0x7f + 1 };
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "empty names and leading \"-\"\n      --portability   check for "
            "all POSIX systems (equivalent to -p -P)\n",
            5),
        stdout)
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "+pP", longopts, ((void *)0))) != -1) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "pathchk", "GNU coreutils", Version, ("Paul Eggert"),
  ...
  for (; optind < argc; ++optind)
    ok &= validate_file_name(argv[optind], check_basic_portability,
...
no_leading_hyphen(char const *file) {
  ...
  for (p = file; (p = strchr(p, '-')); p++)
    if (p == file || p[-1] == '/') {
      error(0, 0,
            dcgettext(((void *)0), "leading '-' in a component of file name %s",
                      5),
...
portable_chars_only(char const *file, size_t filelen) {
  ...
  if (*invalid) {
    ...
    error(0, 0,
          ...
          dcgettext(((void *)0), "nonportable character %s in file name %s", 5),
...
__attribute__((__pure__)) static char *component_start(char *f) {
  while (*f == '/')
    f++;
  return f;
}

__attribute__((__pure__)) static size_t component_len(char const *f) {
  size_t len;
...
validate_file_name(char *file, _Bool check_basic_portability,
  ...
  if ((check_basic_portability || check_extra_portability) && filelen == 0) {
    ...
    error(0, 0, dcgettext(((void *)0), "empty file name", 5));
    return 0;
  }
  ...
  if (check_basic_portability || (!file_exists && 256 <= filelen)) {
    ...
    if (check_basic_portability)
      ...
      if (size < 0 && (*__errno_location()) != 0) {
        error(0, (*__errno_location()),
              ...
              dcgettext(((void *)0),
                        "%s: unable to determine maximum file name length", 5),
              dir);
        return 0;
      }
      maxsize =
    ...
    if (maxsize <= filelen) {
      ...
      error(0, 0,
            dcgettext(((void *)0),
                      "limit %lu exceeded by length %lu of file name %s", 5),
            maxlen, len,
            quotearg_style(shell_escape_always_quoting_style, file));
      return 0;
    }
