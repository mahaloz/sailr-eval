...
static char const *const output_error_args[] = {"warn", "warn-nopipe", "exit",
                                                "exit-nopipe", ((void *)0)

};
static enum output_error const output_error_types[] = {
    output_error_warn, output_error_warn_nopipe, output_error_exit,
    output_error_exit_nopipe};
_Static_assert((sizeof(output_error_args) / sizeof *(output_error_args)) ==
                   (sizeof(output_error_types) / sizeof *(output_error_types)) +
                       1,
               "verify ("
               "ARRAY_CARDINALITY (output_error_args) == ARRAY_CARDINALITY "
               "(output_error_types) + 1"
               ")");
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -p                        diagnose errors writing to non "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "default operation when --output-error is not specified, is "
            "to\nexit immediately on error writing to a pipe, and diagnose "
  ...
  if (close(0) != 0)
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        ...
                        "(((void *)0), \"standard input\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
...
tee_files(int nfiles, char **files) {
  ...
  files[0] = bad_cast(dcgettext(((void *)0), "standard output", 5));
  ...
  for (i = 1; i <= nfiles; i++) {
    ...
    if (descriptors[i] == ((void *)0)) {
      error(output_error == output_error_exit ||
            ...
            (*__errno_location()), "%s",
            quotearg_n_style_colon(0, shell_escape_quoting_style, files[i]));
      ok = 0;
    } else {
  ...
  while (n_outputs) {
    ...
    for (i = 0; i <= nfiles; i++)
      if (descriptors[i] &&
        ...
        _Bool fail =
            (*__errno_location()) != 32 || (output_error == output_error_exit ||
