...
extern int getutline_r(const struct utmp *__line, struct utmp *__buffer,
                       struct utmp **__result)
    __attribute__((__nothrow__, __leaf__));
...
enum { READ_UTMP_CHECK_PIDS = 1, READ_UTMP_USER_PROCESS = 2 };

char *extract_trimmed_name(const STRUCT_UTMP *ut) __attribute__((__malloc__))
__attribute__((__malloc__(free, 1))) __attribute__((__returns_nonnull__));
...
__attribute__((__pure__)) static size_t count_ampersands(char const *str) {
  size_t count = 0;
  do {
    if (*str == '&')
      count++;
...
static char *create_fullname(char const *gecos_name, char const *user_name) {
  ...
  if (ampersands != 0) {
    ...
    size_t product;
    if (__builtin_mul_overflow(ulen, ampersands - 1, &product) ||
        __builtin_add_overflow(rsize, product, &rsize))
      xalloc_die();
  }
  ...
  while (*gecos_name) {
    if (*gecos_name == '&') {
      ...
      if (((*__ctype_b_loc())[(int)((to_uchar(*uname)))] &
           (unsigned short int)_ISlower))
        *r++ = (__extension__({
          int __res;
          if (sizeof(to_uchar(*uname++)) > 1) {
            if (__builtin_constant_p(to_uchar(*uname++))) {
              int __c = (to_uchar(*uname++));
              __res =
                  __c < -128 || __c > 255 ? __c : (*__ctype_toupper_loc())[__c];
            } else
              __res = toupper(to_uchar(*uname++));
          } else
            __res = (*__ctype_toupper_loc())[(int)(to_uchar(*uname++))];
          __res;
        }));
      while (*uname)
        *r++ = *uname++;
...
static char const *idle_string(time_t when) {
  static time_t now = 0;
  static char buf[(((((sizeof(long int) * 8) -
                      (!((__typeof__(long int))0 < (__typeof__(long int))-1))) *
                         146 +
                     484) /
                    485) +
                   (!((__typeof__(long int))0 < (__typeof__(long int))-1))) +
...
static char const *time_string(const STRUCT_UTMP *utmp_ent) {
  static char buf[(((((sizeof(intmax_t) * 8) -
...
static void print_entry(const STRUCT_UTMP *utmp_ent) {
  struct stat stats;
  time_t last_change;
  char mesg;
  ...
  printf(" %c%-8.*s", mesg, (int)sizeof(utmp_ent->ut_line), utmp_ent->ut_line);
...
static void print_long_entry(const char name[]) {
  ...
  printf(dcgettext(((void *)0), "Login name: ", 5));
  printf("%-28s", name);

  printf(dcgettext(((void *)0), "In real life: ", 5));
  if (pw == ((void *)0)) {

    printf(" %s", dcgettext(((void *)0), "???\n", 5));
    return;
  } else {
    char *const comma = strchr(pw->pw_gecos, ',');
    char *result;
  ...
  if (include_home_and_shell) {
    printf(dcgettext(((void *)0), "Directory: ", 5));
    printf("%-29s", pw->pw_dir);
    printf(dcgettext(((void *)0), "Shell: ", 5));
  ...
  if (include_project) {
    ...
    char *const project = xmalloc(strlen(pw->pw_dir) + strlen(baseproject) + 1);
    ...
    if (stream) {
      ...
      printf(dcgettext(((void *)0), "Project: ", 5));
  ...
  if (include_plan) {
    ...
    char *const plan = xmalloc(strlen(pw->pw_dir) + strlen(baseplan) + 1);
...
static void print_heading(void) {
  printf("%-8s", dcgettext(((void *)0), "Login", 5));
  if (include_fullname)
    printf(" %-19s", dcgettext(((void *)0), "Name", 5));
  printf(" %-9s", dcgettext(((void *)0), " TTY", 5));
  if (include_idle)
    printf(" %-6s", dcgettext(((void *)0), "Idle", 5));
  printf(" %-*s", time_format_width, dcgettext(((void *)0), "When", 5));

  if (include_where)
    printf(" %s", dcgettext(((void *)0), "Where", 5));
    ...
    if ((((utmp_buf)->ut_user)[0] &&
      ...
      if (argc_names) {
        for (int i = 0; i < argc_names; i++)
          if ((strncmp(((utmp_buf)->ut_user), argv_names[i], UT_USER_SIZE) ==
               0)) {
...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... [USER]...\n", 5),
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0),
                  "\n  -l              produce long format output for the "
                  "specified USERs\n  -b              omit the user's home "
                  "directory and shell in long format\n  -h              omit "
                  "the user's project file in long format\n  -p              "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -f              omit the line of column headings in short "
                  "format\n  -w              omit the user's full name in "
                  "short format\n  -i              omit the user's full name "
                  "and remote host in short format\n  -q              omit the "
                  "user's full name, remote host and idle time\n               "
                  "   in short format\n",
                  5),
        stdout)
...
int main(int argc, char **argv) {
  ...
  if (!do_short_format && n_users == 0) {
    error(0, 0,
          dcgettext(((void *)0),
                    "no username specified; at least one must be specified "
                    "when using -l",
                    5)
