...
enum Upper_Lower_class { UL_LOWER, UL_UPPER, UL_NONE };
...
void usage(int status) {
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0), "Usage: %s [OPTION]... STRING1 [STRING2]\n", 5)
...
__attribute__((__pure__)) static enum Char_class
...
static void append_normal_char(struct Spec_list *list, unsigned char c) {
  ...
  ((void)sizeof((list->tail) ? 1 : 0), __extension__({
     if (list->tail)
       ...
       __assert_fail("list->tail", "src/tr.c", 649,
...
append_range(struct Spec_list *list, unsigned char first, unsigned char last) {
  if (last < first) {
    ...
    error(0, 0,
          ...
          dcgettext(((void *)0),
                    "range-endpoints of '%s-%s' are in reverse collating "
                    "sequence order",
                    5),
          tmp1, tmp2);
    free(tmp1);
  ...
  ((void)sizeof((list->tail) ? 1 : 0), __extension__({
     if (list->tail)
       ...
       __assert_fail("list->tail", "src/tr.c", 679,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  list->tail->next = new;
  list->tail = new;
...
append_char_class(struct Spec_list *list, char const *char_class_str,
                  size_t len) {
  ...
  ((void)sizeof((list->tail) ? 1 : 0), __extension__({
     if (list->tail)
       ...
       __assert_fail("list->tail", "src/tr.c", 701,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  list->tail->next = new;
  list->tail = new;
...
static void append_repeated_char(struct Spec_list *list, unsigned char the_char,
  ...
  ((void)sizeof((list->tail) ? 1 : 0), __extension__({
     if (list->tail)
       ...
       __assert_fail("list->tail", "src/tr.c", 721,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  list->tail->next = new;
  list->tail = new;
...
static int find_bracketed_repeat(const struct E_string *es, size_t start_idx,
                                 unsigned char *char_to_repeat,
                                 count *repeat_count,
                                 size_t *closing_bracket_idx) {

  ((void)sizeof((start_idx + 1 < es->len) ? 1 : 0), __extension__({
     if (start_idx + 1 < es->len)
       ;
     else
       __assert_fail("start_idx + 1 < es->len", "src/tr.c", 784,
                     __extension__ __PRETTY_FUNCTION__);
   }));
      ...
      if (digit_str_len == 0) {
        ...
        *repeat_count = 0;
        ...
        if ((xstrtoumax(digit_str, &d_end, *digit_str == '0' ? 8 : 10,
                        repeat_count, ((void *)0)) != LONGINT_OK) ||
          ...
          error(0, 0,
                ...
                dcgettext(((void *)0),
                          "invalid repeat count %s in [c*n] construct", 5),
                quote(tmp));
          free(tmp);
          return -2;
        }
        ...
        if (find_closing_delim(es, i + 2, p[i + 1], &closing_delim_idx)) {
          ...
          if (opnd_str_len == 0) {
            if (p[i + 1] == ':')
              error(0, 0,
                    dcgettext(((void *)0),
                              "missing character class name '[::]'", 5));
            else
              error(0, 0,

                    dcgettext(((void *)0),
                              "missing equivalence class character '[==]'", 5));
            return 0;
          }
          ...
          if (p[i + 1] == ':') {
            ...
            if (!append_char_class(result, opnd_str, opnd_str_len)) {
              if (star_digits_closebracket(es, i + 2))
                ...
                error(0, 0,
                      dcgettext(((void *)0), "invalid character class %s", 5),
                      quote(tmp));
                free(tmp);
                return 0;
              }
            ...
            if (!append_equiv_class(result, opnd_str, opnd_str_len)) {
              if (star_digits_closebracket(es, i + 2))
                ...
                error(0, 0,
                      ...
                      dcgettext(((void *)0),
                                "%s: equivalence class operand must be a "
                                "single character",
                                5),
                      tmp);
                free(tmp);
...
static int get_next(struct Spec_list *s, enum Upper_Lower_class *class) {
  ...
  case RE_CHAR_CLASS:
    ...
    if (s->state == (((18446744073709551615UL) - 1) + 1)) {
      ...
      ((void)sizeof((i < N_CHARS) ? 1 : 0), __extension__({
         if (i < N_CHARS)
           ;
         else
           __assert_fail("i < N_CHARS", "src/tr.c", 1079,
                         __extension__ __PRETTY_FUNCTION__);
       }));
      s->state = i;
    }

    ((void)sizeof((is_char_class_member(p->u.char_class, s->state)) ? 1 : 0),
     __extension__({
       if (is_char_class_member(p->u.char_class, s->state))
         ;
       else
         __assert_fail("is_char_class_member (p->u.char_class, s->state)",
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 0, dcgettext (((void *)0), \"misaligned [:upper:] "
               "and/or [:lower:] construct\", 5)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(
                         ((void *)0),
                         "misaligned [:upper:] and/or [:lower:] construct", 5)),
...
static void get_spec_stats(struct Spec_list *s) {
  ...
  for (p = s->head->next; p; p = p->next) {
    ...
    case RE_RANGE:
      ...
      ((void)sizeof((p->u.range.last_char >= p->u.range.first_char) ? 1 : 0),
       __extension__({
         if (p->u.range.last_char >= p->u.range.first_char)
           ;
         else
           __assert_fail("p->u.range.last_char >= p->u.range.first_char",
    ...
    if (!(length <= new_length && new_length <= ((18446744073709551615UL) - 1)))
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          "1"
                          ", "
                          "(error (1, 0, dcgettext (((void *)0), \"too many "
                          "characters in set\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0), "too many characters in set", 5)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0,
                     dcgettext(((void *)0), "too many characters in set", 5)),
               ((0) ? (void)0 : __builtin_unreachable()))));
...
static void string2_extend(const struct Spec_list *s1, struct Spec_list *s2) {
  struct List_element *p;
  unsigned char char_to_repeat;

  ((void)sizeof((translating) ? 1 : 0), __extension__({
     if (translating)
       ;
     else
       __assert_fail("translating", "src/tr.c", 1377,
                     __extension__ __PRETTY_FUNCTION__);
   }));

  ((void)sizeof((s1->length > s2->length) ? 1 : 0), __extension__({
     if (s1->length > s2->length)
       ;
     else
       __assert_fail("s1->length > s2->length", "src/tr.c", 1378,
                     __extension__ __PRETTY_FUNCTION__);
   }));

  ((void)sizeof((s2->length > 0) ? 1 : 0), __extension__({
     if (s2->length > 0)
       ;
     else
       __assert_fail("s2->length > 0", "src/tr.c", 1379,
                     __extension__ __PRETTY_FUNCTION__);
   }));
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"when translating with "
             "string1 longer than string2,\\nthe latter string must not end "
             "with a character class\", 5)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"the [c*] repeat "
             "construct may not appear in string1\", 5)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 0, dcgettext (((void *)0), \"only one [c*] repeat "
               "construct may appear in string2\", 5)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
      ...
      if (s2->has_equiv_class) {
        ((!!sizeof(struct {
          _Static_assert(
              ...
              "1"
              ", "
              "(error (1, 0, dcgettext (((void *)0), \"[=c=] expressions may "
              "not appear in string2 when translating\", 5)), assume (false))"
              ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
        ...
        ((!!sizeof(struct {
          _Static_assert(
              1, "verify_expr ("
                 "1"
                 ", "
                 "(error (1, 0, dcgettext (((void *)0), \"when translating, "
                 "the only character classes that may appear in\\nstring2 are "
                 "'upper' and 'lower'\", 5)), assume (false))"
                 ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
          ...
          if (s2->length == 0)
            ((!!sizeof(struct {
              _Static_assert(
                  ...
                  "1"
                  ", "
                  "(error (1, 0, dcgettext (((void *)0), \"when not truncating "
                  "set1, string2 must be non-empty\", 5)), assume (false))"
                  ")");
              int _gl_dummy;
            }))
                 ? ((error(1, 0,
                           dcgettext(((void *)0),
                                     "when not truncating set1, string2 must "
                                     "be non-empty",
                                     5)),
                     ((0) ? (void)0 : __builtin_unreachable())))
                 : ((error(1, 0,
        ...
        ((!!sizeof(struct {
          _Static_assert(
              1, "verify_expr ("
                 ...
                 "characters in the domain to one\", 5)), assume (false))"
                 ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 ...
                                 "classes,\nstring2 must map all characters in "
                                 "the domain to one",
                                 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
      ...
      if (s2->n_indefinite_repeats > 0)
        ((!!sizeof(struct {
          _Static_assert(
              ...
              "1"
              ", "
              "(error (1, 0, dcgettext (((void *)0), \"the [c*] construct may "
              "appear in string2 only when translating\", 5)), assume (false))"
              ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0),
                                 "the [c*] construct may appear in string2 "
...
static void squeeze_filter(char *buf, size_t size,
                           size_t (*reader)(char *, size_t)) {

  const int NOT_A_CHAR = 0x7fffffff;
...
static size_t plain_read(char *buf, size_t size) {
...
static size_t read_and_delete(char *buf, size_t size) {
  size_t n_saved;
...
static size_t read_and_xlate(char *buf, size_t size) {
...
static void set_initialize(struct Spec_list *s, _Bool complement_this_set,
                           _Bool *in_set) {
  int c;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "+AcCdst", long_options, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'A':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "tr", "GNU coreutils", Version, ("Jim Meyering"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (max_operands < non_option_args) {
    error(0, 0, dcgettext(((void *)0), "extra operand %s", 5),
          quote(argv[optind + max_operands]));
    ...
    if (complement) {
      ...
      for (int i = 0; i < N_CHARS; i++) {
        if (!in_s1[i]) {
          ...
          ((void)sizeof((ch != -1 || truncate_set1) ? 1 : 0), __extension__({
             if (ch != -1 || truncate_set1)
               ;
             else
               __assert_fail("ch != -1 || truncate_set1", "src/tr.c", 1840,
                             __extension__ __PRETTY_FUNCTION__);
           }));
          if (ch == -1) {
      ...
      ((void)sizeof((c1 == -1 || truncate_set1) ? 1 : 0), __extension__({
         if (c1 == -1 || truncate_set1)
           ;
         else
           __assert_fail("c1 == -1 || truncate_set1", "src/tr.c", 1893,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
    if (squeeze_repeats) {
