...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             "  -m, --mode=MODE    set file permission bits to "
                             "MODE, not a=rw - umask\n",
                             5),
                   stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -Z                   set the SELinux security context to "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "or u, and they\nmust be omitted when TYPE is p.  If MAJOR "
                  "or MINOR begins with 0x or 0X,\nit is interpreted as "
...
int main(int argc, char **argv) {
  ...
  struct selabel_handle *set_security_context = ((void *)0);
  ...
  while ((optc = getopt_long(argc, argv, "m:Z", longopts, ((void *)0))) != -1) {
    ...
    case 'Z':
      if (is_smack_enabled()) {
        ...
        if (optarg)
          ...
          set_security_context = selabel_open(0,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "mknod", "GNU coreutils", Version,
  ...
  newmode = (0400 | 0200 | (0400 >> 3) | (0200 >> 3) | ((0400 >> 3) >> 3) |
             ((0200 >> 3) >> 3));
  if (specified_mode) {
    mode_t umask_value;
    ...
    if (!change)
      ((!!sizeof(struct {
        ...
        int _gl_dummy;
           ...
           ? ((error(1, 0, dcgettext(((void *)0), "invalid mode", 5)),
    ...
    free(change);
    if (newmode & ~((0400 | 0200 | 0100) | ((0400 | 0200 | 0100) >> 3) |
                    (((0400 | 0200 | 0100) >> 3) >> 3)))
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 0, dcgettext (((void *)0), \"mode must specify only "
               "file permission bits\", 5)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
  ...
  expected_operands =
      (argc <= optind || (optind + 1 < argc && argv[optind + 1][0] == 'p') ? 2
                                                                           : 4);
    ...
    if (ret < 0)
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       "*)0), \"failed to set default file creation context to "
                       "%s\", 5), quote (scontext)), assume (false))"
                       ")");
        int _gl_dummy;
      }))
  ...
  block_or_character : {
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 0, dcgettext (((void *)0), \"invalid major device "
               "number %s\", 5), quote (s_major)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(
                   1, 0,
                   dcgettext(((void *)0), "invalid major device number %s", 5),
                   quote(s_major)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(
                   1, 0,
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               ...
               "number %s\", 5), quote (s_minor)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(
                   1, 0,
                   dcgettext(((void *)0), "invalid minor device number %s", 5),
                   quote(s_minor)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(
                   1, 0,
    ...
    if (device == ((dev_t)-1))
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       "1"
                       ", "
                       "(error (1, 0, dcgettext (((void *)0), \"invalid device "
                       "%s %s\", 5), s_major, s_minor), assume (false))"
                       ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0, dcgettext(((void *)0), "invalid device %s %s", 5),
                     s_major, s_minor),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0, dcgettext(((void *)0), "invalid device %s %s", 5),
                     s_major, s_minor),
    ...
    if (rpl_mknod(argv[optind], newmode | node_type, device) != 0)
      ((!!sizeof(struct {
        _Static_assert(
            1,
            "verify_expr ("
            ...
            "(0, shell_escape_quoting_style, argv[optind])), assume (false))"
            ")");
        int _gl_dummy;
      }))
  ...
  case 'p':
    if (set_security_context)
      defaultcon(set_security_context, argv[optind], 0010000);
  ...
  default:
    error(0, 0, dcgettext(((void *)0), "invalid device type %s", 5),
          quote(argv[optind + 1]));
    usage(1);
  }
  ...
  if (specified_mode && lchmod(argv[optind], newmode) != 0)
    ((!!sizeof(struct {
      _Static_assert(
          ...
          "set permissions of %s\", 5), quotearg_style "
          "(shell_escape_always_quoting_style, argv[optind])), assume (false))"
          ")");
      int _gl_dummy;
    }))
         ? ((error(1, (*__errno_location()),
                   dcgettext(((void *)0), "cannot set permissions of %s", 5),
