...
enum sync_mode { MODE_FILE, MODE_DATA, MODE_FILE_SYSTEM, MODE_SYNC };
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "or more files are specified, sync only them,\nor their "
    ...
    fputs_unlocked(dcgettext(((void *)0),
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "  -f, --file-system      sync the file systems "
...
sync_arg(enum sync_mode mode, char const *file) {
  ...
  if (fd < 0) {
    ...
    if (fd < 0) {
      error(0, rd_errno, dcgettext(((void *)0), "error opening %s", 5),
  ...
  if (fdflags == -1 || rpl_fcntl(fd, 4, fdflags & ~04000) < 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "couldn't reset non-blocking mode %s", 5),
          quotearg_style(shell_escape_always_quoting_style, file));
    ret = 0;
  }
  ...
  if (ret == 1) {
    ...
    if (sync_status < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "error syncing %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file));
      ret = 0;
    }
  }
  ...
  if (arg_data && arg_file_system) {
    ((!!sizeof(struct {
      _Static_assert(1,
                     ...
                     "1"
                     ", "
                     "(error (1, 0, dcgettext (((void *)0), \"cannot specify "
                     "both --data and --file-system\", 5)), assume (false))"
                     ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
  ...
  if (!args_specified && arg_data)
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"--data needs "
                        "at least one argument\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0), "--data needs at least one argument",
                             5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0), "--data needs at least one argument",
