...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
  ...
  algo_name_len = strlen("SHA512");
  if ((strncmp(s + i, "SHA512", algo_name_len) == 0)) {
    i += algo_name_len;
    if (s[i] == ' ')
      ++i;
    if (s[i] == '(') {
...
digest_file(char const *filename, int *binary, unsigned char *bin_result,
  ...
  *missing = 0;
  ...
  err = sha512_stream(fp, bin_result);
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "sha512sum", "GNU coreutils", Version,
  ...
  min_digest_line_length = ((512 / 4) + 1 + 1);
  digest_hex_bytes = (512 / 4);
