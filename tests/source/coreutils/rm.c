...
static char const *const interactive_args[] = {"never",    "no",     "none",
                                               "once",     "always", "yes",
                                               ((void *)0)
...
static enum interactive_type const interactive_types[] = {
    ...
    interactive_once,  interactive_always, interactive_always};
_Static_assert((sizeof(interactive_args) / sizeof *(interactive_args)) ==
                   (sizeof(interactive_types) / sizeof *(interactive_types)) +
                       1,
               "verify ("
               "ARRAY_CARDINALITY (interactive_args) == ARRAY_CARDINALITY "
               "(interactive_types) + 1"
               ")");
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg[0] == '-' && arg[1] && lstat(arg, &st) == 0) {
      fprintf(
          ...
          dcgettext(((void *)0), "Try '%s ./%s' to remove the file %s.\n", 5),
          argv[0], quotearg_n_style(1, shell_escape_quoting_style, arg),
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "mistakes\n      --interactive[=WHEN]  prompt according to "
                  "WHEN: never, once (-I), or\n                          "
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "dfirvIR", long_opts, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'd':
    ...
    case INTERACTIVE_OPTION: {
      ...
      if (optarg)
        i = ((interactive_types)[__xargmatch_internal(
            "--interactive", optarg, interactive_args,
            (void const *)(interactive_types), sizeof *(interactive_types),
            argmatch_die, 1)]);
      else
      ...
      if (!(strcmp(argv[optind - 1], "--no-preserve-root") == 0))
        ((!!sizeof(struct {
          _Static_assert(
              ...
              "1"
              ", "
              "(error (1, 0, dcgettext (((void *)0), \"you may not abbreviate "
              "the --no-preserve-root option\", 5)), assume (false))"
              ")");
          int _gl_dummy;
        }))
             ? ((error(
        ...
        if (strcmp(optarg, "all") == 0)
          ...
          ((!!sizeof(struct {
            _Static_assert(
                ...
                "1"
                ", "
                "(error (1, 0, dcgettext (((void *)0), \"unrecognized "
                "--preserve-root argument: %s\", 5), quotearg_style "
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "rm", "GNU coreutils", Version, ("Paul Rubin"),
                  ("David MacKenzie"), ("Richard M. Stallman"),
  ...
  ((void)sizeof((((status) == RM_OK || (status) == RM_USER_DECLINED ||
                  (status) == RM_ERROR))
                    ? 1
                    : 0),
   __extension__({
     if (((status) == RM_OK || (status) == RM_USER_DECLINED ||
          (status) == RM_ERROR))
       ;
     else
       __assert_fail("VALID_STATUS (status)", "src/rm.c", 371,
