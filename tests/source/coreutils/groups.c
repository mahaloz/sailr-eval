...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... [USERNAME]...\n", 5),
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0),
                  "Print group memberships for each USERNAME or, if no "
                  "USERNAME is specified, for\nthe current process (which may "
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "", longopts, ((void *)0))) != -1) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "groups", "GNU coreutils", Version,
                  ("David MacKenzie"), ("James Youngman"), (char *)((void *)0));
      exit(0);
      break;
      ;
    default:
  ...
  if (optind == argc) {
    ...
    for (; optind < argc; optind++) {
      ...
      if (pwd == ((void *)0)) {
        error(0, 0, dcgettext(((void *)0), "%s: no such user", 5),
              quote(argv[optind]));
        ok = 0;
        continue;
      }
