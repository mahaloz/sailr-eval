...
__attribute__((__pure__)) static int path_common_prefix(char const *path1,
                                                        char const *path2) {
  int i = 0;
  int ret = 0;
...
relpath (char const *can_fname, char const *can_reldir, char *buf, size_t len)
{
  ...
  if (*relto_suffix) {
    ...
    buf_err |= buffer_or_output(*fname_suffix ? fname_suffix : ".", &buf, &len);
  }

  if (buf_err)
    error(0, 36, "%s", dcgettext(((void *)0), "generating relative path", 5));
