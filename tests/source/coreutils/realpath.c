...
enum { RELATIVE_TO_OPTION = 0x7f + 1, RELATIVE_BASE_OPTION };
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "--relative-base=DIR      print absolute paths unless paths below "
            "DIR\n  -s, --strip, --no-symlinks   don't expand symlinks\n  -z, "
            "--zero                   end each output line with NUL, not "
            "newline\n",
            5),
        stdout);
    fputs_unlocked(dcgettext(((void *)0),
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             ...
             "(shell_escape_always_quoting_style, path)), assume (false))"
             ")");
      int _gl_dummy;
    }))
  ...
  if (!can_fname) {
    if (verbose)
      error(0, (*__errno_location()), "%s",
            quotearg_n_style_colon(0, shell_escape_quoting_style, fname));
    return 0;
  }
...
int main(int argc, char **argv) {
  ...
  char const *relative_to = ((void *)0);
  char const *relative_base = ((void *)0);
  ...
  while (1) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "realpath", "GNU coreutils", Version,
    ...
    if (!can_relative_to)
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "(0, shell_escape_quoting_style, relative_to)), assume (false))"
            ")");
        int _gl_dummy;
      }))
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 20, \"%s\", quotearg_n_style_colon (0, "
               "shell_escape_quoting_style, relative_to)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 20, "%s",
    ...
    if (!base)
      ((!!sizeof(struct {
        _Static_assert(
            ...
            ", "
            "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon "
            "(0, shell_escape_quoting_style, relative_base)), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(1, (*__errno_location()), "%s",
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               ...
               ", "
               "(error (1, 20, \"%s\", quotearg_n_style_colon (0, "
               "shell_escape_quoting_style, relative_base)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 20, "%s",
