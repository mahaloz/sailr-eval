  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "numbers (default: \\n)\n  -w, --equal-width        equalize "
                  "width by padding with leading zeroes\n",
                  5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "current number and\nINCREMENT would become greater than "
            "LAST.\nFIRST, INCREMENT, and LAST are interpreted as floating "
            "point values.\nINCREMENT is usually positive if FIRST is smaller "
            "than LAST, and\nINCREMENT is usually negative if FIRST is greater "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "LAST are all fixed point\ndecimal numbers with maximum "
...
static operand scan_arg(char const *arg) {
  ...
  if (!xstrtold(arg, ((void *)0), &ret.value, cl_strtold)) {
    error(0, 0,
          dcgettext(((void *)0), "invalid floating point argument: %s", 5),
          quote(arg));
    usage(1);
  }
  ...
  if (((ret.value) != (ret.value))) {
    error(0, 0, dcgettext(((void *)0), "invalid %s argument: %s", 5),
          quote_n(0, "not-a-number"), quote_n(1, arg));
    usage(1);
  }
...
static char const *long_double_format(char const *fmt, struct layout *layout) {
  ...
  i++;
  i += strspn (fmt + i, "-+  i += strspn (fmt + i, "0123456789");
  if (fmt[i] == '.')
    {
    i++;
  ...
  if (fmt[i] == '\0')
    ...
    _Static_assert(1, "verify_expr ("
                      "1"
                      ", "
                      "(error (1, 0, dcgettext (((void *)0), \"format %s ends "
                      "in %%\", 5), quote (fmt)), assume (false))"
                      ")");
    int _gl_dummy; })) ? ((error (
   1
   , 0, 
  ...
  if (! strchr ("efgaEFGA", fmt[i]))
    ...
    _Static_assert(
        1, "verify_expr ("
           "1"
           ", "
           "(error (1, 0, dcgettext (((void *)0), \"format %s has unknown %%%c "
           "directive\", 5), quote (fmt), fmt[i]), assume (false))"
           ")");
    int _gl_dummy; })) ? ((error (
   1
   , 0, 
  ...
  for (i++; ; i += (fmt[i] == '%') + 1)
    ...
    _Static_assert(1,
                   ...
                   "1"
                   ", "
                   "(error (1, 0, dcgettext (((void *)0), \"format %s has too "
                   "many %% directives\", 5), quote (fmt)), assume (false))"
                   ")");
    int _gl_dummy; })) ? ((error (
     1
     , 0, 
...
static void print_numbers(char const *fmt, struct layout layout,
                          long double first, long double step,
                          long double last) {
  ...
  if (!out_of_range) {
    ...
    for (i = 1;; i++) {
      ...
      if (out_of_range) {
        ...
        if (xstrtold(x_str + layout.prefix_len, ((void *)0), &x_val,
                     cl_strtold) &&
            x_val == last) {
          char *x0_str = ((void *)0);
...
static void incr(char **s0, size_t *s_len) {
  char *s = *s0;
  char *endp = s + *s_len - 1;
...
__attribute__((__pure__)) static char const *trim_leading_zeros(char const *s) {
  char const *p = s;
  while (*s == '0')
    ++s;
...
static void seq_fast(char const *a, char const *b, uintmax_t step) {
  ...
  size_t inc_size =
      ((((((p_len + 1) > (q_len)) ? (p_len + 1) : (q_len))) > (31))
           ? ((((p_len + 1) > (q_len)) ? (p_len + 1) : (q_len)))
           : (31));

  _Static_assert(3 < 31 - 1,
                 "verify ("
                 "SEQ_FAST_STEP_LIMIT_DIGITS < INITIAL_ALLOC_DIGITS - 1"
                 ")");
  ...
  if (ok) {
    ...
    size_t buf_size =
        (((8192) > ((inc_size + 1) * 2)) ? (8192) : ((inc_size + 1) * 2));
...
int main(int argc, char **argv) {
  ...
  while (optind < argc) {
    if (argv[optind][0] == '-' && ((optc = argv[optind][1]) == '.' ||
                                   ((unsigned int)(optc) - '0' <= 9))) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "seq", "GNU coreutils", Version, ("Ulrich Drepper"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (all_digits_p(argv[optind]) &&
      ...
      (n_args < 3 || (fast_step_ok && all_digits_p(argv[optind + 2]))) &&
  ...
  if (optind < argc) {
    ...
    if (optind < argc) {
      ...
      if (step.value == 0) {
        error(0, 0,
              dcgettext(((void *)0), "invalid Zero increment value: %s", 5),
              quote(argv[optind - 1]));
        usage(1);
      }
  ...
  if (first.precision == 0 && step.precision == 0 && last.precision == 0 &&
      ...
      0 < step.value && step.value <= 200 && !equal_width && !format_str &&
      strlen(separator) == 1) {
    char *s1;
    char *s2;
