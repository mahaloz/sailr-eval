...
static char *samedir_template(char const *dstname, char buf[smallsize]) {
...
extern int force_linkat(int srcdir, char const *srcname, int dstdir,
                        char const *dstname, int flags, _Bool force,
                        int linkat_errno) {
  if (linkat_errno < 0)
  ...
  if (try_tempname_len(dsttmp, 0, &arg, try_link, x_suffix_len) != 0)
    ...
    err = renameat(dstdir, dsttmp, dstdir, dstname) == 0
              ? -1
              : (*__errno_location());
...
extern int force_symlinkat(char const *srcname, int dstdir, char const *dstname,

                           _Bool force, int symlinkat_errno) {
  if (symlinkat_errno < 0)
    symlinkat_errno =
        symlinkat(srcname, dstdir, dstname) == 0 ? 0 : (*__errno_location());
