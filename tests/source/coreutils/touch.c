...
static char const *const time_args[] = {"atime",  "access",   "use", "mtime",
                                        "modify", ((void *)0)

};

static int const time_masks[] = {1, 1, 1, 2, 2};

static void get_reldate(struct timespec *result, char const *flex_date,
  ...
  if (!parse_datetime(result, flex_date, now))
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"invalid date "
                        "format %s\", 5), quote (flex_date)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0, dcgettext(((void *)0), "invalid date format %s", 5),
                   quote(flex_date)),
...
touch(char const *file) {
  ...
  if (change_times != (1 | 2)) {
    ...
    if (change_times == 2)
      ...
      ((void)sizeof((change_times == 1) ? 1 : 0), __extension__({
         if (change_times == 1)
           ;
         else
           __assert_fail("change_times == CH_ATIME", "src/touch.c", 147,
  ...
  char const *file_opt = fd == 1 ? ((void *)0) : file;
  int atflag = no_dereference ? 0x100 : 0;
  int utime_errno =
      (fdutimensat(fd, -100, file_opt, t, atflag) == 0 ? 0
                                                       : (*__errno_location()));
  ...
  if (utime_errno != 0) {
    ...
    if (open_errno && !(open_errno == 21
      ...
      error(0, open_errno, dcgettext(((void *)0), "cannot touch %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file));
    } else {
      if (no_create && utime_errno == 2)
        return 1;
      error(0, utime_errno, dcgettext(((void *)0), "setting times of %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file));
    }
    return 0;
  }
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "--date=STRING      parse STRING and use it instead of "
                  "current time\n  -f                     (ignored)\n",
                  5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -h, --no-dereference   affect each symbolic link instead "
                  "of any referenced\n                         file (useful "
                  "only on systems that can change the\n                       "
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "acd:fhmr:t:", longopts, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'a':
      change_times |= 1;
    ...
    case 't':
      if (!posixtime(&newtime[0].tv_sec, optarg, 0 | 2 | 4))
        ((!!sizeof(struct {
          _Static_assert(1,
                         "verify_expr ("
                         "1"
                         ", "
                         "(error (1, 0, dcgettext (((void *)0), \"invalid date "
                         "format %s\", 5), quote (optarg)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "invalid date format %s", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "invalid date format %s", 5),
    ...
    case TIME_OPTION:
      change_times |= ((time_masks)[__xargmatch_internal(
          "--time", optarg, time_args, (void const *)(time_masks),
          sizeof *(time_masks), argmatch_die, 1)]);
      break;
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "touch", "GNU coreutils", Version, ("Paul Rubin"),
                  ("Arnold Robbins"), ("Jim Kingdon"), ("David MacKenzie"),
                  ("Randy Smith"), (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (date_set && (use_ref || flex_date)) {
    error(0, 0,
          dcgettext(((void *)0),
                    "cannot specify times from more than one source", 5));
    usage(1);
  }
  ...
  if (use_ref) {
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               ...
               "(shell_escape_always_quoting_style, ref_file)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(
    ...
    if (flex_date) {
      ...
      if (change_times == (1 | 2) && newtime[0].tv_sec == now.tv_sec &&
  ...
  if (!date_set && 2 <= argc - optind && posix2_version() < 200112 &&
      posixtime(&newtime[0].tv_sec, argv[optind], 1 | 8)) {
    newtime[0].tv_nsec = 0;
    newtime[1] = newtime[0];
    date_set = 1;
      ...
      if (tm)
        error(0, 0,
              ...
              dcgettext(((void *)0),
                  ...
                  ,
              argv[optind], tm->tm_year + 1900L, tm->tm_mon + 1, tm->tm_mday,
