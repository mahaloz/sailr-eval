...
static int compute_context_from_mask(char const *context, context_t *ret) {
  ...
  if (!new_context) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "failed to create security context: %s", 5),
          quote(context));
    return 1;
  }
  do {
    if (specified_user && context_user_set((new_context), specified_user)) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0),
                      "failed to set %s security context component to %s", 5),
            "user", quote(specified_user));
      ok = 0;
    }
  } while (0);
  do {
    if (specified_range && context_range_set((new_context), specified_range)) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0),
                      "failed to set %s security context component to %s", 5),
            "range", quote(specified_range));
      ok = 0;
    }
  } while (0);
  do {
    if (specified_role && context_role_set((new_context), specified_role)) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0),
                      "failed to set %s security context component to %s", 5),
            "role", quote(specified_role));
      ok = 0;
    }
  } while (0);
  do {
    if (specified_type && context_type_set((new_context), specified_type)) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0),
                      "failed to set %s security context component to %s", 5),
            "type", quote(specified_type));
      ok = 0;
    }
  } while (0);
...
static int change_file_context(int fd, char const *file) {
  ...
  if (specified_context == ((void *)0)) {
    int status =
        (affect_symlink_referent ? getfileconat(fd, file, &file_context)
    ...
    if (file_context == ((void *)0)) {
      error(0, 0,
            dcgettext(((void *)0),
                      "can't apply partial context to unlabeled file %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file));
      return 1;
    }
  ...
  if (file_context == ((void *)0) ||
    ...
    int fail =
        (affect_symlink_referent ? setfileconat(fd, file, context_string)
                                 : lsetfileconat(fd, file, context_string));

    if (fail) {
      errors = 1;
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to change context of %s to %s", 5),
...
process_file(FTS *fts, FTSENT *ent) {
  ...
  if (ok) {
    if (verbose)
      printf(dcgettext(((void *)0), "changing security context of %s\n", 5),
             quotearg_style(shell_escape_always_quoting_style, file_full_name));
  ...
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0),
                  ...
                  "[OPTION]... [-u USER] [-r ROLE] [-l RANGE] [-t TYPE] "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "CONTEXT.\nWith --reference, change the security context of "
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "  -u, --user=USER        set user USER in the target security "
            "context\n  -r, --role=ROLE        set role ROLE in the target "
            "security context\n  -t, --type=TYPE        set type TYPE in the "
            "target security context\n  -l, --range=RANGE      set range RANGE "
...
int main(int argc, char **argv) {
      ...
      if (dereference == 0)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"-R -h "
                            "requires -P\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0, dcgettext(((void *)0), "-R -h requires -P", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0, dcgettext(((void *)0), "-R -h requires -P", 5)),
                 ((0) ? (void)0 : __builtin_unreachable()))));
    ...
    if (0 < is_selinux_enabled() &&
      ...
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          ...
                          "(((void *)0), \"invalid context: %s\", 5), quote "
                          "(specified_context)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, (*__errno_location()),
  ...
  if (reference_file && component_specified) {
    error(0, 0,
          dcgettext(((void *)0),
                    "conflicting security context specifiers given", 5));
    usage(1);
  }
