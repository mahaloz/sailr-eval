...
extern char *find_mount_point(char const *file, struct stat const *file_stat) {
  ...
  if (save_cwd(&cwd) != 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "cannot get current directory", 5));
    return ((void *)0);
  }
...

  {
    ...
    if (chdir(file) < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "cannot change to directory %s", 5),
...

  {
    ...
    if (stat(".", &last_stat) < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "cannot stat current directory (now %s)", 5),
            quotearg_style(shell_escape_always_quoting_style, dir));
      goto done;
    }
  }
  ...
  if (restore_cwd(&cwd) != 0)
    ((!!sizeof(struct {
      _Static_assert(
          ...
          "to return to initial working directory\", 5)), assume (false))"
          ")");
      int _gl_dummy;
    }))
