...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             "\n  -r              use BSD sum algorithm (the "
                             "default), use 1K blocks\n  -s, --sysv      use "
                             "System V sum algorithm, use 512 bytes blocks\n",
                             5),
                   stdout)
...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
  ...
  algo_name_len = strlen("BSD");
  if ((strncmp(s + i, "BSD", algo_name_len) == 0)) {
    i += algo_name_len;
    if (s[i] == ' ')
      ++i;
    if (s[i] == '(') {
...
digest_file(char const *filename, int *binary, unsigned char *bin_result,
  ...
  *missing = 0;
  ...
  err = sumfns[sum_algorithm](fp, bin_result, length);
...
digest_check(char const *checkfile_name) {
  ...
  unsigned char bin_buffer_unaligned[(16 / 8) + 4];
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "sum", "GNU coreutils", Version,
                  ("Kayvan Aghaiepour"), ("David MacKenzie"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  min_digest_line_length = ((16 / 4) + 1 + 1);
  digest_hex_bytes = (16 / 4);
  ...
  for (char **operandp = argv + optind; operandp < operand_lim; operandp++) {
    ...
    if (do_check)
      ...
      if (!digest_file(file, &binary_file, bin_buffer, &missing, &length))
        ...
        sum_output_fns[sum_algorithm](file, binary_file, bin_buffer, prefix_tag,
