  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "HHHH (4 digits)\n  \\UHHHHHHHH  Unicode character with hex "
                  "value HHHHHHHH (8 digits)\n",
                  5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "escapes interpreted,\n          except that octal escapes are of "
            "the form \\0 or \\0NNN\n  %q      ARGUMENT is printed in a format "
            "that can be reused as shell input,\n          escaping "
            "non-printable characters with the proposed POSIX $'' "
...
static void verify_numeric(char const *s, char const *end) {
  if ((*__errno_location())) {
    ...
    if (s == end)
      error(0, 0, dcgettext(((void *)0), "%s: expected a numeric value", 5),
            quote(s));
    else
      error(0, 0,
            dcgettext(((void *)0), "%s: value not completely converted", 5),
            quote(s));
    exit_status = 1;
  }
}
static intmax_t vstrtoimax(char const *s) {
  ...
  if ((*s == '\"' || *s == '\'') && *(s + 1)) {
    ...
    if ((__ctype_get_mb_cur_max()) > 1 && *(s + 1)) {
      ...
      ssize_t bytes;
      bytes = rpl_mbrtowc(&wc, s, slen, &mbstate);
      if (0 < bytes) {
        val = wc;
        s += bytes - 1;
      }
    ...
    if (*++s != 0 && !posixly_correct)
      error(0, 0, dcgettext(((void *)0), cfcc_msg, 5), s);
  } else {
    (*__errno_location()) = 0;
...
static int print_esc(char const *escstart, _Bool octal_0) {
  ...
  if (*p == 'x') {
    ...
    for (esc_length = 0, ++p;
         esc_length < 2 && ((*__ctype_b_loc())[(int)((to_uchar(*p)))] &
                            (unsigned short int)_ISxdigit);
         ++esc_length, ++p)
      esc_value =
          esc_value * 16 + ((*p) >= 'a' && (*p) <= 'f'   ? (*p) - 'a' + 10
                            : (*p) >= 'A' && (*p) <= 'F' ? (*p) - 'A' + 10
                                                         : (*p) - '0');
    if (esc_length == 0)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          ...
                          "hexadecimal number in escape\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0),
                               "missing hexadecimal number in escape", 5)),
    ...
    for (esc_length = 0, p += octal_0 && *p == '0';
         esc_length < 3 && ((*p) >= '0' && (*p) <= '7'); ++esc_length, ++p)
    ...
    uni_value = 0;
    for (esc_length = (esc_char == 'u' ? 4 : 8), ++p; esc_length > 0;
         --esc_length, ++p) {
        ...
        ((!!sizeof(struct {
          ...
          int _gl_dummy;
             ...
             : ((error(1, 0,
                 ...
                 ((0) ? (void)0 : __builtin_unreachable()))));
      uni_value =
          uni_value * 16 + ((*p) >= 'a' && (*p) <= 'f'   ? (*p) - 'a' + 10
                            : (*p) >= 'A' && (*p) <= 'F' ? (*p) - 'A' + 10
                                                         : (*p) - '0');
    }

    if ((uni_value <= 0x9f && uni_value != 0x24 && uni_value != 0x40 &&
         uni_value != 0x60) ||
      ...
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       "universal character name \\\\%c%0*x\", 5), esc_char, "
                       "(esc_char == 'u' ? 4 : 8), uni_value), assume (false))"
                       ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0),
                               "invalid universal character name \\%c%0*x", 5),
...
static void print_direc(char const *start, size_t length, char conversion,
...
static int print_formatted(char const *format, int argc, char **argv) {
  ...
  for (f = format; *f; ++f) {
    ...
    case '%':
      ...
      if (*f == 'q') {
        if (argc > 0) {
          fputs_unlocked(quotearg_style(shell_escape_quoting_style, *argv),
                         stdout);
          ++argv;
          --argc;
        }
      ...
      ok['a'] = ok['A'] = ok['c'] = ok['d'] = ok['e'] = ok['E'] = ok['f'] =
          ok['F'] = ok['g'] = ok['G'] = ok['i'] = ok['o'] = ok['s'] = ok['u'] =
              ok['x'] = ok['X'] = 1;
      ...
      for (;; f++, direc_length++)
        ...
        case ' ':
          break;
        case '                ok[' c '] = ok[' d '] = ok[' i '] = ok[' s '] = ok['u'] = 0; break;
            case '0':
          ok['c'] = ok['s'] = 0;
          break;
        default:
    ...
    no_more_flag_characters:
      ...
      if (*f == '*') {
        ...
        if (argc > 0) {
          ...
          if ((-0x7fffffff - 1) <= width && width <= 0x7fffffff)
            field_width = width;
          else
            ((!!sizeof(struct {
              _Static_assert(
                  1, "verify_expr ("
                     ...
                     "width: %s\", 5), quote (*argv)), assume (false))"
                     ")");
              int _gl_dummy;
            }))
                 ? ((error(1, 0,
                           dcgettext(((void *)0), "invalid field width: %s", 5),
                           quote(*argv)),
                     ((0) ? (void)0 : __builtin_unreachable())))
                 : ((error(1, 0,
                           dcgettext(((void *)0), "invalid field width: %s", 5),
              ...
              ((!!sizeof(struct {
                _Static_assert(
                    1, "verify_expr ("
                       ...
                       ", "
                       "(error (1, 0, dcgettext (((void *)0), \"invalid "
                       "precision: %s\", 5), quote (*argv)), assume (false))"
                       ")");
                int _gl_dummy;
              }))
                   ? ((error(1, 0,
                             dcgettext(((void *)0), "invalid precision: %s", 5),
                             quote(*argv)),
                       ((0) ? (void)0 : __builtin_unreachable())))
                   : ((error(1, 0,
                             dcgettext(((void *)0), "invalid precision: %s", 5),
        ...
        if (!ok[conversion])
          ((!!sizeof(struct {
            _Static_assert(1,
                           ...
                           "invalid conversion specification\", 5), (int) (f + "
                           "1 - direc_start), direc_start), assume (false))"
                           ")");
            int _gl_dummy;
          }))
               ? ((error(1, 0,
                         dcgettext(((void *)0),
                                   "%.*s: invalid conversion specification", 5),
...
int main(int argc, char **argv) {
  ...
  if (argc == 2) {
    ...
    if ((strcmp(argv[1], "--version") == 0)) {
      version_etc(stdout, "printf", "GNU coreutils", Version,
  ...
  if (argc > 0)
    error(0, 0,
          ...
          dcgettext(((void *)0),
                    "warning: ignoring excess arguments, starting with %s", 5),
          quote(argv[0]));
