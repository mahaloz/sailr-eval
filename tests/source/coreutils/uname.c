...
void usage(int status) {
  if (status != 0)
    ...
    if (uname_mode == 1) {
      ...
      fputs_unlocked(
          dcgettext(((void *)0), "Print machine architecture.\n\n", 5), stdout)
...
static void print_element_env(char const *element, char const *envvar) {
...
static int decode_switches(int argc, char **argv) {
  ...
  if (uname_mode == 2) {
    while ((c = getopt_long(argc, argv, "", arch_long_options, ((void *)0))) !=
           -1) {
      switch (c) {
      ...
      case GETOPT_VERSION_CHAR:
        version_etc(stdout, (uname_mode == 1 ? "uname" : "arch"),
                    "GNU coreutils", Version, "David MacKenzie", "Karel Zak",
                    (char *)((void *)0));
        exit(0);
        break;
        ;
    ...
    while ((c = getopt_long(argc, argv, "asnrvmpio", uname_long_options,
                            ((void *)0))) != -1) {
      switch (c) {
      case 'a':
        toprint = (0x7fffffff * 2U + 1U);
        break;
      ...
      case GETOPT_VERSION_CHAR:
        version_etc(stdout, (uname_mode == 1 ? "uname" : "arch"),
                    "GNU coreutils", Version, ("David MacKenzie"),
                    (char *)((void *)0));
        exit(0);
        break;
        ;
    ...
    if (uname(&name) == -1)
      ((!!sizeof(struct {
        _Static_assert(1,
                       ...
                       "*)0), \"cannot get system name\", 5)), assume (false))"
                       ")");
        int _gl_dummy;
      }))
           ? ((error(1, (*__errno_location()),
                     dcgettext(((void *)0), "cannot get system name", 5)),
