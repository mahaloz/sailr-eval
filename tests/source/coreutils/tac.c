...
static char compiled_separator_fastmap[(0x7f * 2 + 1) + 1];
...
static void output(char const *start, char const *past_end) {
...
tac_seekable(int input_fd, char const *file, off_t file_pos) {
  ...
  if (remainder != 0) {
    ...
    if (lseek(input_fd, file_pos, 0) < 0)
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "%s: seek failed", 5),
  ...
  if (saved_record_size == ((size_t)-1)) {
    error(0, (*__errno_location()), dcgettext(((void *)0), "%s: read error", 5),
          quotearg_n_style_colon(0, shell_escape_quoting_style, file));
    return 0;
  }
  ...
  while (1) {
    ...
    if (sentinel_length == 0) {
      ...
      if (1 < range)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"record "
                            "too large\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0, dcgettext(((void *)0), "record too large", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0, dcgettext(((void *)0), "record too large", 5)),
                 ((0) ? (void)0 : __builtin_unreachable()))));

      if (range == 1 || ((ret = re_search(&compiled_separator, G_buffer, i,
                                          i - 1, range, &regs)) == -1))
        ...
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"error in "
                            "regular expression search\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
      ...
      while (*--match_start != first_char ||
             (match_length1 &&
              !(strncmp(match_start + 1, separator1, match_length1) == 0)))
        ;
    }
...
static void record_or_unlink_tempfile(char const *fn, FILE *fp) { unlink(fn); }
...
temp_stream(FILE **fp, char **file_name) {
  ...
  if (tempfile == ((void *)0)) {
    ...
    if (tempdir == ((void *)0)) {
      error(0, 0, dcgettext(((void *)0), "memory exhausted", 5));
      return 0;
    }
    int fd = mkstemp_safer(tempfile);
    if (fd < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to create temporary file in %s", 5),
    ...
    if (!tmp_fp) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to open %s for writing", 5),
    ...
    if (rpl_fseeko(tmp_fp, 0, 0) < 0 || ftruncate(fileno(tmp_fp), 0) < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to rewind stream for %s", 5),
            quotearg_style(shell_escape_always_quoting_style, tempfile));
      return 0;
    }
  }
...
static off_t copy_to_temp(FILE **g_tmp, char **g_tempfile, int input_fd,
  ...
  while (1) {
    ...
    if (fwrite_unlocked(G_buffer, 1, bytes_read, fp) != bytes_read) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "%s: write error", 5),
            quotearg_n_style_colon(0, shell_escape_quoting_style, file_name));
      return -1;
    }
...
tac_file(char const *filename) {
  ...
  if (is_stdin) {
    have_read_stdin = 1;
    fd = 0;
    filename = dcgettext(((void *)0), "standard input", 5);
    xset_binary_mode(0, 0);
  } else {
    fd = open(filename, 00 | 0);
    if (fd < 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to open %s for reading", 5),
  ...
  ok = (file_size < 0 || isatty(fd) ? tac_nonseekable(fd, filename)
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "brs:", longopts, ((void *)0))) !=
         -1) {
    switch (optc) {
    case 'b':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "tac", "GNU coreutils", Version, ("Jay Lepreau"),
  ...
  if (sentinel_length == 0) {
    if (*separator == 0)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          "1"
                          ", "
                          "(error (1, 0, dcgettext (((void *)0), \"separator "
                          "cannot be empty\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0), "separator cannot be empty", 5)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0,
                     dcgettext(((void *)0), "separator cannot be empty", 5)),
               ((0) ? (void)0 : __builtin_unreachable()))));
    ...
    error_message =
        re_compile_pattern(separator, strlen(separator), &compiled_separator);
    if (error_message)
      ((!!sizeof(struct {
        _Static_assert(1,
                       "verify_expr ("
                       "1"
                       ", "
                       "(error (1, 0, \"%s\", (error_message)), assume (false))"
                       ")");
        int _gl_dummy;
      }))
  ...
  file =
      (optind < argc ? (char const *const *)&argv[optind] : default_file_list);
