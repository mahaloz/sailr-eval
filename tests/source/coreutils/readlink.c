...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "  -f, --canonicalize            canonicalize by following every "
            "symlink in\n                                every component of "
            "the given name recursively;\n                                all "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            " -s, --silent                  suppress most error messages (on "
            "by default)\n  -v, --verbose                 report error "
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "efmnqsvz", longopts, ((void *)0))) !=
         -1) {
    switch (optc) {
    case 'e':
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "readlink", "GNU coreutils", Version,
                  ("Dmitry V. Levin"), (char *)((void *)0));
      exit(0);
      break;
      ;
    default:
  ...
  if (argc - optind > 1) {
    if (no_newline)
      error(0, 0,
            dcgettext(((void *)0),
                      "ignoring --no-newline with multiple arguments", 5));
    no_newline = 0;
  }

  for (; optind < argc; ++optind) {
    char const *fname = argv[optind];
    char *value = (can_mode != -1 ? canonicalize_filename_mode(fname, can_mode)
