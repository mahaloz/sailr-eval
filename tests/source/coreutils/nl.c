...
uintmax_t xnumtoumax(char const *n_str, int base, uintmax_t min, uintmax_t max,
                     char const *suffixes, char const *err, int err_exit)
    __attribute__((__nonnull__(1, 6)));
static char const FORMAT_RIGHT_NOLZ[] = "%*"
                                        "l"
                                        "d"
                                        "%s";

static char const FORMAT_RIGHT_LZ[] = "%0*"
                                      "l"
                                      "d"
                                      "%s";

static char const FORMAT_LEFT[] = "%-*"
                                  "l"
                                  "d"
                                  "%s";
...
static char body_fastmap[(0x7f * 2 + 1) + 1];
static char header_fastmap[(0x7f * 2 + 1) + 1];
static char footer_fastmap[(0x7f * 2 + 1) + 1];
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -b, --body-numbering=STYLE      use STYLE for numbering "
                  "body lines\n  -d, --section-delimiter=CC      use CC for "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -v, --starting-line-number=NUMBER  first line number for "
                  "each section\n  -w, --number-width=NUMBER       use NUMBER "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "-s<TAB> -v1 -w6\n\nCC are two delimiter characters used to "
                  "construct logical page delimiters;\na missing second "
                  "character implies ':'.  As a GNU extension one can "
...
build_type_arg(char const **typep, struct re_pattern_buffer *regexp,
               char *fastmap) {
  char const *errmsg;
    ...
    if (errmsg)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          "1"
                          ", "
                          "(error (1, 0, \"%s\", (errmsg)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0, "%s", (errmsg)),
  ...
  if (line_no_overflow)
    ((!!sizeof(struct {
      _Static_assert(1, "verify_expr ("
                        "1"
                        ", "
                        "(error (1, 0, dcgettext (((void *)0), \"line number "
                        "overflow\", 5)), assume (false))"
                        ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0, dcgettext(((void *)0), "line number overflow", 5)),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0, dcgettext(((void *)0), "line number overflow", 5)),
             ((0) ? (void)0 : __builtin_unreachable()))));
...
static void proc_text(void) {
  ...
  case 'n':
    ...
    break;
  case 'p':
    switch (re_search(current_regex, line_buf.buffer, line_buf.length - 1, 0,
                      line_buf.length - 1, ((void *)0))) {
    case -2:
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               ...
               "\"error in regular expression search\", 5)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, (*__errno_location()),
                     dcgettext(((void *)0),
                               "error in regular expression search", 5)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, (*__errno_location()),
                     dcgettext(((void *)0),
                               "error in regular expression search", 5)),
...
nl_file(char const *file) {
  ...
  else if (rpl_fclose(stream) != 0 && !err)
  ...
  while ((c = getopt_long(argc, argv, "h:b:f:v:i:pl:s:w:n:d:", longopts,
    ...
    case 'h':
      if (!build_type_arg(&header_type, &header_regex, header_fastmap)) {
        error(0, 0,
              dcgettext(((void *)0), "invalid header numbering style: %s", 5),
              quote(optarg));
        ok = 0;
      }
      break;
    case 'b':
      if (!build_type_arg(&body_type, &body_regex, body_fastmap)) {
        error(0, 0,
              dcgettext(((void *)0), "invalid body numbering style: %s", 5),
              quote(optarg));
        ok = 0;
      }
      break;
    case 'f':
      if (!build_type_arg(&footer_type, &footer_regex, footer_fastmap)) {
        error(0, 0,
              dcgettext(((void *)0), "invalid footer numbering style: %s", 5),
              quote(optarg));
        ok = 0;
      }
      break;
    case 'v':
      starting_line_number = xdectoimax(
          optarg, (-9223372036854775807L - 1), (9223372036854775807L), "",

          dcgettext(((void *)0), "invalid starting line number", 5), 0);
      break;
    case 'i':
      page_incr = xdectoimax(
          optarg, (-9223372036854775807L - 1), (9223372036854775807L), "",

          dcgettext(((void *)0), "invalid line number increment", 5), 0);
      break;
    case 'p':
      reset_numbers = 0;
      break;
    case 'l':
      blank_join = xdectoimax(
          optarg, 1, (9223372036854775807L), "",

          dcgettext(((void *)0), "invalid line number of blank lines", 5), 0);
      break;
    case 's':
    ...
    case 'w':
      lineno_width = xdectoimax(
          ...
          dcgettext(((void *)0), "invalid line number field width", 5), 0);
      break;
    case 'n':
      if ((strcmp(optarg, "ln") == 0))
        ...
        error(0, 0,
              dcgettext(((void *)0), "invalid line numbering format: %s", 5),
              quote(optarg));
        ok = 0;
      }
      break;
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "nl", "GNU coreutils", Version, ("Scott Bartram"),
