...
enum Ternary { T_UNKNOWN = 2, T_NO, T_YES };
typedef enum Ternary Ternary;

enum Prompt_action { PA_DESCEND_INTO_DIR = 2, PA_REMOVE_DIR };
...
static inline struct stat *cache_stat_init(struct stat *st) {
  st->st_size = -1;
  return st;
}
...
static enum RM_status prompt(FTS const *fts, FTSENT const *ent, _Bool is_dir,
  ...
  if (write_protected || x->interactive == RMI_ALWAYS) {
    ...
    char const *quoted_name =
        quotearg_style(shell_escape_always_quoting_style, full_name);

    if (write_protected < 0) {
      error(0, wp_errno, dcgettext(((void *)0), "cannot remove %s", 5),
            quoted_name);
      return RM_ERROR;
    }
    ...
    if (dirent_type == DT_DIR
        ...
        && mode == PA_DESCEND_INTO_DIR && !is_empty)
      fprintf(
          stderr,
          (write_protected
               ? dcgettext(((void *)0),
                           "%s: descend into write-protected directory %s? ", 5)

               : dcgettext(((void *)0), "%s: descend into directory %s? ", 5)),
          program_name, quoted_name);
    else {
      if (cache_fstatat(fd_cwd, filename, sbuf, 0x100) != 0) {
        error(0, (*__errno_location()),
              dcgettext(((void *)0), "cannot remove %s", 5), quoted_name);
        return RM_ERROR;
      }
      ...
      fprintf(
          ...
          (write_protected
               ...
               ? dcgettext(((void *)0), "%s: remove write-protected %s %s? ", 5)

               : dcgettext(((void *)0), "%s: remove %s %s? ", 5)),
...
static enum RM_status excise(FTS *fts, FTSENT *ent, struct rm_options const *x,
  ...
  if (unlinkat(fts->fts_cwd_fd, ent->fts_accpath, flag) == 0) {
    if (x->verbose) {
      printf((is_dir ? dcgettext(((void *)0), "removed directory %s\n", 5)

                     : dcgettext(((void *)0), "removed %s\n", 5)),
  ...
  error(0, (*__errno_location()), dcgettext(((void *)0), "cannot remove %s", 5),
        quotearg_style(shell_escape_always_quoting_style, ent->fts_path));
...
static enum RM_status rm_fts(FTS *fts, FTSENT *ent,
  ...
  case 1:
    if (!x->recursive && !(x->remove_empty_directories &&
      ...
      error(0, err, dcgettext(((void *)0), "cannot remove %s", 5),
            quotearg_style(shell_escape_always_quoting_style, ent->fts_path));
      mark_ancestor_dirs(ent);
    ...
    if (ent->fts_level == 0) {
      ...
      if (dot_or_dotdot(last_component(ent->fts_accpath))) {
        error(0, 0,
              ...
              quotearg_n_style(2, shell_escape_always_quoting_style,
                               ent->fts_path));
        fts_skip_tree(fts, ent);
        return RM_ERROR;
      }
      ...
      if (x->preserve_all_root) {
        ...
        if (!parent || lstat(parent, &statbuf)) {
          error(0, 0,
                ...
                dcgettext(((void *)0), "failed to stat %s: skipping %s", 5),
        ...
        if (failed || fts->fts_dev != statbuf.st_dev) {
          if (!failed) {
            error(0, 0,
                  ...
                  dcgettext(((void *)0),
                            "skipping %s, since it's on a different device", 5),
                  quotearg_style(shell_escape_always_quoting_style,
                                 ent->fts_path));
            error(0, 0,
                  dcgettext(((void *)0), "and --preserve-root=all is in effect",
                            5));
          }
          fts_skip_tree(fts, ent);
          return RM_ERROR;
...

    {
      ...
      enum RM_status s =
          prompt(fts, ent, 1, x, PA_DESCEND_INTO_DIR, &is_empty_directory);
  ...
  case 3: {
    ...
    if (ent->fts_info == 6 && x->one_file_system && 0 < ent->fts_level &&
  ...
  case 7:
    ...
    error(0, ent->fts_errno, dcgettext(((void *)0), "traversal failed: %s", 5),
          quotearg_n_style_colon(0, shell_escape_quoting_style, ent->fts_path));
    fts_skip_tree(fts, ent);
    return RM_ERROR;
