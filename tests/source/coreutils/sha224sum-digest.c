...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
  ...
  algo_name_len = strlen("SHA224");
  if ((strncmp(s + i, "SHA224", algo_name_len) == 0)) {
    i += algo_name_len;
    if (s[i] == ' ')
      ++i;
    if (s[i] == '(') {
...
digest_file(char const *filename, int *binary, unsigned char *bin_result,
  ...
  *missing = 0;
  ...
  err = sha224_stream(fp, bin_result);
...
digest_check(char const *checkfile_name) {
  ...
  unsigned char bin_buffer_unaligned[(224 / 8) + 4];
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "sha224sum", "GNU coreutils", Version,
  ...
  min_digest_line_length = ((224 / 4) + 1 + 1);
  digest_hex_bytes = (224 / 4);
