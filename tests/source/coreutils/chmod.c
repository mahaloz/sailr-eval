...
enum { NO_PRESERVE_ROOT = 0x7f + 1, PRESERVE_ROOT, REFERENCE_FILE_OPTION };
  ...
  if (new_mode & (04000 | 02000 | 01000)) {
    ...
    if (fstatat(dir_fd, file, &new_stats, 0) != 0) {
      if (!force_silent)
        error(
            0, (*__errno_location()),
            dcgettext(((void *)0), "getting new attributes of %s", 5),
...
static void describe_change(char const *file, struct change_status const *ch) {
  char perms[12];
  char old_perms[12];
  char const *fmt;
  char const *quoted_file =
  ...
  case CH_NO_STAT:
    printf(dcgettext(((void *)0), "%s could not be accessed\n", 5),
           quoted_file);
    return;
  ...
  unsigned long int old_m = ch->old_mode &
                            (04000 | 02000 | 01000 | (0400 | 0200 | 0100) |
                             ((0400 | 0200 | 0100) >> 3) |
                             (((0400 | 0200 | 0100) >> 3) >> 3)),
                    m = ch->new_mode &
  ...
  case CH_NO_CHANGE_REQUESTED:
    fmt = dcgettext(((void *)0), "mode of %s retained as %04lo (%s)\n", 5);
...
process_file(FTS *fts, FTSENT *ent) {
  ...
  case 13:
    if (!force_silent)
      error(0, 0,
            dcgettext(((void *)0), "cannot operate on dangling symlink %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file_full_name));
    break;
  ...
  if (ch.status == CH_NOT_APPLIED &&
    ...
    ch.new_mode = mode_adjust(ch.old_mode,
                              ((((ch.old_mode)) & 0170000) == (0040000)) != 0,
                              umask_value, change, ((void *)0));
    if (chmodat(fts->fts_cwd_fd, file, ch.new_mode) == 0)
      ...
      if (!force_silent)
        error(
            0, (*__errno_location()),
            dcgettext(((void *)0), "changing permissions of %s", 5),
            quotearg_style(shell_escape_always_quoting_style, file_full_name));
      ch.status = CH_FAILED;
    }
  }
  ...
  if (verbosity != V_off) {
    if (ch.status == CH_SUCCEEDED &&
        !mode_changed(fts->fts_cwd_fd, file, file_full_name, ch.old_mode,
                      ch.new_mode))
  ...
  if (CH_NO_CHANGE_REQUESTED <= ch.status && diagnose_surprises) {
    mode_t naively_expected_mode = mode_adjust(
        ch.old_mode, ((((ch.old_mode)) & 0170000) == (0040000)) != 0, 0, change,
        ((void *)0));
    if (ch.new_mode & ~naively_expected_mode) {
      ...
      error(
          ...
          dcgettext(((void *)0), "%s: new permissions are %s, not %s", 5),
...
int main(int argc, char **argv) {
  ...
  if (reference_file) {
    if (mode) {
      error(0, 0,
            dcgettext(((void *)0),
                      "cannot combine mode and --reference options", 5));
      usage(1);
    }
  } else {
    if (!mode)
  ...
  if (reference_file) {
    ...
    if (!change) {
      error(0, 0, dcgettext(((void *)0), "invalid mode: %s", 5), quote(mode));
      usage(1);
    }
    umask_value = umask(0);
  }
