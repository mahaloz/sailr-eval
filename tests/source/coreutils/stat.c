...
extern char *xasprintf(const char *format, ...) __attribute__((__malloc__))
__attribute__((__malloc__(free, 1)))
...
static char const printf_flags[] = "'-+ 
...
enum { PRINTF_OPTION = 0x7f + 1 };

enum cached_mode { cached_default, cached_never, cached_always };

static char const *const cached_args[] = {"default", "never", "always",
                                          ((void *)0)

};

static enum cached_mode const cached_modes[] = {cached_default, cached_never,
                                                cached_always};
...
print_stat(char *pformat, size_t prefix_len, char mod, char m, int fd,
...
static int out_int(char *pformat, size_t prefix_len, intmax_t arg) {
  make_format(pformat, prefix_len, "'-+ 0",
              "l"
              "d");
...
static void out_uint_o(char *pformat, size_t prefix_len, uintmax_t arg) {
  make_format(pformat, prefix_len,
              "-                                          " l " " o "
  );
  printf(pformat, arg);
}
...
static void out_epoch_sec(char *pformat, size_t prefix_len,
  ...
  if ((!((time_t)0 < (time_t)-1))) {
    ...
    int_len = (minus_zero ? out_minus_zero(pformat, sec_prefix_len)
  ...
  if (precision) {
    ...
    int trailing_width = (ilen < width && decimal_point_len < width - ilen
                              ...
                              : 0);
    printf("%s%.*d%-*.*d", decimal_point, prec, frac_sec, trailing_width,
           trailing_prec, 0);
  }
}
...
out_file_context(char *pformat, size_t prefix_len, char const *filename) {
  ...
  if ((follow_links ? rpl_getfilecon(filename, &scontext)
                    : rpl_lgetfilecon(filename, &scontext)) < 0) {
...
print_statfs(char *pformat, size_t prefix_len, char mod, char m, int fd,
             char const *filename, void const *data) {
...
static char const *find_bind_mount(char const *name) {
  ...
  if (!tried_mount_list) {
    if (!(mount_list = read_file_system_list(0)))
      error(0, (*__errno_location()), "%s",
            dcgettext(((void *)0), "cannot read table of mounted file systems",
                      5));
    tried_mount_list = 1;
  }
...
out_mount_point(char const *filename, char *pformat, size_t prefix_len,
  ...
  if (follow_links || !((((statp->st_mode)) & 0170000) == (0120000))) {
    ...
    if (!resolved) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "failed to canonicalize %s", 5),
            quotearg_style(shell_escape_always_quoting_style, filename));
...
static void getenv_quoting_style(void) {
  ...
  if (q_style) {
    int i = argmatch(q_style, quoting_style_args,
                     (void const *)(quoting_style_vals),
...
static void print_esc_char(char c) {
  ...
  default:
    error(0, 0,
          dcgettext(((void *)0), "warning: unrecognized escape '\\%c'", 5), c);
    break;
  }
  putchar_unlocked(c);
}

__attribute__((__pure__)) static size_t
...
print_it(char const *format, int fd, char const *filename,
         ...
         _Bool (*print_func)(char *, size_t, char, char, int, char const *,
                             void const *),
         void const *data) {
  ...
  for (b = format; *b; b++) {
    ...
    case '%': {
          ...
          ((!!sizeof(struct {
            _Static_assert(
                1, "verify_expr ("
                   ...
                   "directive\", 5), quote (dest)), assume (false))"
                   ")");
            int _gl_dummy;
          }))
               ? ((error(1, 0,
                         dcgettext(((void *)0), "%s: invalid directive", 5),
                         quote(dest)),
      ...
      case 'L':
        mod_char = fmt_char;
        fmt_char = *(b + 1);
        if (print_func == print_stat && (fmt_char == 'd' || fmt_char == 'r')) {
          b++;
        } else {
          fmt_char = mod_char;
          mod_char = 0;
      ...
      default:
        fail |= print_func(dest, len, mod_char, fmt_char, fd, filename, data);
        break;
      }
      break;
    }
    ...
    case '\\':
      ...
      if (('0' <= (*b) && (*b) <= '7')) {
        ...
        --b;
      } else if (*b == 'x' && ((*__ctype_b_loc())[(int)((to_uchar(b[1])))] &
                               (unsigned short int)_ISxdigit)) {
        int esc_value = ((b[1]) >= 'a' && (b[1]) <= 'f'   ? (b[1]) - 'a' + 10
                         : (b[1]) >= 'A' && (b[1]) <= 'F' ? (b[1]) - 'A' + 10
                                                          : (b[1]) - '0');

        ++b;
        if (((*__ctype_b_loc())[(int)((to_uchar(b[1])))] &
             (unsigned short int)_ISxdigit)) {
          ++b;
          esc_value =
              esc_value * 16 + ((*b) >= 'a' && (*b) <= 'f'   ? (*b) - 'a' + 10
                                : (*b) >= 'A' && (*b) <= 'F' ? (*b) - 'A' + 10
                                                             : (*b) - '0');
        }
        putchar_unlocked(esc_value);
        ...
        error(0, 0,
              dcgettext(((void *)0), "warning: backslash at end of format", 5));
        putchar_unlocked('\\');
...
do_statfs(char const *filename, char const *format) {
  ...
  if (statfs(filename, &statfsbuf) != 0) {
    error(
        0, (*__errno_location()),
        dcgettext(((void *)0), "cannot read file system information for %s", 5),
        quotearg_style(shell_escape_always_quoting_style, filename));
    return 0;
  }
...
static unsigned int fmt_to_mask(char fmt) {
  ...
  case 'A':
    return 0x00000002U;
  case 'f':
    return 0x00000002U | 0x00000001U;
  case 'F':
    return 0x00000001U;
  case 'h':
    return 0x00000004U;
  ...
  case 'G':
    return 0x00000010U;
  case 'm':
    return 0x00000002U | 0x00000100U;
  case 's':
    return 0x00000200U;
  case 't':
  case 'T':
...
do_stat(char const *filename, char const *format, char const *format2) {
  int fd = (strcmp(filename, "-") == 0) ? 0 : -100;
  int flags = 0;
  struct stat st;
  struct statx stx = {
      0,
  ...
  if (fd < 0) {
    if (flags & 0x1000)
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "cannot stat standard input", 5));
    else
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "cannot statx %s", 5),
            quotearg_style(shell_escape_always_quoting_style, filename));
    return 0;
  }

  if (((((stx.stx_mode)) & 0170000) == (0060000)) ||
      ((((stx.stx_mode)) & 0170000) == (0020000)))
    format = format2;
...
print_stat(char *pformat, size_t prefix_len, char mod, char m, int fd,
  ...
  case 'N':
    out_string(pformat, prefix_len,
               quotearg_style(get_quoting_style(((void *)0)), filename));
    if (((((statbuf->st_mode)) & 0170000) == (0120000))) {
      ...
      printf(" -> ");
      out_string(pformat, prefix_len,
                 quotearg_style(get_quoting_style(((void *)0)), linkname));
      free(linkname);
    }
    break;
  case 'd':
    if (mod == 'H')
      out_uint(pformat, prefix_len, gnu_dev_major(statbuf->st_dev));
    else if (mod == 'L')
      out_uint(pformat, prefix_len, gnu_dev_minor(statbuf->st_dev));
    else
  ...
  case 'U':
    ...
    out_string(pformat, prefix_len, pw_ent ? pw_ent->pw_name : "UNKNOWN");
    break;
  case 'g':
  ...
  case 'G':
    ...
    out_string(pformat, prefix_len, gw_ent ? gw_ent->gr_name : "UNKNOWN");
    break;
  case 'm':
  ...
  case 'r':
    if (mod == 'H')
      out_uint(pformat, prefix_len, gnu_dev_major(statbuf->st_rdev));
    else if (mod == 'L')
      out_uint(pformat, prefix_len, gnu_dev_minor(statbuf->st_rdev));
    else
  ...
  case 'b':
    ...
    break;
  case 'o':
    out_uint(pformat, prefix_len,
             ((0 < (*statbuf).st_blksize &&
    ...
    if (terse) {
      ...
      if (device) {
        ...
        format = xasprintf("%s%s", format,
                           dcgettext(((void *)0),
                                     ""
                                     "Device: %Hd,%Ld\tInode: %-10i  Links: "
                                     "%-5h Device type: %Hr,%Lr\n",
                                     5)
...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Usage: %s [OPTION]... FILE...\n", 5),
           program_name);
    fputs_unlocked(
        dcgettext(((void *)0), "Display file or file system status.\n", 5),
        stdout)
    ...
    fputs_unlocked(
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "      --cached=MODE     specify how to use "
                             "cached attributes;\n                          "
    ...
    fputs_unlocked(
      ...
      fputs_unlocked (
     dcgettext (((void *)0), 
     "\nThe valid format sequences for files (without --file-system):\n\n  %a   permission bits in octal (note '     , 5)
     ,
     stdout
     )
