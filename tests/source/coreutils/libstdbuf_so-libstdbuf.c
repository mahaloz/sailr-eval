...
static char const *fileno_to_name(const int fd) {
  char const *ret = ((void *)0);
...
static void apply_mode(FILE *stream, char const *mode) {
  char *buf = ((void *)0);
  int setvbuf_mode;
  uintmax_t size = 0;
  ...
  if (*mode == '0')
    ...
    if (size == 0 || *mode_end) {
      fprintf(stderr,
              dcgettext(((void *)0), "invalid buffering mode %s for %s\n", 5),
  ...
  if (setvbuf(stream, buf, setvbuf_mode, size) != 0) {
    fprintf(
        stderr,
        dcgettext(((void *)0), "could not set buffering of %s to mode %s\n", 5),
