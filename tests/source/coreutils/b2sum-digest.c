...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0), "Print or check %s (%d-bit) checksums.\n", 5)
    ...
    if (0)
      fputs_unlocked(dcgettext(((void *)0),
                               ...
                               "(default unless reading tty stdin)\n",
                               5),
                     stdout)
...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
  ...
  algo_name_len = strlen("BLAKE2b");
  if ((strncmp(s + i, "BLAKE2b", algo_name_len) == 0)) {
    i += algo_name_len;
  ...
  *hex_digest = (unsigned char *)&s[i];
  ...
  while (((*__ctype_b_loc())[(int)((*hp++))] & (unsigned short int)_ISxdigit))
    digest_hex_bytes++;
  if (digest_hex_bytes < 2 || digest_hex_bytes % 2 || 64 * 2 < digest_hex_bytes)
    return 0;
...
digest_file(char const *filename, int *binary, unsigned char *bin_result,
  ...
  *missing = 0;
  ...
  err = blake2b_stream(fp, bin_result, digest_length / 8);
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "b2sum", "GNU coreutils", Version, ("Padraig Brady"),
                  ("Samuel Neves"), (char *)((void *)0));
      exit(0);
      break;
      ;
    default:
  ...
  if (digest_length > 64 * 8) {
    ...
    ((!!sizeof(struct {
      _Static_assert(
          ...
          "1"
          ", "
          "(error (1, 0, dcgettext (((void *)0), \"maximum digest length for "
          "%s is %d bits\", 5), quote (\"BLAKE2b\"), 64 * 8), assume (false))"
          ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
  ...
  for (char **operandp = argv + optind; operandp < operand_lim; operandp++) {
    ...
    if (do_check)
      ...
      if (!digest_file(file, &binary_file, bin_buffer, &missing, &length))
        ...
        output_file(file, binary_file, bin_buffer, prefix_tag, digest_delim,
                    optind != argc, length);
      }
    }
  }
