...
void usage(int status) {
  if (status != 0)
    ...
    printf(
        dcgettext(((void *)0), "Usage: %s [STRING]...\n  or:  %s OPTION\n", 5)

            ,
        program_name, program_name);

    fputs_unlocked(dcgettext(((void *)0),
                             "Repeatedly output a line with all specified "
                             "STRING(s), or 'y'.\n\n",
                             5),
                   stdout)
...
int main(int argc, char **argv) {
  ...
  parse_gnu_standard_options_only(argc, argv, "yes", "GNU coreutils", Version,
                                  1, usage, ("David MacKenzie"),
                                  (char const *)((void *)0));
  ...
  while (full_write(1, buf, bufused) == bufused)
    continue;
  error(0, (*__errno_location()), dcgettext(((void *)0), "standard output", 5));
  exit(1);
}
