...
__attribute__((__format__(printf, 1, 2))) static _Noreturn void
...
static void beyond(void) {
  test_syntax_error(dcgettext(((void *)0), "missing argument after %s", 5),
                    quote(argv[argc - 1]));
}
...
static char const *find_int(char const *string) {
  ...
  test_syntax_error(dcgettext(((void *)0), "invalid integer %s", 5),
                    quote(string));
}
...
term(void) {
  ...
  if (argv[pos][0] == '(' && argv[pos][1] == '\0') {
    ...
    if (argv[pos] == 0)
      test_syntax_error(dcgettext(((void *)0), "%s expected", 5), quote(")"));
    else if (argv[pos][0] != ')' || argv[pos][1])
      test_syntax_error(dcgettext(((void *)0), "%s expected, found %s", 5),
  ...
  if (argv[op][0] == '-') {
    ...
    case 'n':
      if (argv[op][2] == 't' && !argv[op][3]) {
        ...
        if (l_is_l || r_is_l)
          test_syntax_error(
              dcgettext(((void *)0), "-nt does not accept -l", 5));
    ...
    case 'e':
      if (argv[op][2] == 'f' && !argv[op][3]) {
        ...
        if (l_is_l || r_is_l)
          test_syntax_error(
              dcgettext(((void *)0), "-ef does not accept -l", 5));
    ...
    case 'o':
      if ('t' == argv[op][2] && '\000' == argv[op][3]) {
        ...
        if (l_is_l || r_is_l)
          test_syntax_error(
              dcgettext(((void *)0), "-ot does not accept -l", 5));
        le = get_mtime(argv[op - 1], &lt);
        re = get_mtime(argv[op + 1], &rt);
    ...
    test_syntax_error(dcgettext(((void *)0), "%s: unknown binary operator", 5),
                      quote(argv[op]));
  }
...
unary_operator(void) {
  ...
  switch (argv[pos][1]) {
  default:
    test_syntax_error(dcgettext(((void *)0), "%s: unary operator expected", 5),
                      quote(argv[pos]));
    return 0;
  ...
  case 'O': {
    ...
    uid_t euid = geteuid();
    uid_t NO_UID = -1;
    return !(euid == NO_UID && (*__errno_location())) &&
  ...
  case 'G': {
    ...
    gid_t egid = getegid();
    gid_t NO_GID = -1;
    return !(egid == NO_GID && (*__errno_location())) &&
  ...
  case 's':
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && 0 < stat_buf.st_size);
  ...
  case 'g':
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && (stat_buf.st_mode & 02000));

  case 'k':
    unary_advance();
    return (stat(argv[pos - 1], &stat_buf) == 0 && (stat_buf.st_mode & 01000));
  ...
  case 't': {
    ...
    return ((*__errno_location()) != 34 && 0 <= fd && fd <= 0x7fffffff &&
            isatty(fd));
  }
...
three_arguments(void) {
  ...
  if (binop(argv[pos + 1]))
    ...
    test_syntax_error(dcgettext(((void *)0), "%s: binary operator expected", 5),
                      quote(argv[pos + 1]));
  return (value);
}
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "EXPRESSION2 are true\n  EXPRESSION1 -o EXPRESSION2   either "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  " INTEGER1 -ge INTEGER2   INTEGER1 is greater than or equal "
                  "to INTEGER2\n  INTEGER1 -gt INTEGER2   INTEGER1 is greater "
                  "than INTEGER2\n  INTEGER1 -le INTEGER2   INTEGER1 is less "
    ...
    fputs_unlocked(
        dcgettext(
            ((void *)0),
            "\n  FILE1 -ef FILE2   FILE1 and FILE2 have the same device and "
    ...
    fputs_unlocked(
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "\nNOTE: [ honors the --help and --version "
    ...
    printf(dcgettext(((void *)0),
                     ...
                     5),
           dcgettext(((void *)0), "test and/or [", 5));
    emit_ancillary_info("[");
  }
  exit(status);
}
int main(int margc, char **margv) {
  ...
  if (1) {
    ...
    if (margc == 2) {
      ...
      if ((strcmp(margv[1], "--version") == 0)) {
        version_etc(stdout, "[", "GNU coreutils", Version, ("Kevin Braunsdorf"),
                    ("Matthew Bradburn"), (char *)((void *)0));
        return 0;
      }
    }
    if (margc < 2 || !(strcmp(margv[margc - 1], "]") == 0))
      test_syntax_error(dcgettext(((void *)0), "missing %s", 5), quote("]"));
  ...
  if (pos != argc)
    test_syntax_error(dcgettext(((void *)0), "extra argument %s", 5),
                      quote(argv[pos]));
