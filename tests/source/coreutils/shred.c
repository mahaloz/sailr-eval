...
struct randint_source *randint_all_new(char const *, size_t)
    ...
    __attribute__((__malloc__(randint_all_free, 1)));
...
static inline randint randint_choose(struct randint_source *s,
                                     randint choices) {
...
_Static_assert(0 < SECTOR_SIZE && (SECTOR_SIZE & SECTOR_MASK) == 0,
               "verify ("
               "0 < SECTOR_SIZE && (SECTOR_SIZE & SECTOR_MASK) == 0"
               ")");
...
static char const *const remove_args[] = {"unlink", "wipe", "wipesync",
                                          ((void *)0)

};

static enum remove_method const remove_methods[] = {remove_unlink, remove_wipe,
                                                    remove_wipesync};
...
enum { RANDOM_SOURCE_OPTION = 0x7f + 1 };
...
void usage(int status) {
  if (status != 0)
    ...
    printf(dcgettext(((void *)0),
                     ...
                     "necessary\n  -n, --iterations=N  overwrite N times "
                     "instead of the default (%d)\n      --random-source=FILE  "
                     "get random bytes from FILE\n  -s, --size=N   shred this "
                     "many bytes (suffixes like K, M, G accepted)\n",
                     5)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "not round file sizes up to the next full block;\n                 "
            "  this is the default for non-regular files\n  -z, --zero     add "
            "a final overwrite with zeros to hide shredding\n",
            5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "not to remove\nthe files because it is common to operate on "
            "device files like /dev/hda,\nand those files usually should not "
            "be removed.\nThe optional HOW parameter indicates how to remove a "
            "directory entry:\n'unlink' => use a standard unlink call.\n'wipe' "
            "=> also first obfuscate bytes in the name.\n'wipesync' => also "
            "sync each obfuscated byte to the device.\nThe default mode is "
    ...
    fputs_unlocked(
        dcgettext(
            ...
            "otherwise.  Also, backups\nand mirrors may contain unremovable "
            "copies that will let a shredded file\nbe recovered later.  See "
            "the GNU coreutils manual for details.\n",
            5),
        stdout)
...
static int dosync(int fd, char const *qname) {
  ...
  if (!ignorable_sync_errno(err)) {
    error(0, err, dcgettext(((void *)0), "%s: fdatasync failed", 5), qname);
  ...
  err = (*__errno_location());
  if (!ignorable_sync_errno(err)) {
    error(0, err, dcgettext(((void *)0), "%s: fsync failed", 5), qname);
...
static void direct_mode(int fd, _Bool enable) {
  if (040000) {
    ...
    if (0 < fd_flags) {
      int new_flags = (enable ? (fd_flags | 040000) : (fd_flags & ~040000));
...
static int dopass(int fd, struct stat const *st, char const *qname,
  ...
  if (!dorewind(fd, st)) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: cannot rewind", 5), qname);
    other_error = 1;
  ...
  if (n) {
    error(0, 0, dcgettext(((void *)0), "%s: pass %lu/%lu (%s)...", 5), qname, k,
          n, pass_string);
  ...
  while (1) {
    ...
    for (soff = 0; soff < lim; soff += ssize) {
      ...
      if (0 < ssize)
        ...
        if (!known(size) && (ssize == 0 || (*__errno_location()) == 28)) {
          ...
          error(0, errnum,
                dcgettext(((void *)0), "%s: error writing at offset %s", 5),
          ...
          if (errnum == 5 && known(size) && (soff | SECTOR_MASK) < lim) {
            ...
            error(0, (*__errno_location()),
                  dcgettext(((void *)0), "%s: lseek failed", 5), qname);
          }
          other_error = 1;
          goto free_pattern_mem;
        }
    ...
    if (((off_t)(!(!((off_t)0 < (off_t)-1))
            ...
            offset <
        soff) {
      error(0, 0, dcgettext(((void *)0), "%s: file too large", 5), qname);
      other_error = 1;
      goto free_pattern_mem;
    }
    ...
    if (n && ((done && *previous_human_offset) ||
      ...
      if (done || !(strcmp(previous_human_offset, human_offset) == 0)) {
        if (!known(size))
          error(0, 0, dcgettext(((void *)0), "%s: pass %lu/%lu (%s)...%s", 5),
          ...
          error(0, 0,
                dcgettext(((void *)0), "%s: pass %lu/%lu (%s)...%s/%s %d%%", 5),
                qname, k, n, pass_string, human_offset, human_size, percent);
        }
...
static int const patterns[] = {
    -2,     2,      0x000,  0xFFF,  2,      0x555,  0xAAA,  -1,     6,
    0x249,  0x492,  0x6DB,  0x924,  0xB6D,  0xDB6,  12,     0x111,  0x222,
    0x333,  0x444,  0x666,  0x777,  0x888,  0x999,  0xBBB,  0xCCC,  0xDDD,
    0xEEE,  -1,

    8,      0x1000, 0x1249, 0x1492, 0x16DB, 0x1924, 0x1B6D, 0x1DB6, 0x1FFF,
    14,     0x1111, 0x1222, 0x1333, 0x1444, 0x1555, 0x1666, 0x1777, 0x1888,
    0x1999, 0x1AAA, 0x1BBB, 0x1CCC, 0x1DDD, 0x1EEE, -1,     0};
...
do_wipefd(int fd, char const *qname, struct randint_source *s,
  ...
  if (fstat(fd, &st)) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: fstat failed", 5), qname);
    return 0;
  }
  ...
  if ((((((st.st_mode)) & 0170000) == (0020000)) && isatty(fd)) ||
    ...
    error(0, 0, dcgettext(((void *)0), "%s: invalid file type", 5), qname);
    return 0;
  } else if (((((st.st_mode)) & 0170000) == (0100000)) && st.st_size < 0) {
    error(0, 0, dcgettext(((void *)0), "%s: file has negative size", 5), qname);
    return 0;
  }
  ...
  if (size == -1) {
    if (((((st.st_mode)) & 0170000) == (0100000))) {
      ...
      if (!flags->exact) {
        ...
        off_t remainder = size % ((0 < (st).st_blksize &&
                                   (st).st_blksize <= ((size_t)-1) / 8 + 1)
                                      ? (st).st_blksize
                                      : 512);
        if (size && size < ((0 < (st).st_blksize &&
                             (st).st_blksize <= ((size_t)-1) / 8 + 1)
                                ? (st).st_blksize
                                : 512))
          i_size = size;
        if (remainder != 0) {
          off_t size_incr =
              ((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1)
                   ? (st).st_blksize
                   : 512) -
              remainder;
          size +=
  ...
  if (flags->remove_file && ftruncate(fd, 0) != 0 &&
    ...
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: error truncating", 5), qname);
    ok = 0;
    goto wipefd_out;
  }
...
wipefd(int fd, char const *qname, struct randint_source *s,
  ...
  if (fd_flags < 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: fcntl failed", 5), qname);
    return 0;
  }
  if (fd_flags & 02000) {
    error(0, 0,
          dcgettext(((void *)0), "%s: cannot shred append-only file descriptor",
                    5),
          qname);
    return 0;
  }
...
incname(char *name, size_t len) {
  while (len--) {
    ...
    ((void)sizeof((p) ? 1 : 0), __extension__({
       if (p)
         ;
       else
         __assert_fail("p", "src/shred.c", 1000,
...
wipename(char *oldname, char const *qoldname, struct Options const *flags) {
  ...
  char *qdir =
      xstrdup(quotearg_n_style_colon(0, shell_escape_quoting_style, dir));
  ...
  if (flags->verbose)
    error(0, 0, dcgettext(((void *)0), "%s: removing", 5), qoldname);
      ...
      if (rename_ok) {
        ...
        if (flags->verbose) {
          ...
          error(0, 0,
                ...
                dcgettext(((void *)0), "%s: renamed to %s", 5), old, newname);
          first = 0;
        }
  ...
  if (unlink(oldname) != 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: failed to remove", 5), qoldname);
    ok = 0;
  } else if (flags->verbose)
    error(0, 0, dcgettext(((void *)0), "%s: removed", 5), qoldname);
  if (0 <= dir_fd) {
    if (dosync(dir_fd, qdir) != 0)
      ok = 0;
    if (close(dir_fd) != 0) {
      error(0, (*__errno_location()),
            dcgettext(((void *)0), "%s: failed to close", 5), qdir);
      ok = 0;
    }
  }
  free(newname);
...
wipefile(char *name, char const *qname, struct randint_source *s,
         struct Options const *flags) {
  ...
  if (fd < 0 && ((*__errno_location()) == 13 && flags->force) &&
      chmod(name, 0200) == 0)
    fd = open_safer(name, 01 | 0400 | 0);
  if (fd < 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: failed to open for writing", 5), qname);
    return 0;
  }
  ...
  if (close(fd) != 0) {
    error(0, (*__errno_location()),
          dcgettext(((void *)0), "%s: failed to close", 5), qname);
    ok = 0;
  }
...
static void clear_random_data(void) { randint_all_free(randint_source); }
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "fn:s:uvxz", long_opts, ((void *)0))) !=
         -1) {
    switch (c) {
    case 'f':
      flags.force = 1;
      ...
      flags.n_iterations =
          xdectoumax(optarg, 0,
                     ...
                     ((((0x7fffffffffffffffL * 2UL + 1UL)) <
                       ((18446744073709551615UL) / sizeof(int)))
                          ? ((0x7fffffffffffffffL * 2UL + 1UL))
                          : ((18446744073709551615UL) / sizeof(int)))
                     ...
                     dcgettext(((void *)0), "invalid number of passes", 5), 0);
      break;
    ...
    case 'u':
      if (optarg == ((void *)0))
        ...
        flags.remove_file = ((remove_methods)[__xargmatch_internal(
            "--remove", optarg, remove_args, (void const *)(remove_methods),
            sizeof *(remove_methods), argmatch_die, 1)]);
      break;
    ...
    case 's':
      flags.size = xnumtoumax(
          ...
          dcgettext(((void *)0), "invalid file size", 5), 0);
      break;
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "shred", "GNU coreutils", Version, ("Colin Plumb"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             ...
             "random_source ? random_source : \"getrandom\")), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, (*__errno_location()), "%s",
