...
enum { IGNORE_FAIL_ON_NON_EMPTY_OPTION = 0x7f + 1 };
...
ignorable_failure(int error_number, char const *dir) {
  return (ignore_fail_on_non_empty &&
          (errno_rmdir_non_empty(error_number) ||
           (errno_may_be_non_empty(error_number) && !is_empty_dir(-100, dir) &&
            (*__errno_location()) == 0)));
}
...
remove_parents(char *dir) {
  ...
  while (1) {
    ...
    if (verbose)
      prog_fprintf(stdout, dcgettext(((void *)0), "removing directory, %s", 5),
    ...
    if (!ok) {
      ...
      if (ignorable_failure(rmdir_errno, dir)) {
        ...
        error(0, rmdir_errno, dcgettext(((void *)0), error_msg, 5),
              quotearg_style(shell_escape_always_quoting_style, dir));
      }
      break;
    }
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        ...
        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "  -p, --parents     remove DIRECTORY and its "
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "pv", longopts, ((void *)0))) != -1) {
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "rmdir", "GNU coreutils", Version,
    ...
    if (rmdir(dir) != 0) {
      ...
      if (rmdir_errno == 20) {
        ...
        if (last_unix_slash && (*(last_unix_slash + 1) == '\0')) {
          ...
          if ((ret != 0 && (*__errno_location()) != 20) ||
              (ret == 0 && ((((st.st_mode)) & 0170000) == (0040000)))) {

            char *dir_arg = xstrdup(dir);
            strip_trailing_slashes(dir);
            ret = lstat(dir, &st);
            if (ret == 0 && ((((st.st_mode)) & 0170000) == (0120000))) {
              error(0, 0,
      ...
      if (!custom_error)
        error(0, rmdir_errno, dcgettext(((void *)0), "failed to remove %s", 5),
              quotearg_style(shell_escape_always_quoting_style, dir));
