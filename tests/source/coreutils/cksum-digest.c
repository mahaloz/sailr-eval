...
extern int bsd_sum_stream(FILE *stream, void *resstream, uintmax_t *length);

extern int sysv_sum_stream(FILE *stream, void *resstream, uintmax_t *length);

typedef int (*sumfn)(FILE *, void *, uintmax_t *);

extern void output_bsd(char const *file, int binary_file, void const *digest,

                       _Bool tagged, unsigned char delim, _Bool args,
                       uintmax_t length);

extern void output_sysv(char const *file, int binary_file, void const *digest,
...
extern void output_crc(char const *file, int binary_file, void const *digest,

                       _Bool tagged, unsigned char delim, _Bool args,
                       uintmax_t length) __attribute__((__nonnull__(3)));
...
extern void *sha1_finish_ctx(struct sha1_ctx *ctx, void *__restrict__ resbuf);

extern void *sha1_read_ctx(const struct sha1_ctx *ctx,
                           void *__restrict__ resbuf);
...
extern void *sha256_finish_ctx(struct sha256_ctx *ctx,
                               void *__restrict__ resbuf);
extern void *sha224_finish_ctx(struct sha256_ctx *ctx,
                               void *__restrict__ resbuf);
...
extern void *sha512_finish_ctx(struct sha512_ctx *ctx,
                               void *__restrict__ resbuf);
extern void *sha384_finish_ctx(struct sha512_ctx *ctx,
                               void *__restrict__ resbuf);
...
extern void *sm3_finish_ctx(struct sm3_ctx *ctx, void *__restrict__ resbuf);

extern void *sm3_read_ctx(const struct sm3_ctx *ctx, void *__restrict__ resbuf);
...
static _Bool status_only = 0;

static _Bool warn = 0;

static _Bool ignore_missing = 0;

static _Bool quiet = 0;

static _Bool strict = 0;
...
static int md5_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return md5_stream(stream, resstream);
}
static int sha1_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return sha1_stream(stream, resstream);
}
static int sha224_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return sha224_stream(stream, resstream);
}
static int sha256_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return sha256_stream(stream, resstream);
}
static int sha384_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return sha384_stream(stream, resstream);
}
static int sha512_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
  return sha512_stream(stream, resstream);
}
static int blake2b_sum_stream(FILE *stream, void *resstream,
                              uintmax_t *length) {
  return blake2b_stream(stream, resstream, *length);
}
static int sm3_sum_stream(FILE *stream, void *resstream, uintmax_t *length) {
...
static char const *const algorithm_args[] = {
    "bsd",    "sysv",   "crc",     "md5", "sha1",     "sha224", "sha256",
    "sha384", "sha512", "blake2b", "sm3", ((void *)0)

};
static enum Algorithm const algorithm_types[] = {
    bsd, sysv, crc, md5, sha1, sha224, sha256, sha384, sha512, blake2b, sm3,
};
_Static_assert((sizeof(algorithm_args) / sizeof *(algorithm_args)) ==
                   (sizeof(algorithm_types) / sizeof *(algorithm_types)) + 1,
               "verify ("
               "ARRAY_CARDINALITY (algorithm_args) == ARRAY_CARDINALITY "
               "(algorithm_types) + 1"
               ")");
...
static int const algorithm_bits[] = {16,  16,  32,  128, 160, 224,
                                     256, 384, 512, 512, 256, 0};

_Static_assert(
    (sizeof(algorithm_bits) / sizeof *(algorithm_bits)) ==
        (sizeof(algorithm_args) / sizeof *(algorithm_args)),
    "verify ("
    "ARRAY_CARDINALITY (algorithm_bits) == ARRAY_CARDINALITY (algorithm_args)"
    ")");

static _Bool algorithm_specified = 0;
static enum Algorithm cksum_algorithm = crc;
static sumfn cksumfns[] = {
    bsd_sum_stream,    sysv_sum_stream,    crc_sum_stream,    md5_sum_stream,
    sha1_sum_stream,   sha224_sum_stream,  sha256_sum_stream, sha384_sum_stream,
    sha512_sum_stream, blake2b_sum_stream, sm3_sum_stream,
};
static digest_output_fn cksum_output_fns[] = {
    output_bsd,  output_sysv, output_crc,  output_file,
...
static struct option const long_options[] = {
    ...
    {"check", 0, ((void *)0), 'c'},
    {"ignore-missing", 0, ((void *)0), IGNORE_MISSING_OPTION},
    {"quiet", 0, ((void *)0), QUIET_OPTION},
    {"status", 0, ((void *)0), STATUS_OPTION},
    {"warn", 0, ((void *)0), 'w'},
    {"strict", 0, ((void *)0), STRICT_OPTION},
    {"tag", 0, ((void *)0), TAG_OPTION},
    {"zero", 0, ((void *)0), 'z'},

    {"algorithm", 1, ((void *)0), 'a'},
    {"debug", 0, ((void *)0), DEBUG_PROGRAM_OPTION},
    {"untagged", 0, ((void *)0), UNTAG_OPTION},
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(dcgettext(((void *)0),
                             "  -a, --algorithm=TYPE  select the digest type "
                             "to use.  See DIGEST below.\n",
                             5),
                   stdout)

        ;
    fputs_unlocked(dcgettext(((void *)0),
                             "  -c, --check           read checksums from the "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -l, --length=BITS     digest length in bits; must not "
                  "exceed the max for\n                          the blake2 "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "print OK for each successfully verified file\n      "
                  "--status          don't output anything, status code shows "
                  "success\n      --strict          exit non-zero for "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "\nDIGEST determines the digest algorithm and default output "
                  "format:\n  sysv      (equivalent to sum -s)\n  bsd       "
                  "(equivalent to sum -r)\n  crc       (equivalent to cksum)\n "
                  " md5       (equivalent to md5sum)\n  sha1      (equivalent "
                  "to sha1sum)\n  sha224    (equivalent to sha224sum)\n  "
                  "sha256    (equivalent to sha256sum)\n  sha384    "
                  "(equivalent to sha384sum)\n  sha512    (equivalent to "
...
hex_digits(unsigned char const *s) {
  for (unsigned int i = 0; i < digest_hex_bytes; i++) {
    if (!((*__ctype_b_loc())[(int)((*s))] & (unsigned short int)_ISxdigit))
      return 0;
    ++s;
  }
  return *s == '\0';
...
bsd_split_3(char *s, size_t s_len, unsigned char **hex_digest, char **file_name,
  ...
  *file_name = s;
  ...
  if (escaped_filename && filename_unescape(s, i) == ((void *)0))
    return 0;
...
static ptrdiff_t algorithm_from_tag(char *s) {
  ...
  if (!max_tag_len) {
    ...
    while (*tag) {
      ...
      max_tag_len = (((tag_len) > (max_tag_len)) ? (tag_len) : (max_tag_len));
    }
  }

  size_t i = 0;

  while (i <= max_tag_len && s[i] && !((s[i]) == ' ' || (s[i]) == '\t') &&
         s[i] != '-' && s[i] != '(')
    ++i;
...
split_3(char *s, size_t s_len, unsigned char **hex_digest, int *binary,
        char **file_name) {

  _Bool escaped_filename = 0;
  ...
  algo_name_len = strlen(algorithm_tags[cksum_algorithm]);
  if ((strncmp(s + i, algorithm_tags[cksum_algorithm], algo_name_len) == 0)) {
    i += algo_name_len;

    char const *algo_name = &s[i - algo_name_len];

    _Bool length_specified = s[i] == '-';

    _Bool openssl_format = s[i] == '(';
    s[i++] = '\0';
    if (!(strcmp(algo_name, algorithm_tags[cksum_algorithm]) == 0))
      return 0;
    if (openssl_format)
      s[--i] = '(';
    ...
    if (length_specified) {
      uintmax_t length;
      char *siend;
      if (!(xstrtoumax(s + i, &siend, 0, &length, ((void *)0)) == LONGINT_OK &&
            0 < length && length <= digest_length && length % 8 == 0))
        return 0;
    ...
    if (s[i] == '(') {
      ++i;
      *binary = 0;
      return bsd_split_3(s + i, s_len - i, hex_digest, file_name,
                         escaped_filename);
    }
    return 0;
  }
  ...
  *hex_digest = (unsigned char *)&s[i];
  ...
  if (cksum_algorithm == blake2b) {
    ...
    digest_hex_bytes = 0;
    while (((*__ctype_b_loc())[(int)((*hp++))] & (unsigned short int)_ISxdigit))
      digest_hex_bytes++;
    if (digest_hex_bytes < 2 || digest_hex_bytes % 2 ||
        64 * 2 < digest_hex_bytes)
      return 0;
  ...
  *file_name = &s[i];
  ...
  if (escaped_filename)
    return filename_unescape(&s[i], s_len - i) != ((void *)0);
...
digest_file(char const *filename, int *binary, unsigned char *bin_result,
            ...
            _Bool *missing, uintmax_t *length) {
  FILE *fp;
  int err;
  ...
  *missing = 0;
  ...
  if (is_stdin) {
    ...
    fp = fopen_safer(filename, (0 && *binary ? "rb" : "r"));
    if (fp == ((void *)0)) {
      if (ignore_missing && (*__errno_location()) == 2) {
        *missing = 1;
        return 1;
      }
  ...
  err = cksumfns[cksum_algorithm](fp, bin_result, length);

  err = err ? (*__errno_location()) : 0;
  if (is_stdin)
    clearerr_unlocked(fp);
  else if (rpl_fclose(fp) != 0 && !err)
...
digest_check(char const *checkfile_name) {
  ...
  _Bool properly_formatted_lines = 0;

  _Bool matched_checksums = 0;
  unsigned char bin_buffer_unaligned[(512 / 8) + 8];
  ...
  _Bool is_stdin = (strcmp(checkfile_name, "-") == 0);

  if (is_stdin) {
    have_read_stdin = 1;
    checkfile_name = dcgettext(((void *)0), "standard input", 5);
    checkfile_stream = stdin;
  } else {
    checkfile_stream = fopen_safer(checkfile_name, "r");
    if (checkfile_stream == ((void *)0)) {
      error(0, (*__errno_location()), "%s",
      ...
      ((!!sizeof(struct {
        _Static_assert(
            1, "verify_expr ("
               "1"
               ", "
               "(error (1, 0, dcgettext (((void *)0), \"%s: too many checksum "
               "lines\", 5), quotearg_n_style_colon (0, "
               "shell_escape_quoting_style, checkfile_name)), assume (false))"
               ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0), "%s: too many checksum lines", 5),
                     quotearg_n_style_colon(0, shell_escape_quoting_style,
                                            checkfile_name)),
    ...
    if (line_length <= 0)
      ...
      if (warn) {
        error(0, 0,
              ...
              quotearg_n_style_colon(0, shell_escape_quoting_style,
                                     checkfile_name),
              line_number, algorithm_tags[cksum_algorithm]);
      }
      ...
      _Bool needs_escape = !status_only && strchr(filename, '\n');
      ...
      if (!ok) {
        ...
        if (!status_only) {
          ...
          printf(": %s\n", dcgettext(((void *)0), "FAILED open or read", 5));
        }
        ...
        for (cnt = 0; cnt < digest_bin_bytes; ++cnt) {
          if ((__extension__({
                int __res;
                if (sizeof(hex_digest[2 * cnt]) > 1) {
                  if (__builtin_constant_p(hex_digest[2 * cnt])) {
                    int __c = (hex_digest[2 * cnt]);
                    __res = __c < -128 || __c > 255
                                ? __c
                                : (*__ctype_tolower_loc())[__c];
                  } else
                    __res = tolower(hex_digest[2 * cnt]);
                } else
                  __res = (*__ctype_tolower_loc())[(int)(hex_digest[2 * cnt])];
                __res;
              }))
              ...
              ((__extension__({
                 int __res;
                 if (sizeof(hex_digest[2 * cnt + 1]) > 1) {
                   if (__builtin_constant_p(hex_digest[2 * cnt + 1])) {
                     int __c = (hex_digest[2 * cnt + 1]);
                     __res = __c < -128 || __c > 255
                                 ? __c
                                 : (*__ctype_tolower_loc())[__c];
                   } else
                     __res = tolower(hex_digest[2 * cnt + 1]);
                 } else
                   __res =
                       (*__ctype_tolower_loc())[(int)(hex_digest[2 * cnt + 1])];
                 __res;
               }))
  ...
  } while (!feof_unlocked(checkfile_stream) &&
           !ferror_unlocked(checkfile_stream));

  free(line);

  int err = ferror_unlocked(checkfile_stream) ? 0 : -1;
  if (is_stdin)
    clearerr_unlocked(checkfile_stream);
  else if (rpl_fclose(checkfile_stream) != 0 && err < 0)
    err = (*__errno_location());

  if (0 <= err) {
    error(
        0, err, err ? "%s" : dcgettext(((void *)0), "%s: read error", 5),
        quotearg_n_style_colon(0, shell_escape_quoting_style, checkfile_name));
    return 0;
  }
  ...
  if (!properly_formatted_lines) {
    ...
    error(
        0, 0,
        dcgettext(((void *)0), "%s: no properly formatted checksum lines found",
                  5),
        quotearg_n_style_colon(0, shell_escape_quoting_style, checkfile_name));
  } else {
    if (!status_only) {
      if (n_misformatted_lines != 0)
        error(0, 0,
              (dcngettext(((void *)0),
                          ...
                          select_plural(n_misformatted_lines), 5)
      ...
      if (n_open_or_read_failures != 0)
        error(0, 0,
              (dcngettext(((void *)0),
                          ...
                          select_plural(n_open_or_read_failures), 5)
      ...
      if (n_mismatched_checksums != 0)
        error(0, 0,
              (dcngettext(((void *)0),
                          ...
                          select_plural(n_mismatched_checksums), 5)

                   ),
              n_mismatched_checksums);

      if (ignore_missing && !matched_checksums)
        error(0, 0, dcgettext(((void *)0), "%s: no file was verified", 5),
              quotearg_n_style_colon(0, shell_escape_quoting_style,
                                     checkfile_name));
    }
  }
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, short_opts, long_options,
    ...
    case 'a':
      cksum_algorithm = ((algorithm_types)[__xargmatch_internal(
          "--algorithm", optarg, algorithm_args,
          (void const *)(algorithm_types), sizeof *(algorithm_types),
          argmatch_die, 0)]);
    ...
    case 'l':
      digest_length =
          xdectoumax(optarg, 0, (18446744073709551615UL), "",
                     ...
                     dcgettext(((void *)0), "invalid length", 5), 0);
      digest_length_str = optarg;
      if (digest_length % 8 != 0) {
        error(0, 0, dcgettext(((void *)0), "invalid length: %s", 5),
              quote(digest_length_str));
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"length is "
                            "not a multiple of 8\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "length is not a multiple of 8",
                                 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "length is not a multiple of 8",
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "cksum", "GNU coreutils", Version, ("Padraig Brady"),
                  ("Q. Frank Xia"), (char *)((void *)0));
      exit(0);
      break;
      ;
    default:
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"--length is only "
             "supported with --algorithm=blake2b\", 5)), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
    ...
    ((!!sizeof(struct {
      _Static_assert(
          1, "verify_expr ("
             "1"
             ", "
             "(error (1, 0, dcgettext (((void *)0), \"maximum digest length "
             "for %s is %d bits\", 5), quote "
             "(algorithm_tags[cksum_algorithm]), 64 * 8), assume (false))"
             ")");
      int _gl_dummy;
    }))
         ? ((error(1, 0,
                   dcgettext(((void *)0),
                             "maximum digest length for %s is %d bits", 5),
                   quote(algorithm_tags[cksum_algorithm]), 64 * 8),
             ((0) ? (void)0 : __builtin_unreachable())))
         : ((error(1, 0,
                   dcgettext(((void *)0),
                             "maximum digest length for %s is %d bits", 5),
    ...
    if (do_check && algorithm_specified)
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "1"
            ", "
            "(error (1, 0, dcgettext (((void *)0), \"--check is not supported "
            "with --algorithm={bsd,sysv,crc}\", 5)), assume (false))"
            ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0),
                               "--check is not supported with "
                               "--algorithm={bsd,sysv,crc}",
                               5)),
               ((0) ? (void)0 : __builtin_unreachable())))
           : ((error(1, 0,
                     dcgettext(((void *)0),
  ...
  if (prefix_tag && !binary) {
    ...
    error(0, 0,
          dcgettext(((void *)0), "--tag does not support --text mode", 5));
    usage(1);
  }
  ...
  for (char **operandp = argv + optind; operandp < operand_lim; operandp++) {
    ...
    if (do_check)
      ...
      if (!digest_file(file, &binary_file, bin_buffer, &missing, &length))
        ...
        cksum_output_fns[cksum_algorithm](file, binary_file, bin_buffer,
                                          prefix_tag, digest_delim,
                                          optind != argc, length);
      }
    }
  }
