...
enum { DEFAULT_MODE, INODES_MODE, HUMAN_MODE, POSIX_MODE, OUTPUT_MODE };
...
typedef enum { BLOCK_FLD, INODE_FLD, OTHER_FLD } field_type_t;
...
static struct field_data_t field_data[] = {
    [SOURCE_FIELD] = {SOURCE_FIELD, "source", OTHER_FLD, "Filesystem", 14,
                      MBS_ALIGN_LEFT, 0},

    [FSTYPE_FIELD] = {FSTYPE_FIELD, "fstype", OTHER_FLD, "Type", 4,
                      MBS_ALIGN_LEFT, 0},

    [SIZE_FIELD] = {SIZE_FIELD, "size", BLOCK_FLD, "blocks", 5, MBS_ALIGN_RIGHT,
                    0},

    [USED_FIELD] = {USED_FIELD, "used", BLOCK_FLD, "Used", 5, MBS_ALIGN_RIGHT,
                    0},

    [AVAIL_FIELD] = {AVAIL_FIELD, "avail", BLOCK_FLD, "Available", 5,
                     MBS_ALIGN_RIGHT, 0},

    [PCENT_FIELD] = {PCENT_FIELD, "pcent", BLOCK_FLD, "Use%", 4,
                     MBS_ALIGN_RIGHT, 0},

    [ITOTAL_FIELD] = {ITOTAL_FIELD, "itotal", INODE_FLD, "Inodes", 5,
                      MBS_ALIGN_RIGHT, 0},

    [IUSED_FIELD] = {IUSED_FIELD, "iused", INODE_FLD, "IUsed", 5,
                     MBS_ALIGN_RIGHT, 0},

    [IAVAIL_FIELD] = {IAVAIL_FIELD, "iavail", INODE_FLD, "IFree", 5,
                      MBS_ALIGN_RIGHT, 0},

    [IPCENT_FIELD] = {IPCENT_FIELD, "ipcent", INODE_FLD, "IUse%", 4,
                      MBS_ALIGN_RIGHT, 0},

    [TARGET_FIELD] = {TARGET_FIELD, "target", OTHER_FLD, "Mounted on", 0,
                      MBS_ALIGN_LEFT, 0},

    [FILE_FIELD] = {FILE_FIELD, "file", OTHER_FLD, "File", 0, MBS_ALIGN_LEFT,
                    0}};
...
enum { NO_SYNC_OPTION = 0x7f + 1, SYNC_OPTION, TOTAL_OPTION, OUTPUT_OPTION };
...
static int automount_stat_err(char const *file, struct stat *st) {
  int fd = open(file, 00 | 0400 | 04000);
  if (fd < 0) {
...
static void alloc_field(int f, char const *c) {
  ...
  if (field_data[f].used)
    ...
    ((void)sizeof((!"field used") ? 1 : 0), __extension__({
       if (!"field used")
         ;
       else
         __assert_fail("!\"field used\"", "src/df.c", 425,
...
static void decode_output_arg(char const *arg) {
  ...
  do {
    ...
    for (unsigned int i = 0; i < (sizeof(field_data) / sizeof *(field_data));
         i++) {
    ...
    if (field == INVALID_FIELD) {
      error(0, 0,
            dcgettext(((void *)0), "option --output: field %s unknown", 5),
            quote(s));
      usage(1);
    }
    ...
    if (field_data[field].used) {
      ...
      error(0, 0,
            dcgettext(((void *)0),
                      "option --output: field %s used more than once", 5),
    ...
    default:
      ...
      ((void)sizeof((!"invalid field") ? 1 : 0), __extension__({
         if (!"invalid field")
           ;
         else
           __assert_fail("!\"invalid field\"", "src/df.c", 496,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
    s = comma;
...
static void get_header(void) {
  ...
  for (col = 0; col < ncolumns; col++) {
    char *cell = ((void *)0);
    char const *header = dcgettext(((void *)0), columns[col]->caption, 5);
    ...
    if (columns[col]->field == SIZE_FIELD &&
      ...
      header = dcgettext(((void *)0), "blocks", 5);

      if (rpl_asprintf(&cell, dcgettext(((void *)0), "%s-%s", 5), num,
                       header) == -1)
        cell = ((void *)0);
...
static void filter_mount_list(_Bool devices_only) {
  ...
  devlist_table = hash_initialize(mount_list_size, ((void *)0), devlist_hash,
                                  devlist_compare, ((void *)0));
  if (devlist_table == ((void *)0))
    xalloc_die();
    ...
    if ((me->me_remote && show_local_fs) ||
      ...
      if (seen_dev) {
        ...
        _Bool target_nearer_root =
            strlen(seen_dev->me->me_mountdir) > strlen(me->me_mountdir);

        _Bool source_below_root =
            seen_dev->me->me_mntroot != ((void *)0)
...
__attribute__((__pure__)) static struct mount_entry const *
...
static char const *df_readable(_Bool negative, uintmax_t n, char *buf,
...
static void add_uint_with_neg_flag(uintmax_t *dest, _Bool *dest_neg,
...
static void get_dev(char const *device, char const *mount_point,
                    char const *file, char const *stat_file, char const *fstype,
  ...
  if (!force_fsu && mount_point && !(((mount_point)[0]) == '/'))
    return;
  ...
  if (force_fsu)
    ...
    if (process_all &&
        ((*__errno_location()) == 13 || (*__errno_location()) == 2)) {
      if (!show_all_fs)
        return;

      fstype = "-";
      fsu.fsu_bavail_top_bit_set = 0;
      fsu.fsu_blocksize = fsu.fsu_blocks = fsu.fsu_bfree = fsu.fsu_bavail =
          fsu.fsu_files = fsu.fsu_ffree = (18446744073709551615UL);
    } else {
      error(0, (*__errno_location()), "%s",
            quotearg_n_style_colon(0, shell_escape_quoting_style, stat_file));
      exit_status = 1;
      return;
    }
    ...
    if (stat(stat_file, &sb) == 0) {
      ...
      if (dev_me && !(strcmp(dev_me->me_devname, device) == 0) &&
  ...
  if (process_all && has_uuid_suffix(dev_name) &&
  ...
  for (col = 0; col < ncolumns; col++) {
    ...
    default:
      ...
      ((void)sizeof((!"bad field_type") ? 1 : 0), __extension__({
         if (!"bad field_type")
           ;
         else
           __assert_fail("!\"bad field_type\"", "src/df.c", 1152,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
    ...
    case ITOTAL_FIELD:
      cell = xstrdup(
          df_readable(0, v->total, buf, v->input_units, v->output_units));
      break;

    case USED_FIELD:
    case IUSED_FIELD:
      cell = xstrdup(df_readable(v->negate_used, v->used, buf, v->input_units,
                                 v->output_units));
      break;
    ...
    case IPCENT_FIELD: {
      ...
      if (!known_value(v->used) || !known_value(v->available))
        ;
               ...
               v->used <=
                   ((uintmax_t)(!(!((uintmax_t)0 < (uintmax_t)-1))
                                    ? (uintmax_t)-1
                                    : ((((uintmax_t)1
                                         << ((sizeof(uintmax_t) * 8) - 2)) -
                                        1) *
                                           2 +
                                       1))) /
                       100 &&
               v->used + v->available != 0 &&
               (v->used + v->available < v->used) == v->negate_available) {
        ...
        double a = v->negate_available ? -(double)-v->available : v->available;
    ...
    default:
      ...
      ((void)sizeof((!"unhandled field") ? 1 : 0), __extension__({
         if (!"unhandled field")
           ;
         else
           __assert_fail("!\"unhandled field\"", "src/df.c", 1254,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
    ...
    if (!cell)
      ...
      ((void)sizeof((!"empty cell") ? 1 : 0), __extension__({
         if (!"empty cell")
           ;
         else
           __assert_fail("!\"empty cell\"", "src/df.c", 1258,
                         __extension__ __PRETTY_FUNCTION__);
       }));
...
get_device(char const *device) {
  ...
  if (best_match) {
    ...
    error(0, 0,
          dcgettext(((void *)0),
                    "cannot access %s: over-mounted by another device", 5),
...
static void get_entry(char const *name, struct stat const *statp) {
  if ((((((statp->st_mode)) & 0170000) == (0060000)) ||
       ((((statp->st_mode)) & 0170000) == (0020000))) &&
      get_device(name))
    return;
...
static void get_all_entries(void) {
  ...
  for (me = mount_list; me; me = me->me_next)
    get_dev(me->me_devname, me->me_mountdir, ((void *)0), ((void *)0),
            me->me_type, me->me_dummy, me->me_remote, ((void *)0), 1);
}
...
void usage(int status) {
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "  -i, --inodes          list inode information instead of "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  "      --output[=FIELD_LIST]  use the output format defined "
                  "by FIELD_LIST,\n                               or print all "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  " -x, --exclude-type=TYPE   limit listing to file systems "
                  "not of type TYPE\n  -v                    (ignored)\n",
                  5),
        stdout)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "'avail', 'pcent', 'file' and 'target' (see info page).\n",
                  5),
        stdout)
...
int main(int argc, char **argv) {
  ...
  char const *msg_mut_excl =
      dcgettext(((void *)0), "options %s and %s are mutually exclusive", 5);
      ...
      for (fs_excl = fs_exclude_list; fs_excl; fs_excl = fs_excl->fs_next) {
        if ((strcmp(fs_incl->fs_name, fs_excl->fs_name) == 0)) {
          error(0, 0,
                ...
                dcgettext(((void *)0),
                          "file system type %s both selected and excluded", 5),
  ...
  if (mount_list == ((void *)0)) {
    ...
    char const *warning =
        (status == 0 ? dcgettext(((void *)0), "Warning: ", 5) : "");
    error(
        status, (*__errno_location()), "%s%s", warning,

        dcgettext(((void *)0), "cannot read table of mounted file systems", 5));
  }
    ...
    if (exit_status == 0)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          "1"
                          ", "
                          "(error (1, 0, dcgettext (((void *)0), \"no file "
                          "systems processed\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 0,
                     dcgettext(((void *)0), "no file systems processed", 5)),
