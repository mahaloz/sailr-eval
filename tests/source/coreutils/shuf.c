...
static size_t read_input_reservoir_sampling(FILE *in, char eolbyte, size_t k,
                                            struct randint_source *s,
                                            struct linebuffer **out_rsrv) {
  randint n_lines = 0;
  size_t n_alloc_lines =
      (((k) < (RESERVOIR_LINES_INCREMENT)) ? (k) : (RESERVOIR_LINES_INCREMENT));
  struct linebuffer *line = ((void *)0);
  struct linebuffer *rsrv;

  rsrv = xcalloc(n_alloc_lines, sizeof(struct linebuffer));

  while (n_lines < k && (line = readlinebuffer_delim(&rsrv[n_lines], in,
                                                     eolbyte)) != ((void *)0)) {
    n_lines++;
  ...
  if (line != ((void *)0)) {
    ...
    } while (readlinebuffer_delim(line, in, eolbyte) != ((void *)0) &&
             n_lines++);
    ...
    if (!n_lines)
      ((!!sizeof(struct {
        _Static_assert(1, "verify_expr ("
                          "1"
                          ", "
                          "(error (1, 75, dcgettext (((void *)0), \"too many "
                          "input lines\", 5)), assume (false))"
                          ")");
        int _gl_dummy;
      }))
           ? ((error(1, 75, dcgettext(((void *)0), "too many input lines", 5)),
...
static int write_permuted_output_reservoir(size_t n_lines,
                                           struct linebuffer *lines,
                                           size_t const *permutation) {
  for (size_t i = 0; i < n_lines; i++) {
    const struct linebuffer *p = &lines[permutation[i]];
    if (fwrite_unlocked(p->buffer, sizeof(char), p->length, stdout) !=
        p->length)
      return -1;
  }
...
static int write_permuted_lines(size_t n_lines, char *const *line,
                                size_t const *permutation) {
  for (size_t i = 0; i < n_lines; i++) {
...
static int write_permuted_numbers(size_t n_lines, size_t lo_input,
...
static int write_random_numbers(struct randint_source *s, size_t count,
                                size_t lo_input, size_t hi_input,
                                char eolbyte) {
...
static int write_random_lines(struct randint_source *s, size_t count,
...
int main(int argc, char **argv) {
  ...
  while ((optc = getopt_long(argc, argv, "ei:n:o:rz", long_opts,
                             ((void *)0))) != -1)
    switch (optc) {
    case 'e':
      echo = 1;
      ...
      if (input_range)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "-i options specified\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "multiple -i options specified",
                                 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "multiple -i options specified",
        ...
        *p = '\0';
        lo_input =
            xdectoumax(optarg, 0, (18446744073709551615UL), "",
                       ...
                       dcgettext(((void *)0), "invalid input range", 5), 0);
        *p = '-';
        hi_optarg = p + 1;
      }

      hi_input =
          xdectoumax(hi_optarg, 0, (18446744073709551615UL), "",
      ...
      if (invalid)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "dcgettext (((void *)0), \"invalid input range\", "
                            "5), quote (optarg)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, (*__errno_location()), "%s: %s",
                       dcgettext(((void *)0), "invalid input range", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, (*__errno_location()), "%s: %s",
                       dcgettext(((void *)0), "invalid input range", 5),
      ...
      if (e == LONGINT_OK)
        ...
        ((!!sizeof(struct {
          _Static_assert(1,
                         ...
                         "count: %s\", 5), quote (optarg)), assume (false))"
                         ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0,
                       dcgettext(((void *)0), "invalid line count: %s", 5),
                       quote(optarg)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "invalid line count: %s", 5),
      ...
      if (outfile && !(strcmp(outfile, optarg) == 0))
        ((!!sizeof(struct {
          ...
          int _gl_dummy;
             ...
             ? ((error(1, 0,
                       dcgettext(((void *)0), "multiple output files specified",
                                 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0,
                       dcgettext(((void *)0), "multiple output files specified",
    ...
    case GETOPT_VERSION_CHAR:
      version_etc(stdout, "shuf", "GNU coreutils", Version, ("Paul Eggert"),
                  (char *)((void *)0));
      exit(0);
      break;
      ;
  ...
  if (head_lines == 0) {
    ...
    if (n_operands == 1 && !((strcmp(operand[0], "-") == 0) ||
      ...
      ((!!sizeof(struct {
        _Static_assert(
            ...
            "(0, shell_escape_quoting_style, operand[0])), assume (false))"
            ")");
        int _gl_dummy;
      }))
      ...
      if (n_lines == 0)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            "1"
                            ", "
                            "(error (1, 0, dcgettext (((void *)0), \"no lines "
                            "to repeat\", 5)), assume (false))"
                            ")");
          int _gl_dummy;
        }))
             ? ((error(1, 0, dcgettext(((void *)0), "no lines to repeat", 5)),
                 ((0) ? (void)0 : __builtin_unreachable())))
             : ((error(1, 0, dcgettext(((void *)0), "no lines to repeat", 5)),
                 ((0) ? (void)0 : __builtin_unreachable()))));
      if (input_range)
        i = write_random_numbers(randint_source, ahead_lines, lo_input,
                                 hi_input, eolbyte);
      else
