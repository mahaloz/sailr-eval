...
static __inline void __cpuidex(int __cpuid_info[4], int __leaf, int __subleaf) {
  __asm__ __volatile__("cpuid\n\t"
                       : "=a"(__cpuid_info[0]), "=b"(__cpuid_info[1]),
                         "=c"(__cpuid_info[2]), "=d"(__cpuid_info[3])
                       : "0"(__leaf), "2"(__subleaf));
}
...
avx2_supported(void) {
  ...
  if (!getcpuid_ok) {
    if (debug)
      error(0, 0, "%s", dcgettext(((void *)0), "failed to get cpuid", 5));
    return 0;
  } else if (!avx_enabled) {
    if (debug)
      error(0, 0, "%s", dcgettext(((void *)0), "avx2 support not detected", 5));
    return 0;
  } else {
    if (debug)
      error(0, 0, "%s",
            dcgettext(((void *)0), "using avx2 hardware support", 5));
    return 1;
  }
}
  ...
  if (status != 0)
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "total line if\nmore than one FILE is specified.  A word is "
                  "a non-zero-length sequence of\nprintable characters "
    ...
    fputs_unlocked(
        dcgettext(((void *)0),
                  ...
                  "printed, always in\nthe following order: newline, word, "
...
__attribute__((__pure__)) static int iswnbspace(wint_t wc) {
  return !posixly_correct &&
         (wc == 0x00A0 || wc == 0x2007 || wc == 0x202F || wc == 0x2060);
}

static int isnbspace(int c) { return iswnbspace(btowc(c)); }

static void write_counts(uintmax_t lines, uintmax_t words, uintmax_t chars,
                         uintmax_t bytes, uintmax_t linelength,
                         char const *file) {
...
wc(int fd, char const *file_x, struct fstatus *fstatus, off_t current_pos) {
  ...
  char const *file =
      file_x ? file_x : dcgettext(((void *)0), "standard input", 5);
...
static struct fstatus *get_input_fstatus(size_t nfiles, char *const *file) {
  ...
  if (nfiles == 0 || (nfiles == 1 && ((print_lines + print_words + print_chars +
...
__attribute__((__pure__)) static int
      ...
      if (!readtokens0(stream, &tok) || rpl_fclose(stream) != 0)
        ((!!sizeof(struct {
          _Static_assert(1, "verify_expr ("
                            ...
                            "read file names from %s\", 5), quotearg_style "
          ...
          int _gl_dummy;
             ...
             ? ((error(1, 0,
                       dcgettext(((void *)0), "cannot read file names from %s",
                                 5),
                       quotearg_style(shell_escape_always_quoting_style,
                                      files_from)),
                 ((0) ? (void)0 : __builtin_unreachable())))
...
argv_iter_done:
  ...
  if (1 < argv_iter_n_args(ai))
    write_counts(total_lines, total_words, total_chars, total_bytes,
                 max_line_length, dcgettext(((void *)0), "total", 5));
